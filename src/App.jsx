...
function ProjectsBrowser({onOpenCalc}){
  ...
  return(
    <div style={{padding:24,textAlign:"center",color:"#868e96"}}>
      ...
      <div style={{fontSize:13,fontFamily:MONO,fontWeight:700,marginBottom:6}}>SAVED CALCULATIONS</div>
      <div style={{fontSize:11}}>Open a saved calculation from the Projects workspace to view it here.</div>
    </div>
  );
}
  ...
  return(
    <div style={{maxWidth:680,margin:"0 auto",padding:"24px 0"}}>
      <div style={{textAlign:"center",marginBottom:32}}>
        ...
        <div style={{fontSize:20,fontWeight:800,fontFamily:MONO,textTransform:"uppercase",letterSpacing:1,color:"#212529"}}>BT Structural Calculator</div>
        <div style={{fontSize:12,color:"#868e96",marginTop:6}}>PCI 8th Ed. · ACI 318-19 · CSA A23.3-19 · CPCI 5th Ed.</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
        {[
          {id:"pci",  icon:"📐",label:"PCI Hollowcore",   desc:"ACI 318-19 precast slab design"},
          {id:"cpci", icon:"🇨🇦",label:"CPCI Hollowcore",  desc:"CSA A23.3-19 precast slab design"},
          {id:"col",  icon:"🏛️",label:"Column Design",    desc:"ACI 318-19 P–M interaction"},
          {id:"crush",icon:"💥",label:"HC End Crushing",  desc:"CPCI core crush capacity"},
          {id:"beam", icon:"📏",label:"Beam Design",      desc:"Composite beam analysis"},
        ].map(m=>(
          <div key={m.id} onClick={()=>onNavigate(m.id)} style={{padding:20,background:"#fff",border:"1px solid #dee2e6",borderRadius:8,cursor:"pointer",transition:"box-shadow 0.15s"}}
            onMouseEnter={e=>e.currentTarget.style.boxShadow="0 4px 16px rgba(0,0,0,0.1)"}
            onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
            <div style={{fontSize:28,marginBottom:8}}>{m.icon}</div>
            <div style={{fontSize:13,fontWeight:700,marginBottom:4}}>{m.label}</div>
            <div style={{fontSize:11,color:"#868e96"}}>{m.desc}</div>
          </div>
        ))}
      </div>
    </div>
...
// PCI TAB
// ═════════════════════════════════════════
function PCITab({loadedCalc, onConsumedLoad, workspace, uiMode}){
  ...
  if(uiMode==="profis"){
    ...
    const sections = [
      ...
      {id:"loads",    icon:"⚖️", label:"Loads"},
      {id:"concrete", icon:"🧱", label:"Material"},
    ...
    const SectionHeader = ({id, title}) => (
      <div onClick={()=>toggleSection(id)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px",background:"#f0f0f0",borderBottom:"1px solid #ddd",cursor:"pointer",userSelect:"none"}}>
        <span style={{fontSize:11,fontWeight:700,color:"#333",letterSpacing:0.5,textTransform:"uppercase"}}>{title}</span>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          <span style={{fontSize:11,color:"#999"}}>☆</span>
          <span style={{fontSize:13,color:"#666"}}>{collapsed[id]?"∨":"∧"}</span>
        </div>
      </div>
    );

    const Spinner = ({label, value, onChange, unit, step=1}) => (
      <div style={{marginBottom:10}}>
        {label&&<div style={{fontSize:10,color:"#555",marginBottom:3}}>{label}</div>}
        <div style={{display:"flex",alignItems:"stretch",border:"1px solid #ccc",borderRadius:2,overflow:"hidden"}}>
          <input type="number" value={value} step={step} onChange={e=>onChange(Number(e.target.value))}
            style={{flex:1,padding:"4px 8px",border:"none",fontSize:12,outline:"none",background:"#f9f9f9"}}/>
          {unit&&<span style={{padding:"0 6px",background:"#f0f0f0",display:"flex",alignItems:"center",fontSize:10,color:"#666",borderLeft:"1px solid #ccc"}}>{unit}</span>}
          <div style={{display:"flex",flexDirection:"column",borderLeft:"1px solid #ccc"}}>
            <button onClick={()=>onChange(value+step)} style={{flex:1,padding:"0 5px",border:"none",background:"#f0f0f0",cursor:"pointer",fontSize:10,lineHeight:1}}>+</button>
            <button onClick={()=>onChange(value-step)} style={{flex:1,padding:"0 5px",border:"none",background:"#f0f0f0",cursor:"pointer",fontSize:10,lineHeight:1,borderTop:"1px solid #ccc"}}>-</button>
          </div>
        </div>
      </div>
    );
    ...
    const tbBtn = (active) => ({
      ...
      background:active?"#212529":"#f5f5f5", color:active?"#fff":"#555",
      fontWeight:700, fontSize:11, cursor:"pointer", fontFamily:MONO,
    });

    // Calculation steps for the bottom panel
    const CalcSteps = () => {
      if(!r) return <div style={{padding:16,color:"#999",fontSize:11}}>Computing…</div>;
      const f = (v,d=2) => (v==null||isNaN(v)) ? "—" : Number(v).toFixed(d);
      const Row = ({label, formula, result, unit, ok, code}) => (
        <div style={{display:"grid",gridTemplateColumns:"180px 1fr 90px 60px",gap:0,borderBottom:"1px solid #2a2a2a",alignItems:"center",minHeight:26}}>
          <div style={{padding:"3px 10px",fontSize:10,color:"#ccc",borderRight:"1px solid #2a2a2a",fontWeight:600}}>{label}</div>
          <div style={{padding:"3px 10px",fontSize:10,color:"#888",fontFamily:MONO,borderRight:"1px solid #2a2a2a"}}>{formula}</div>
          <div style={{padding:"3px 10px",fontSize:11,fontFamily:MONO,fontWeight:700,textAlign:"right",
            color: ok===true?"#4ade80": ok===false?"#f87171":"#e2e8f0",borderRight:"1px solid #2a2a2a"}}>{result} <span style={{fontSize:9,color:"#666"}}>{unit}</span></div>
          <div style={{padding:"3px 6px",fontSize:9,color:"#555",textAlign:"center"}}>{code}</div>
        </div>
      );
      const GroupHeader = ({title,color="#e8a838"}) => (
        <div style={{padding:"5px 10px",background:"#1a1a1a",fontSize:10,fontWeight:800,color,letterSpacing:1,textTransform:"uppercase",borderBottom:"1px solid #333",display:"flex",alignItems:"center",gap:8}}>
          <span style={{width:3,height:12,background:color,borderRadius:1,display:"inline-block"}}/>
          {title}
        </div>
      );
      return (
        <div style={{fontFamily:SANS}}>
          <GroupHeader title="Section Properties"/>
          <Row label="Strand depth" formula={`dp = h − cover = ${r.s?.h}−${cover}`} result={f(r.dp,3)} unit="in" code="§25.8"/>
          <Row label="Eccentricity" formula="e = yb − cover" result={f(r.e,3)} unit="in"/>
          <Row label="Steel area" formula={`Aps = ${nH}×0.153 + ${nS}×0.217`} result={f(r.Aps,4)} unit="in²" code="ASTM A416"/>
          <Row label="Init. prestress" formula={`fpi = ${fpiR}×fpu = ${fpiR}×270`} result={f(r.fpi,2)} unit="ksi"/>

          <GroupHeader title="Transfer Stresses — ACI 318-19 §24.5.3"/>
          <Row label="Initial force" formula="Po = fpi·Aps·0.95" result={f(r.Po,2)} unit="k"/>
          <Row label="Top @ ends" formula="f = Po/A − Po·e/St + M_sw·12/St" result={f((r.nTtop||0)*1000,1)} unit="psi" ok={r.tensEndOk} code="§24.5.3"/>
          <Row label="Bot @ ends" formula="f = Po/A + Po·e/Sb − M_sw·12/Sb" result={f((r.nTbot||0)*1000,1)} unit="psi" ok={r.compEndOk} code="§24.5.3"/>
          <Row label="Allow. tension" formula="-6√f'ci" result={f(r.aTE,0)} unit="psi" code="§24.5.3.1"/>
          <Row label="Allow. comp." formula="0.70·f'ci" result={f(r.aCE,0)} unit="psi" code="§24.5.3.2"/>

          <GroupHeader title="Prestress Losses — ACI 318-19 §26.10"/>
          <Row label="Elastic Shortening" formula="ES = (Eps/Eci)·fcir" result={f(r.ES,3)} unit="ksi" code="§26.10.2"/>
          <Row label="Creep" formula="CR = 2(Eps/Ec)(fcir−fcds)" result={f(r.CR,3)} unit="ksi" code="§26.10.3"/>
          <Row label="Shrinkage" formula="SH = 8.2×10⁻⁶·Eps·(1−0.06V/S)·(100−RH)" result={f(r.SHv,3)} unit="ksi" code="§26.10.4"/>
          <Row label="Relaxation" formula="RE = (5000/fpu − 0.04·total)·0.75" result={f(r.RE,3)} unit="ksi" code="§26.10.5"/>
          <Row label="Total loss" formula="(ES+CR+SH+RE)/fpi×100" result={f(r.totalLoss,1)} unit="%" code="§26.10"/>
          <Row label="Eff. prestress" formula="fse = fpi·(1−loss/100)" result={f(r.fse,2)} unit="ksi"/>
          <Row label="Eff. force" formula="Pe = Aps·fse" result={f(r.Pe,2)} unit="k"/>

          <GroupHeader title="Service Stresses — ACI 318-19 §24.5.4"/>
          <Row label="Bot @ midspan" formula="f = Pe/A + Pe·e/Sb − Mserv·12/Sb" result={f(r.fbot,4)} unit="ksi" ok={r.classU} code="§24.5.4"/>
          <Row label="Allow. tension" formula="7.5√f'c" result={f(r.aTens,4)} unit="ksi" code="§24.5.4.1"/>
          <Row label="Section class" formula={r.classU?"fbot > −ft → Class U":"fbot < −ft → Class C"} result={r.classU?"Class U":"Class C"} ok={r.classU} code="Table 24.5.2"/>

          <GroupHeader title="Flexural Strength — ACI 318-19 §22.3"/>
          <Row label="Factored moment" formula="Mu = wu·b/12·L²/8" result={f(r.Mu,2)} unit="k-ft" code="§5.3.1"/>
          <Row label="fps (at strength)" formula="fps = Eps·(εse + εd − εdc)" result={f(r.fps3,2)} unit="ksi" code="§20.3.2.4"/>
          <Row label="Depth of stress block" formula="a = (Aps·fps)/(0.85·f'c·b)" result={f(r.a3,3)} unit="in" code="§22.2.2.4"/>
          <Row label="ΦMn" formula="Φ·Aps·fps·(dp − a/2)/12" result={f(r.phiMn3,2)} unit="k-ft" ok={r.flexOk} code="§22.3.2"/>
          <Row label="Utilization" formula="Mu / ΦMn" result={f((r.flexUtil||0)*100,0)} unit="%" ok={r.flexOk}/>

          <GroupHeader title="Shear Strength — ACI 318-19 §22.5"/>
          <Row label="Vu @ d from face" formula="Vu = wu·(L/2 − d/12)" result={f(r.Vu2,2)} unit="k" code="§22.5.1"/>
          <Row label="ΦVcw" formula="Φ·(3.5√f'c + 0.3fpc)·bw·dp" result={f(0.75*(r.Vcw||0),2)} unit="k" ok={r.Vcw&&r.Vu2?(0.75*r.Vcw)>=r.Vu2:true} code="§22.5.8.3"/>

          <GroupHeader title="Camber & Deflection"/>
          <Row label="Initial camber" formula="δi = Pi·e·L²/(8EciI) − 5·w·L⁴/(384EciI)" result={f(r.cI,3)} unit="in"/>
          <Row label="Erection camber" formula="1.8Pi·e·L²/(8EciI) − 1.85·(5·w·L⁴/384EciI)" result={f(r.cE,3)} unit="in"/>
          <Row label="Final camber" formula="2.45·(prestress) − 2.7·(selfwt)" result={f(r.cF,3)} unit="in"/>
          <Row label="Net (after SDL)" formula="δnet = δfinal − δSDL" result={f(r.netC,3)} unit="in"/>
        </div>
      );
    };

    return(
      <div style={{fontFamily:SANS,display:"flex",height:"100vh",overflow:"hidden",background:"#e8e8e8"}}>
        <SaveCalcModal open={saveOpen} onClose={()=>setSaveOpen(false)} onSave={doSave} existingProjects={allCalcs} defaultProject={activeCalc?.project_name||workspace?.project} defaultPart={activeCalc?.part_name||workspace?.part}/>

        {/* FAR LEFT: icon sidebar */}
        <div style={{width:36,flexShrink:0,background:"#2d2d2d",display:"flex",flexDirection:"column",alignItems:"center",paddingTop:8,gap:4,zIndex:10}}>
          {sections.map(s2=>(
            <button key={s2.id} onClick={()=>{setActiveSection(s2.id);setShowInputs(true);}} title={s2.label}
              style={{width:32,height:32,border:"none",borderRadius:3,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",
                background:activeSection===s2.id&&showInputs?"#555":"transparent",color:activeSection===s2.id&&showInputs?"#fff":"#aaa",transition:"all 0.15s"}}>
              {s2.icon}
            </button>
          ))}
        </div>

        {/* LEFT: input panel — toggled by Inputs button */}
        {showInputs&&<div style={{width:230,flexShrink:0,background:"#fff",borderRight:"1px solid #ccc",overflowY:"auto",fontSize:12}}>
          <div style={{padding:"8px 12px",background:"#f5f5f5",borderBottom:"1px solid #ddd",fontSize:11,color:"#555"}}>
            <b style={{color:"#333"}}>{activeSection.toUpperCase()}</b>
          </div>

          {activeSection==="design"&&<>
            <SectionHeader id="section" title="Section"/>
            {!collapsed.section&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{marginBottom:8}}>
                <div style={{fontSize:10,color:"#555",marginBottom:3}}>Slab Type</div>
                <select value={sec} onChange={e=>handleSecChange(e.target.value)}
                  style={{width:"100%",padding:"5px 8px",border:"1px solid #ccc",borderRadius:2,fontSize:12,background:"#f9f9f9"}}>
                  {Object.keys(PCI_SLABS).map(k=><option key={k}>{k}</option>)}
                </select>
              </div>
              <div style={{marginBottom:8}}>
                <div style={{fontSize:10,color:"#555",marginBottom:3}}>Design Standard</div>
                <select style={{width:"100%",padding:"5px 8px",border:"1px solid #ccc",borderRadius:2,fontSize:12,background:"#f9f9f9"}}>
                  <option>ACI 318-19</option><option>CSA A23.3-19</option>
                </select>
              </div>
              {r&&<div style={{padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555",display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
                <span>h = {r.s?.h}"</span><span>b = {r.s?.b}"</span>
                <span>A = {r.s?.A?.toFixed(0)} in²</span><span>Ix = {r.s?.Ix?.toFixed(0)} in⁴</span>
              </div>}
            </div>}
            <SectionHeader id="strands" title="Prestressing"/>
            {!collapsed.strands&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="# Strands (½″)" value={nH} onChange={setNH} step={1}/>
              <Spinner label="fpi / fpu ratio" value={fpiR} onChange={setFpiR} step={0.01}/>
              <Spinner label="Cover (in)" value={cover} onChange={setCover} unit="in" step={0.25}/>
              {r&&<div style={{marginTop:6,padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555"}}>
                <div>dp = {r.dp?.toFixed(3)}" · e = {r.e?.toFixed(3)}"</div>
                <div>Aps = {r.Aps?.toFixed(4)} in²</div>
                <div>fse = {r.fse?.toFixed(2)} ksi · Loss = {r.totalLoss?.toFixed(1)}%</div>
              </div>}
            </div>}
          </>}

          {activeSection==="loads"&&<>
            <SectionHeader id="geomL" title="Geometry"/>
            {!collapsed.geomL&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="Span (ft)" value={span} onChange={setSpan} unit="ft" step={0.5}/>
            </div>}
            <SectionHeader id="loads2" title="Applied Loads"/>
            {!collapsed.loads2&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="SDL (psf)" value={sdl} onChange={setSdl} unit="psf" step={5}/>
              <Spinner label="LL (psf)" value={ll} onChange={setLl} unit="psf" step={5}/>
              {r&&<div style={{marginTop:6,padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555"}}>
                <div>wu = {r.wuL?.toFixed(4)} k/in · Mu = {r.Mu?.toFixed(2)} k-ft</div>
                <div>Vu = {r.Vu2?.toFixed(2)} k @ d from support</div>
              </div>}
            </div>}
            <SectionHeader id="combo" title="Load Combination"/>
            {!collapsed.combo&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{padding:"5px 8px",background:"#f9f9f9",border:"1px solid #ccc",borderRadius:2,fontSize:11,fontFamily:MONO}}>1.2D + 1.6L</div>
              <div style={{fontSize:10,color:"#888",marginTop:4}}>ACI 318-19 §5.3.1</div>
            </div>}
          </>}

          {activeSection==="concrete"&&<>
            <SectionHeader id="basemat" title="Base Material"/>
            {!collapsed.basemat&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
                <input type="checkbox" defaultChecked id="nw2"/>
                <label htmlFor="nw2" style={{fontSize:11,color:"#333"}}>Normal weight concrete</label>
              </div>
              <Spinner label="f'c (ksi)" value={fc} onChange={setFc} unit="ksi" step={0.5}/>
              <Spinner label="f'ci (ksi)" value={fci} onChange={setFci} unit="ksi" step={0.5}/>
              {r&&<div style={{padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555"}}>
                <div>Ec = {r.Ec?.toFixed(0)} ksi · Eci = {r.Eci?.toFixed(0)} ksi</div>
                <div>λ = 1.0 (normal weight)</div>
              </div>}
            </div>}
            <SectionHeader id="allowable" title="Allowable Stresses"/>
            {!collapsed.allowable&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:10,color:"#555",marginBottom:6}}>At Transfer (ACI §24.5.3)</div>
              {r&&<div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:4}}>
                <div style={{display:"flex",justifyContent:"space-between"}}><span>Tension (ends)</span><span style={{color:r.tensEndOk?"#16a34a":"#dc2626"}}>{r.aTE?.toFixed(0)} psi</span></div>
                <div style={{display:"flex",justifyContent:"space-between"}}><span>Compression</span><span>{r.aCE?.toFixed(0)} psi</span></div>
              </div>}
              <div style={{fontSize:10,color:"#555",marginBottom:6,marginTop:8}}>At Service (ACI §24.5.4)</div>
              {r&&<div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:4}}>
                <div style={{display:"flex",justifyContent:"space-between"}}><span>Tension (Class U)</span><span style={{color:r.classU?"#16a34a":"#ea580c"}}>{r.aTens?.toFixed(0)} psi</span></div>
                <div style={{display:"flex",justifyContent:"space-between"}}><span>Compression</span><span>{r.aCM?.toFixed(0)} psi</span></div>
              </div>}
            </div>}
          </>}

          {activeSection==="geometry"&&<>
            <SectionHeader id="sprops" title="Section Properties"/>
            {!collapsed.sprops&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              {r&&<div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                {[["h",r.s?.h,"in"],["b",r.s?.b,"in"],["A",r.s?.A?.toFixed(1),"in²"],["Ix",r.s?.Ix?.toFixed(0),"in⁴"],["yb",r.s?.yb?.toFixed(3),"in"],["St",r.s?.St?.toFixed(1),"in³"],["Sb",r.s?.Sb?.toFixed(1),"in³"],["dp",r.dp?.toFixed(3),"in"],["e",r.e?.toFixed(3),"in"]].map(([l,v,u])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:"1px solid #f3f4f6"}}>
                    <span style={{color:"#555"}}>{l}</span><span><b>{v}</b> <span style={{color:"#999",fontSize:9}}>{u}</span></span>
                  </div>
                ))}
              </div>}
            </div>}
            <SectionHeader id="overrides" title="Dimension Overrides"/>
            {!collapsed.overrides&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="Override h (in)" value={slabHOverride||r?.s?.h||8} onChange={v=>setSlabHOverride(Math.round(v))} unit="in" step={1}/>
              <Spinner label="Override b (in)" value={slabBOverride||r?.s?.b||36} onChange={v=>setSlabBOverride(Math.round(v))} unit="in" step={2}/>
              {(slabHOverride||slabBOverride)&&<button onClick={()=>{setSlabHOverride(null);setSlabBOverride(null);}} style={{width:"100%",padding:"5px",border:"1px solid #ccc",borderRadius:2,background:"#f9f9f9",fontSize:11,cursor:"pointer",color:"#666"}}>Reset to defaults</button>}
            </div>}
          </>}

          {activeSection==="results"&&<>
            <SectionHeader id="stresses" title="Stresses @ Transfer"/>
            {!collapsed.stresses&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              {r&&<div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                {[["Pi",r.Pi?.toFixed(2),"k"],["f_top",((r.nTtop||0)*1000).toFixed(1),"psi"],["f_bot",((r.nTbot||0)*1000).toFixed(1),"psi"],["Allow T",r.aTE?.toFixed(1),"psi"],["Allow C",r.aCE?.toFixed(1),"psi"]].map(([l,v,u])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:"1px solid #f3f4f6"}}>
                    <span style={{color:"#555"}}>{l}</span><span><b>{v}</b> <span style={{color:"#999",fontSize:9}}>{u}</span></span>
                  </div>
                ))}
            ...
            <SectionHeader id="losses" title="Prestress Losses"/>
            {!collapsed.losses&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              {r&&<div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                {[["ES",r.ES?.toFixed(3),"ksi"],["CR",r.CR?.toFixed(3),"ksi"],["SH",r.SHv?.toFixed(3),"ksi"],["RE",r.RE?.toFixed(3),"ksi"],["Total",r.totalLoss?.toFixed(1),"%"],["fse",r.fse?.toFixed(2),"ksi"]].map(([l,v,u])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:"1px solid #f3f4f6"}}>
                    <span style={{color:"#555"}}>{l}</span><span><b>{v}</b> <span style={{color:"#999",fontSize:9}}>{u}</span></span>
                  </div>
                ))}
            ...
            <SectionHeader id="flexure2" title="Flexural Strength"/>
            {!collapsed.flexure2&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              {r&&<div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                {[["fps",r.fps3?.toFixed(2),"ksi"],["a",r.a3?.toFixed(3),"in"],["ΦMn",r.phiMn3?.toFixed(2),"k-ft"],["Mu",r.Mu?.toFixed(2),"k-ft"],["Util",((r.flexUtil||0)*100).toFixed(0),"%"]].map(([l,v,u])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:"1px solid #f3f4f6"}}>
                    <span style={{color:"#555"}}>{l}</span><span><b>{v}</b> <span style={{color:"#999",fontSize:9}}>{u}</span></span>
                  </div>
                ))}
        ...
        {/* CENTER: toolbar + 3D + calc steps */}
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>

          {/* Toolbar — Steps/Graphics/Inputs/Outputs toggles */}
          <div style={{display:"flex",alignItems:"center",gap:4,padding:"4px 8px",background:"#fff",borderBottom:"1px solid #ccc",flexShrink:0}}>
            {["↺","↻"].map((b,i)=><button key={i} style={{width:28,height:28,border:"1px solid #ccc",borderRadius:2,background:"#f5f5f5",cursor:"pointer",fontSize:12}}>{b}</button>)}
            <div style={{display:"flex",alignItems:"center",border:"1px solid #ccc",borderRadius:2,overflow:"hidden",height:28}}>
              <input defaultValue="50%" style={{width:40,border:"none",padding:"0 4px",fontSize:11,textAlign:"center",background:"#f9f9f9"}}/>
              <div style={{display:"flex",flexDirection:"column",borderLeft:"1px solid #ccc"}}>
                <button style={{border:"none",background:"#f0f0f0",cursor:"pointer",fontSize:9,padding:"0 4px",lineHeight:"14px"}}>+</button>
                <button style={{border:"none",background:"#f0f0f0",cursor:"pointer",fontSize:9,padding:"0 4px",lineHeight:"14px",borderTop:"1px solid #ccc"}}>-</button>
              </div>
            </div>
            {["🔍","⊙","👁"].map((b,i)=><button key={i} style={{width:28,height:28,border:"1px solid #ccc",borderRadius:2,background:"#f5f5f5",cursor:"pointer",fontSize:12}}>{b}</button>)}
            <div style={{width:1,height:20,background:"#ddd",margin:"0 4px"}}/>
            {/* Functional toggles */}
            <button style={tbBtn(showSteps)}    onClick={()=>setShowSteps(v=>!v)}>Steps</button>
            <button style={tbBtn(showGraphics)} onClick={()=>setShowGraphics(v=>!v)}>Graphics</button>
            <button style={tbBtn(showInputs)}   onClick={()=>setShowInputs(v=>!v)}>Inputs</button>
            <button style={tbBtn(showOutputs)}  onClick={()=>setShowOutputs(v=>!v)}>Outputs</button>
            <div style={{flex:1}}/>
            {r&&<span style={{fontSize:10,color:"#555",fontFamily:MONO}}>{sec} · h={r.s?.h}" · b={r.s?.b}" · L={span}ft · dp={r.dp?.toFixed(2)}"</span>}
          </div>

          {/* 3D model — hidden when Graphics off */}
          {showGraphics&&<div style={{flex:1,overflow:"hidden",background:"#e8e8e8",minHeight:0,position:"relative"}}>
            {r&&<Viewer3D height="fill"
              initialDistance={Math.max(r.s.b,span*2)*0.18+8}
              deps={[r.s.h,r.s.b,r.s.cores,r.s.coreD,nH,nS,r.dp,span]}
              buildScene={(group,THREE,helpers)=>buildHollowcoreScene(group,THREE,{h:r.s.h,b:r.s.b,length:span*12,cores:r.s.cores,coreD:r.s.coreD,nStrands:nH+nS,dp:r.dp},helpers)}
              editableDims={{
                length:{value:Math.round(span*12),unit:"in",step:6,min:0,onChange:(v)=>setSpan(v/12)},
                h:{value:r.s.h,unit:"in",step:1,min:0,onChange:(v)=>setSlabHOverride(Math.round(v))},
                b:{value:r.s.b,unit:"in",step:2,min:0,onChange:(v)=>setSlabBOverride(Math.round(v))},
              }}
            />}
            {!r&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100%",color:"#999"}}>Computing…</div>}
          </div>}

          {/* Bottom status bar — between 3D and steps panel */}
          {r&&<div style={{flexShrink:0,padding:"4px 12px",background:"#fff",borderTop:"1px solid #ccc",borderBottom:showSteps?"1px solid #ccc":"none",display:"flex",gap:16,fontSize:11,alignItems:"center"}}>
            <span><b>ΦMn:</b> {fmt(r.phiMn3,1)} k-ft</span>
            <span><b>Mu:</b> {fmt(r.Mu,1)} k-ft</span>
            <span><b>ΦVcw:</b> {fmt(0.75*(r.Vcw||0),1)} k</span>
            <span><b>Camber:</b> {fmt(r.cF,3)}"</span>
            <span><b>fse:</b> {fmt(r.fse,2)} ksi</span>
            <span><b>Pe:</b> {fmt(r.Pe,1)} k</span>
            <div style={{flex:1}}/>
            <span style={{color:r.allOk?"#16a34a":"#dc2626",fontWeight:700,fontFamily:MONO}}>{r.allOk?"✓ DESIGN OK":"✗ DESIGN FAILS"}</span>
          </div>}

          {/* CALC STEPS PANEL — fixed height drawer below the 3D viewer */}
          {showSteps&&<div style={{flex:1,overflowY:"auto",background:"#111",borderTop:"1px solid #222",minHeight:0}}>
            {/* Steps header */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 12px",background:"#1a1a1a",borderBottom:"1px solid #333",position:"sticky",top:0,zIndex:5}}>
              <span style={{fontSize:10,fontWeight:700,color:"#e8a838",fontFamily:MONO,letterSpacing:1}}>CALCULATION STEPS — PCI 8th Ed. · ACI 318-19</span>
              <span style={{fontSize:10,color:"#555",fontFamily:MONO}}>
                <span style={{color:"#4ade80",marginRight:4}}>▪ PASS</span>
                <span style={{color:"#f87171",marginRight:4}}>▪ FAIL</span>
        ...
        {/* RIGHT: results panel — toggled by Outputs button */}
        {showOutputs&&<div style={{width:240,flexShrink:0,background:"#fff",borderLeft:"1px solid #ccc",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{background:"#333",padding:"10px 14px",flexShrink:0}}>
            <button onClick={()=>setSaveOpen(true)} style={{width:"100%",padding:"8px",borderRadius:3,border:"none",background:"#555",color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
              Create report →
            </button>
          </div>
          <div style={{display:"flex",borderBottom:"1px solid #ddd",flexShrink:0}}>
            {["📤","📋","📥"].map((icon,i)=>(
              <button key={i} style={{flex:1,padding:"8px",border:"none",background:i===1?"#e8e8e8":"#fff",borderBottom:i===1?"2px solid #333":"2px solid transparent",fontSize:16,cursor:"pointer"}}>{icon}</button>
            ))}
          </div>
          <div style={{flex:1,overflowY:"auto"}}>
            {r&&<div style={{padding:"8px 10px",background:r.allOk?"#f0fdf4":"#fef2f2",borderBottom:"1px solid #ddd"}}>
              <div style={{fontSize:12,fontWeight:700,color:r.allOk?"#15803d":"#b91c1c"}}>{r.allOk?"✓ DESIGN OK":"✗ DESIGN FAILS"}</div>
              <div style={{fontSize:10,color:r.allOk?"#166534":"#991b1b"}}>Flexural util: {((r.flexUtil||0)*100).toFixed(0)}%</div>
            </div>}
            {r&&[
              {title:"FLEXURE",items:[{icon:"M",label:"Moment",val:r.flexUtil||0,ok:r.flexOk}]},
              {title:"TRANSFER",items:[
                {icon:"↑",label:"Tension @ ends",val:r.aTE?Math.abs((r.nTtop||0)*1000)/Math.abs(r.aTE):0,ok:r.tensEndOk},
                {icon:"↓",label:"Compression @ ends",val:r.aCE?Math.abs((r.nTbot||0)*1000)/r.aCE:0,ok:r.compEndOk},
              ]},
              {title:"SERVICE",items:[
                {icon:"↑",label:"Tension @ mid",val:r.aTens?Math.abs(Math.min(r.fbot||0,0))/r.aTens:0,ok:r.classU},
                {icon:"↓",label:"Compr @ mid",val:r.aCM?Math.abs(r.nMbot||0)*1000/r.aCM:0,ok:r.compMidOk},
              ]},
              {title:"SHEAR",items:[{icon:"V",label:"Web shear (Vcw)",val:r.Vcw&&r.Vu2?r.Vu2/(0.75*r.Vcw):0,ok:r.Vcw&&r.Vu2?(0.75*r.Vcw)>=r.Vu2:true}]},
            ].map((grp,gi)=>(
              <div key={gi} style={{borderBottom:"1px solid #e8e8e8"}}>
                <div style={{padding:"5px 10px",background:"#f5f5f5",fontSize:10,fontWeight:700,color:"#555",letterSpacing:0.5,display:"flex",justifyContent:"space-between"}}>
                  <span>{grp.title}</span><span style={{color:"#aaa"}}>∧</span>
                </div>
                {grp.items.map((item,ii)=>{
                  const pct=Math.min(100,Math.round(Math.max(0,item.val||0)*100));
                  const barColor=item.ok?(pct>80?"#f59e0b":"#22c55e"):"#ef4444";
                  return(
                    <div key={ii} style={{padding:"7px 10px",borderBottom:"1px solid #f0f0f0"}}>
                      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4}}>
                        <span style={{width:18,height:18,border:"1px solid #ddd",borderRadius:2,background:"#f5f5f5",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,flexShrink:0,color:"#555"}}>{item.icon}</span>
                        <span style={{flex:1,fontSize:11,color:"#333"}}>{item.label}</span>
                        <span style={{fontSize:12,fontWeight:800,color:item.ok?"#16a34a":"#dc2626",minWidth:32,textAlign:"right"}}>{pct}%</span>
                      </div>
            ...
            {r&&<div>
              <div style={{padding:"5px 10px",background:"#f5f5f5",fontSize:10,fontWeight:700,color:"#555",letterSpacing:0.5,display:"flex",justifyContent:"space-between",borderBottom:"1px solid #e8e8e8"}}>
                <span>ℹ MESSAGES</span><span style={{color:"#aaa"}}>∧</span>
              </div>
              <div style={{padding:"8px 10px",display:"flex",flexDirection:"column",gap:6}}>
                <div style={{padding:"7px 8px",background:"#fffbeb",border:"1px solid #fde68a",borderRadius:3,fontSize:10,color:"#92400e",display:"flex",gap:6}}>
                  <span>⚠</span><span>Class {r.classU?"U (Uncracked)":"C (Cracked)"} section at service per ACI 318-19 §24.5.2</span>
                </div>
                <div style={{padding:"7px 8px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:3,fontSize:10,color:"#1e40af",display:"flex",gap:6}}>
                  <span>ℹ</span><span>Transfer length = {r.s?.ltr||"60"}dp = {((r.s?.ltr||60)*r.dp||0).toFixed(1)}" per ACI §25.8.7</span>
                </div>
                {!r.flexOk&&<div style={{padding:"7px 8px",background:"#fef2f2",border:"1px solid #fecaca",borderRadius:3,fontSize:10,color:"#991b1b",display:"flex",gap:6}}>
                  <span>✕</span><span>Flexural demand exceeds capacity. Increase strands or section depth.</span>
                </div>}
              </div>
            </div>}
          </div>
...
// COLUMN TAB
// ═════════════════════════════════════════
function ColTab({loadedCalc, onConsumedLoad, workspace, uiMode}){
  ...
  if(uiMode==="profis"){
    ...
    const Select = ({label, value, onChange, options}) => (
      <div style={{marginBottom:10}}>
        ...
        <select value={value} onChange={e=>onChange(e.target.value)} style={{width:"100%",padding:"5px 8px",border:"1px solid #ccc",borderRadius:2,fontSize:12,background:"#f9f9f9"}}>
          {options.map(o=><option key={o}>{o}</option>)}
        </select>
      </div>
    );
    const axialUtil = r.phi*r.PnA>0 ? Pu/(r.phi*r.PnA) : 0;
    const overallOk = r.demandOk;


    const [showSteps,   setShowSteps]   = useState(true);
    const [showGraphics,setShowGraphics]= useState(true);
    const [showInputsP, setShowInputsP] = useState(true);
    const [showOutputs, setShowOutputs] = useState(true);
    const tbBtn = (active) => ({padding:"3px 10px",border:"1px solid #ccc",borderRadius:2,background:active?"#212529":"#f5f5f5",color:active?"#fff":"#555",fontWeight:700,fontSize:11,cursor:"pointer",fontFamily:MONO});

    const ColCalcSteps = () => {
      if(!r) return <div style={{padding:16,color:"#999",fontSize:11}}>Computing…</div>;
      const f = (v,d=2) => (v==null||isNaN(v)) ? "—" : Number(v).toFixed(d);
      const Row = ({label,formula,result,unit,ok,code}) => (
        <div style={{display:"grid",gridTemplateColumns:"160px 1fr 90px 60px",gap:0,borderBottom:"1px solid #2a2a2a",alignItems:"center",minHeight:26}}>
          <div style={{padding:"3px 10px",fontSize:10,color:"#ccc",borderRight:"1px solid #2a2a2a",fontWeight:600}}>{label}</div>
          <div style={{padding:"3px 10px",fontSize:10,color:"#888",fontFamily:MONO,borderRight:"1px solid #2a2a2a"}}>{formula}</div>
          <div style={{padding:"3px 10px",fontSize:11,fontFamily:MONO,fontWeight:700,textAlign:"right",color:ok===true?"#4ade80":ok===false?"#f87171":"#e2e8f0",borderRight:"1px solid #2a2a2a"}}>{result} <span style={{fontSize:9,color:"#666"}}>{unit}</span></div>
          <div style={{padding:"3px 6px",fontSize:9,color:"#555",textAlign:"center"}}>{code}</div>
        </div>
      );
      const GH = ({title,color="#e8a838"}) => (
        <div style={{padding:"4px 10px",background:"#1a1a1a",fontSize:10,fontWeight:800,color,letterSpacing:1,textTransform:"uppercase",borderBottom:"1px solid #333",display:"flex",alignItems:"center",gap:8}}>
          <span style={{width:3,height:12,background:color,borderRadius:1,display:"inline-block"}}/>{title}
        </div>
      );
      return (
        <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif"}}>
          <GH title="Section Properties"/>
          <Row label="Effective depth" formula={`d = h − cover − dtie − dbar/2 = ${h}−${cov}−${f(r.xD,2)}−${f(r.bD/2,2)}`} result={f(r.d,3)} unit="in" code="§22.2"/>
          <Row label="Compression depth" formula={`d' = cover + dtie + dbar/2`} result={f(r.dp,3)} unit="in"/>
          <Row label="Stress block factor" formula={`β₁ = max(0.65, 0.85 − 0.05(f'c−4))`} result={f(r.b1,4)} code="§22.2.2"/>
          <GH title="Point A — Pure Compression (ACI §22.4.2)"/>
          <Row label="Gross area minus As" formula={`Ag = b·h = ${b}×${h}`} result={f(b*h,0)} unit="in²"/>
          <Row label="Pₙ,A" formula="(b·h − ΣAs)·0.85f'c + ΣAs·fy" result={f(r.PnA,1)} unit="k" code="§22.4.2"/>
          <Row label="ΦPₙ,A" formula="Φ·Pₙ,A (Φ=0.65)" result={f(r.phi*r.PnA,1)} unit="k" ok={Pu<=r.phi*r.PnA}/>
          <GH title="Point B — Balanced Condition (ACI §22.4)"/>
          <Row label="Neutral axis depth" formula="cb = d·0.003/(0.003+εy)" result={f(r.cb,3)} unit="in" code="§22.2"/>
          <Row label="Stress block depth" formula="ab = β₁·cb" result={f(r.ab,3)} unit="in"/>
          <Row label="Comp. steel strain" formula="ε's = 0.003(cb−d')/cb" result={f(r.esT,5)}/>
          <Row label="Comp. steel stress" formula="f's = min(fy, Es·|ε's|)" result={f(r.fsT,2)} unit="ksi"/>
          <Row label="Pₙ,B" formula="0.85f'c·ab·b + As'·f's − As·fy" result={f(r.PnB,1)} unit="k"/>
          <Row label="Mₙ,B" formula="ΣF·(arm about centroid)/12" result={f(r.MnB,1)} unit="k-ft"/>
          <GH title="Demand vs. Capacity Check (ACI §22.4)"/>
          <Row label="Demand — Pu" formula="Factored axial load" result={f(Pu,1)} unit="k"/>
          <Row label="Demand — Mu" formula="Factored moment" result={f(Mu,1)} unit="k-ft"/>
          <Row label="ΦMₙ @ Pu" formula="Interpolated from P–M envelope at Pu" result={f(r.phiMnAtPu,1)} unit="k-ft"/>
          <Row label="Moment utilization" formula="Mu / ΦMₙ" result={f((r.util||0)*100,0)} unit="%" ok={Mu<=r.phiMnAtPu}/>
          <Row label="Axial utilization" formula="Pu / ΦPₙ,A" result={f(axialUtil*100,0)} unit="%" ok={Pu<=r.phi*r.PnA}/>
          <Row label="Overall check" formula="All demands ≤ capacity" result={overallOk?"PASS":"FAIL"} ok={overallOk} code="§22.4"/>
        </div>
      );
    };
    ...
    return(
      <div style={{fontFamily:SANS,display:"flex",height:"100vh",overflow:"hidden",background:"#e8e8e8"}}>
        ...
        <div style={{width:36,flexShrink:0,background:"#2d2d2d",display:"flex",flexDirection:"column",alignItems:"center",paddingTop:8,gap:4,zIndex:10}}>
          {sections.map(sec=>(
            <button key={sec.id} onClick={()=>setActiveSection(sec.id)} title={sec.label}
              style={{width:32,height:32,border:"none",borderRadius:3,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",
                background:activeSection===sec.id?"#555":"transparent",color:activeSection===sec.id?"#fff":"#aaa",transition:"all 0.15s"}}>
              {sec.icon}
            </button>
          ))}
        </div>
        ...
        {/* LEFT: input panel */}
        <div style={{width:230,flexShrink:0,background:"#fff",borderRight:"1px solid #ccc",overflowY:"auto",fontSize:12}}>
          <div style={{padding:"8px 12px",background:"#f5f5f5",borderBottom:"1px solid #ddd",fontSize:11,color:"#555"}}>
            <b style={{color:"#333"}}>{activeSection.toUpperCase()}</b>
          </div>

          {activeSection==="design"&&<>
            <SectionHeader id="geom" title="Column Geometry"/>
            {!collapsed.geom&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="b (in)" value={b} onChange={setB} unit="in" step={1}/>
              <Spinner label="h (in)" value={h} onChange={setH} unit="in" step={1}/>
              <Spinner label="L, height (in)" value={colHeight} onChange={v=>setColHeight(Math.max(12,v))} unit="in" step={6}/>
            </div>}
            <SectionHeader id="rebar" title="Reinforcement"/>
            {!collapsed.rebar&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Select label="Bottom Bar" value={bBar} onChange={setBBar} options={Object.keys(REBAR)}/>
              <Spinner label="Bottom Qty" value={bQ} onChange={setBQ} step={1}/>
              <Select label="Top Bar" value={tBar} onChange={setTBar} options={Object.keys(REBAR)}/>
              <Spinner label="Top Qty" value={tQ} onChange={setTQ} step={1}/>
              <Select label="Ties" value={xBar} onChange={setXBar} options={Object.keys(REBAR)}/>
              <Spinner label="Cover (in)" value={cov} onChange={setCov} unit="in" step={0.25}/>
            </div>}
          </>}

          {activeSection==="loads"&&<>
            <SectionHeader id="demand" title="Factored Design Loads"/>
            {!collapsed.demand&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="Pᵤ (kips)" value={Pu} onChange={setPu} unit="kips" step={10}/>
              <Spinner label="Mᵤ (kip-ft)" value={Mu} onChange={setMu} unit="kip-ft" step={5}/>
            </div>}
            <SectionHeader id="combo" title="Load Combination"/>
            {!collapsed.combo&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{padding:"5px 8px",background:"#f9f9f9",border:"1px solid #ccc",borderRadius:2,fontSize:11,fontFamily:MONO}}>1.2D + 1.6L</div>
          ...
          {activeSection==="concrete"&&<>
            ...
            {!collapsed.basemat&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="f'c (ksi)" value={fc} onChange={setFc} unit="ksi" step={0.5}/>
              <div style={{padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555"}}>
                <div>fy = {fy} ksi · Es = {Es.toLocaleString()} ksi</div>
                <div>β₁ = {fmt(r.b1,4)}</div>
              </div>
            </div>}
          </>}

          {activeSection==="geometry"&&<>
            <SectionHeader id="props" title="Section Properties"/>
            {!collapsed.props&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                {[["b",b,"in"],["h",h,"in"],["d",fmt(r.d,3),"in"],["d'",fmt(r.dp,3),"in"],["β₁",fmt(r.b1,4),""]].map(([l,v,u])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:"1px solid #f3f4f6"}}>
                    <span style={{color:"#555"}}>{l}</span><span><b>{v}</b> <span style={{color:"#999",fontSize:9}}>{u}</span></span>
                  </div>
                ))}
          ...
          {activeSection==="results"&&<>
            <SectionHeader id="pa" title="Point A — Pure Compression"/>
            {!collapsed.pa&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO}}>Pₙ = {fmt(r.PnA,1)} kips</div>
            </div>}
            <SectionHeader id="pb" title="Point B — Balanced"/>
            {!collapsed.pb&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                <div>cb = {fmt(r.cb,3)} in · ab = {fmt(r.ab,3)} in</div>
                <div>Pₙ = {fmt(r.PnB,1)} kips</div>
                <div>Mₙ = {fmt(r.MnB,1)} kip-ft</div>
              </div>
            </div>}
            <SectionHeader id="capatpu" title="Capacity @ Demand"/>
            {!collapsed.capatpu&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO}}>ΦMn @ Pu = {fmt(r.phiMnAtPu,1)} kip-ft</div>
            </div>}
          </>}
        
        </div>}
        {/* CENTER: toolbar + 3D fill + calc steps */}
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:4,padding:"4px 8px",background:"#fff",borderBottom:"1px solid #ccc",flexShrink:0}}>
            {["🔍","⊙","👁"].map((bt,i)=><button key={i} style={{width:28,height:28,border:"1px solid #ccc",borderRadius:2,background:"#f5f5f5",cursor:"pointer",fontSize:12}}>{bt}</button>)}
            <div style={{width:1,height:20,background:"#ddd",margin:"0 4px"}}/>
            <button style={tbBtn(showSteps)}    onClick={()=>setShowSteps(v=>!v)}>Steps</button>
            <button style={tbBtn(showGraphics)} onClick={()=>setShowGraphics(v=>!v)}>Graphics</button>
            <button style={tbBtn(showInputsP)}  onClick={()=>setShowInputsP(v=>!v)}>Inputs</button>
            <button style={tbBtn(showOutputs)}  onClick={()=>setShowOutputs(v=>!v)}>Outputs</button>
            <div style={{flex:1}}/>
            <span style={{fontSize:10,color:"#555",fontFamily:MONO}}>b={b}" · h={h}" · L={colHeight}" · Pu={Pu}k · Mu={Mu}k-ft</span>
          </div>

          {showGraphics&&<div style={{flex:1,overflow:"hidden",background:"#e8e8e8",minHeight:0}}>
            <Viewer3D height="fill"
          ...
          <div style={{flexShrink:0,padding:"4px 12px",background:"#fff",borderTop:"1px solid #ccc",borderBottom:showSteps?"1px solid #ccc":"none",display:"flex",gap:16,fontSize:11,alignItems:"center"}}>
            <span><b>ΦPn,A:</b> {fmt(r.phi*r.PnA,1)} k</span>
            <span><b>ΦMn@Pu:</b> {fmt(r.phiMnAtPu,1)} k-ft</span>
            <span><b>Pu:</b> {fmt(Pu,1)} k</span>
            <span><b>Mu:</b> {fmt(Mu,1)} k-ft</span>
            <div style={{flex:1}}/>
            <span style={{color:overallOk?"#16a34a":"#dc2626",fontWeight:700,fontFamily:MONO}}>{overallOk?"✓ DESIGN OK":"✗ DESIGN FAILS"}</span>
          </div>

          {showSteps&&<div style={{flex:1,overflowY:"auto",background:"#111",borderTop:"1px solid #222",minHeight:0}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 12px",background:"#1a1a1a",borderBottom:"1px solid #333",position:"sticky",top:0,zIndex:5}}>
              <span style={{fontSize:10,fontWeight:700,color:"#e8a838",fontFamily:MONO,letterSpacing:1}}>CALCULATION STEPS — ACI 318-19 §22.4 · Column P–M Interaction</span>
              <span style={{fontSize:10,color:"#555",fontFamily:MONO}}><span style={{color:"#4ade80",marginRight:4}}>▪ PASS</span><span style={{color:"#f87171",marginRight:4}}>▪ FAIL</span><span style={{color:"#e2e8f0"}}>▪ value</span></span>
            </div>
            <ColCalcSteps/>
          </div>}
        </div>
        ...
        {showOutputs&&<div style={{width:240,flexShrink:0,background:"#fff",borderLeft:"1px solid #ccc",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{background:"#333",padding:"10px 14px",flexShrink:0}}>
            <button onClick={()=>setSaveOpen(true)} style={{width:"100%",padding:"8px",borderRadius:3,border:"none",background:"#555",color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer"}}>Create report →</button>
          </div>
          <div style={{display:"flex",borderBottom:"1px solid #ddd",flexShrink:0}}>
            {["📤","📋","📥"].map((icon,i)=>(
              <button key={i} style={{flex:1,padding:"8px",border:"none",background:i===1?"#e8e8e8":"#fff",borderBottom:i===1?"2px solid #333":"2px solid transparent",fontSize:16,cursor:"pointer"}}>{icon}</button>
          ...
          <div style={{flex:1,overflowY:"auto"}}>
            <div style={{padding:"8px 10px",background:overallOk?"#f0fdf4":"#fef2f2",borderBottom:"1px solid #ddd"}}>
              <div style={{fontSize:12,fontWeight:700,color:overallOk?"#15803d":"#b91c1c"}}>{overallOk?"✓ DESIGN OK":"✗ DESIGN FAILS"}</div>
              <div style={{fontSize:10,color:overallOk?"#166534":"#991b1b"}}>Moment util: {((r.util||0)*100).toFixed(0)}%</div>
            </div>
            {[
              {title:"FLEXURE",items:[{icon:"M",label:"Moment @ Pu",val:r.util||0,ok:Mu<=r.phiMnAtPu}]},
              {title:"AXIAL",items:[{icon:"P",label:"Axial load",val:axialUtil,ok:Pu<=r.phi*r.PnA}]},
            ].map((grp,gi)=>(
              <div key={gi} style={{borderBottom:"1px solid #e8e8e8"}}>
                <div style={{padding:"5px 10px",background:"#f5f5f5",fontSize:10,fontWeight:700,color:"#555",letterSpacing:0.5}}>{grp.title}</div>
                {grp.items.map((item,ii)=>{
                  const pct=Math.min(100,Math.round(Math.max(0,item.val||0)*100));
                  const barColor=item.ok?(pct>80?"#f59e0b":"#22c55e"):"#ef4444";
                  return(<div key={ii} style={{padding:"7px 10px",borderBottom:"1px solid #f0f0f0"}}>
                    <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4}}>
                      <span style={{width:18,height:18,border:"1px solid #ddd",borderRadius:2,background:"#f5f5f5",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,flexShrink:0,color:"#555"}}>{item.icon}</span>
                      <span style={{flex:1,fontSize:11,color:"#333"}}>{item.label}</span>
                      <span style={{fontSize:12,fontWeight:800,color:item.ok?"#16a34a":"#dc2626",minWidth:32,textAlign:"right"}}>{pct}%</span>
                    </div>
                    <div style={{height:5,background:"#e5e7eb",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:barColor,borderRadius:2,transition:"width 0.4s"}}/></div>
                  </div>);
                })}
              </div>
            ))}
            <div>
              <div style={{padding:"5px 10px",background:"#f5f5f5",fontSize:10,fontWeight:700,color:"#555",letterSpacing:0.5,borderBottom:"1px solid #e8e8e8"}}>ℹ MESSAGES</div>
              <div style={{padding:"8px 10px",display:"flex",flexDirection:"column",gap:6}}>
                <div style={{padding:"7px 8px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:3,fontSize:10,color:"#1e40af",display:"flex",gap:6}}>
                  <span>ℹ</span><span>Φ = 0.65 for tied compression member per ACI 318-19 §21.2.2 (simplified)</span>
                </div>
                {!overallOk&&<div style={{padding:"7px 8px",background:"#fef2f2",border:"1px solid #fecaca",borderRadius:3,fontSize:10,color:"#991b1b",display:"flex",gap:6}}>
                  <span>✕</span><span>Demand exceeds capacity. Increase section size or reinforcement.</span>
                </div>}
              </div>
            </div>
          </div>
...
// CPCI TAB
// ═════════════════════════════════════════
function CPCITab({loadedCalc, onConsumedLoad, workspace, uiMode}){
  ...
  if(uiMode==="profis"){
    ...
    const [showStepsC,   setShowStepsC]   = useState(true);
    const [showGraphicsC,setShowGraphicsC]= useState(true);
    const [showInputsC,  setShowInputsC]  = useState(true);
    const [showOutputsC, setShowOutputsC] = useState(true);
    const overallOk = r.Mr>r.Mf && r.Vc>r.Vf && r.topT>r.aTe && r.botT<r.aCe;
    const flexUtil = r.Mr>0 ? r.Mf/r.Mr : 0;
    const shearUtil = r.Vc>0 ? r.Vf/r.Vc : 0;
    const tensUtil = r.aTe!==0 ? Math.abs(r.topT/r.aTe) : 0;
    const compUtil = r.aCe>0 ? Math.abs(r.botT/r.aCe) : 0;
    ...
    return(
      <div style={{fontFamily:SANS,display:"flex",height:"100vh",overflow:"hidden",background:"#e8e8e8"}}>
        ...
        {/* LEFT: input panel */}
        {showInputsC&&<div style={{width:230,flexShrink:0,background:"#fff",borderRight:"1px solid #ccc",overflowY:"auto",fontSize:12}}>
          <div style={{padding:"8px 12px",background:"#f5f5f5",borderBottom:"1px solid #ddd",fontSize:11,color:"#555"}}>
            <b style={{color:"#333"}}>{activeSection.toUpperCase()}</b>
          </div>
          ...
          {activeSection==="design"&&<>
            ...
            {!collapsed.section&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:10,color:"#555",marginBottom:3}}>Slab Type</div>
              <select value={sec} onChange={e=>setSec(e.target.value)} style={{width:"100%",padding:"5px 8px",border:"1px solid #ccc",borderRadius:2,fontSize:12,background:"#f9f9f9",marginBottom:8}}>
                {Object.keys(CPCI_SLABS).map(k=><option key={k}>{k}</option>)}
              </select>
              <div style={{padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555",display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
                <span>h = {r.s.h} mm</span><span>b = {r.s.b} mm</span>
                <span>A = {r.s.A} mm²</span><span>bw = {r.s.bw} mm</span>
              </div>
            </div>}
            <SectionHeader id="strands" title="Prestressing"/>
            {!collapsed.strands&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="# 13mm Strands" value={nH} onChange={setNH} step={1}/>
              <Spinner label="# 15mm Strands" value={nS} onChange={setNS} step={1}/>
              <Spinner label="Strand Height (mm)" value={stH} onChange={setStH} unit="mm" step={1}/>
              <Spinner label="Cover (mm)" value={cover} onChange={setCover} unit="mm" step={1}/>
              <div style={{marginTop:6,padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555"}}>
                <div>Aps = {r.Aps} mm² · e = {fmt(r.e,1)} mm</div>
                <div>fpi = {fmt(r.fpi,0)} MPa · dp = {fmt(r.dp,1)} mm</div>
                <div>Total loss = {fmt(r.totalLoss,1)}%</div>
              </div>
            </div>}
          </>}

          {activeSection==="loads"&&<>
            <SectionHeader id="span" title="Span"/>
            {!collapsed.span&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="Span (m)" value={spanM} onChange={setSpanM} unit="m" step={0.5}/>
            </div>}
            <SectionHeader id="loads2" title="Applied Loads"/>
            {!collapsed.loads2&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="SDL (kPa)" value={sdl} onChange={setSdl} unit="kPa" step={0.1}/>
              <Spinner label="LL (kPa)" value={ll} onChange={setLl} unit="kPa" step={0.1}/>
              <Spinner label="SL (kPa)" value={sl} onChange={setSl} unit="kPa" step={0.1}/>
              <div style={{marginTop:6,padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555"}}>
                <div>Mf = {fmt(r.Mf,2)} kNm · Vf = {fmt(r.Vf,2)} kN</div>
              </div>
            </div>}
          </>}
          ...
          {activeSection==="concrete"&&<>
            <SectionHeader id="basemat" title="Base Material"/>
            {!collapsed.basemat&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="f'c (MPa)" value={fc} onChange={setFc} unit="MPa" step={5}/>
              <Spinner label="f'ci (MPa)" value={fci} onChange={setFci} unit="MPa" step={5}/>
              <Spinner label="Aggregate (mm)" value={ag} onChange={setAg} unit="mm" step={1}/>
              <div style={{padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555"}}>
                <div>Ec = {r.Ec} MPa · Eci = {r.Eci} MPa</div>
                <div>α₁ = {fmt(r.a1,3)} · β₁ = {fmt(r.b1,3)}</div>
              </div>
            </div>}
            <SectionHeader id="allowable" title="Allowable Stresses"/>
            {!collapsed.allowable&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:10,color:"#555",marginBottom:6}}>At Transfer (CSA A23.3-19 §18.3.1)</div>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:4}}>
                <div style={{display:"flex",justifyContent:"space-between"}}><span>Tension (end)</span><span style={{color:r.topT>r.aTe?"#16a34a":"#dc2626"}}>{fmt(r.aTe,2)} MPa</span></div>
                <div style={{display:"flex",justifyContent:"space-between"}}><span>Compression (end)</span><span>{fmt(r.aCe,1)} MPa</span></div>
              </div>
            </div>}
          </>}
          ...
          {activeSection==="geometry"&&<>
            ...
            {!collapsed.props&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                {[["h",r.s.h,"mm"],["b",r.s.b,"mm"],["bw",r.s.bw,"mm"],["A",r.s.A,"mm²"],["Sb",fmt(r.Sb,0),"mm³"],["St",fmt(r.St,0),"mm³"],["dp",fmt(r.dp,1),"mm"],["e",fmt(r.e,1),"mm"]].map(([l,v,u])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:"1px solid #f3f4f6"}}>
                    <span style={{color:"#555"}}>{l}</span><span><b>{v}</b> <span style={{color:"#999",fontSize:9}}>{u}</span></span>
                  </div>
                ))}
          ...
          {activeSection==="results"&&<>
            <SectionHeader id="transfer" title="Stresses @ Transfer"/>
            {!collapsed.transfer&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                <div>Po = {fmt(r.Po,2)} kN</div>
                <div>f_top = {fmt(r.topT,2)} MPa · f_bot = {fmt(r.botT,1)} MPa</div>
              </div>
            </div>}
            <SectionHeader id="losses" title="Prestress Losses"/>
            {!collapsed.losses&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                <div>ES = {fmt(r.ES,3)} MPa · CR = {fmt(r.CR,3)} MPa</div>
                <div>SH = {fmt(r.SHv,3)} MPa · Total = {fmt(r.totalLoss,1)}%</div>
              </div>
            </div>}
            <SectionHeader id="flexure" title="Flexural & Shear Strength"/>
            {!collapsed.flexure&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                <div>Mr = {fmt(r.Mr,2)} kNm · Mf = {fmt(r.Mf,2)} kNm</div>
                <div>Vc = {fmt(r.Vc,2)} kN · Vf = {fmt(r.Vf,2)} kN</div>
                <div>Tcr = {fmt(r.Tcr,2)} kNm</div>
              </div>
            </div>}
          </>}
        </div>}

        {/* CENTER: toolbar + 3D fill + CPCI calc steps */}
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:4,padding:"4px 8px",background:"#fff",borderBottom:"1px solid #ccc",flexShrink:0}}>
            {["🔍","⊙","👁"].map((bt,i)=><button key={i} style={{width:28,height:28,border:"1px solid #ccc",borderRadius:2,background:"#f5f5f5",cursor:"pointer",fontSize:12}}>{bt}</button>)}
            <div style={{width:1,height:20,background:"#ddd",margin:"0 4px"}}/>
            {[["Steps",showStepsC,setShowStepsC],["Graphics",showGraphicsC,setShowGraphicsC],["Inputs",showInputsC,setShowInputsC],["Outputs",showOutputsC,setShowOutputsC]].map(([lbl,val,set])=>(
              <button key={lbl} onClick={()=>set(v=>!v)} style={{padding:"3px 10px",border:"1px solid #ccc",borderRadius:2,background:val?"#212529":"#f5f5f5",color:val?"#fff":"#555",fontWeight:700,fontSize:11,cursor:"pointer",fontFamily:MONO}}>{lbl}</button>
            ))}
            <div style={{flex:1}}/>
            <span style={{fontSize:10,color:"#555",fontFamily:MONO}}>{sec} · h={r.s.h}mm · b={r.s.b}mm · L={spanM}m · dp={fmt(r.dp,1)}mm</span>
          </div>

          {showGraphicsC&&<div style={{flex:1,overflow:"hidden",background:"#e8e8e8",minHeight:0}}>
            <Viewer3D height="fill"
          ...
          <div style={{flexShrink:0,padding:"4px 12px",background:"#fff",borderTop:"1px solid #ccc",borderBottom:showStepsC?"1px solid #ccc":"none",display:"flex",gap:16,fontSize:11,alignItems:"center"}}>
            <span><b>Mr:</b> {fmt(r.Mr,1)} kNm</span>
            <span><b>Mf:</b> {fmt(r.Mf,1)} kNm</span>
            <span><b>Vc:</b> {fmt(r.Vc,1)} kN</span>
            <span><b>Vf:</b> {fmt(r.Vf,1)} kN</span>
            <span><b>Loss:</b> {fmt(r.totalLoss,1)}%</span>
            <div style={{flex:1}}/>
            <span style={{color:overallOk?"#16a34a":"#dc2626",fontWeight:700,fontFamily:MONO}}>{overallOk?"✓ DESIGN OK":"✗ DESIGN FAILS"}</span>
          </div>

          {showStepsC&&<div style={{flex:1,overflowY:"auto",background:"#111",borderTop:"1px solid #222",minHeight:0}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 12px",background:"#1a1a1a",borderBottom:"1px solid #333",position:"sticky",top:0,zIndex:5}}>
              <span style={{fontSize:10,fontWeight:700,color:"#e8a838",fontFamily:MONO,letterSpacing:1}}>CALCULATION STEPS — CSA A23.3-19 · CPCI Design Manual 5th Ed.</span>
              <span style={{fontSize:10,color:"#555",fontFamily:MONO}}><span style={{color:"#4ade80",marginRight:4}}>▪ PASS</span><span style={{color:"#f87171",marginRight:4}}>▪ FAIL</span><span style={{color:"#e2e8f0"}}>▪ value</span></span>
            </div>
            {(()=>{
              const f=(v,d=2)=>(v==null||isNaN(v))?"—":Number(v).toFixed(d);
              ...
              return (<>
                <GH title="Section & Material Properties"/>
                <Row label="Elastic modulus" formula={`Ec = 4500√f'c = 4500√${fc}`} result={f(r.Ec,0)} unit="MPa" code="§8.6.2"/>
                <Row label="Initial modulus" formula={`Eci = 4500√f'ci = 4500√${fci}`} result={f(r.Eci,0)} unit="MPa"/>
                <Row label="Strand area" formula={`Aps = ${nH}×99 + ${nS}×143`} result={f(r.Aps,0)} unit="mm²" code="CPCI §3.2"/>
                <Row label="Eccentricity" formula={`e = yb − stH = ${r.s?.yb||"—"} − ${stH}`} result={f(r.e,1)} unit="mm"/>
                <Row label="Strand depth" formula="dp = yb + e" result={f(r.dp,1)} unit="mm"/>
                <GH title="Transfer Stresses (CSA §18.3.1)"/>
                <Row label="Initial force" formula="Po = fpi·Aps/1000" result={f(r.Po,2)} unit="kN"/>
                <Row label="Top @ transfer" formula="Po/A − Po·e/St + MdT·1e6/St" result={f(r.topT,3)} unit="MPa" ok={r.topT>r.aTe} code="§18.3.1"/>
                <Row label="Bot @ transfer" formula="Po/A + Po·e/Sb − MdT·1e6/Sb" result={f(r.botT,3)} unit="MPa" ok={r.botT<r.aCe}/>
                <Row label="Allow. tension" formula="-0.5√f'ci" result={f(r.aTe,3)} unit="MPa" code="§18.3.1"/>
                <Row label="Allow. comp." formula="0.67·f'ci" result={f(r.aCe,1)} unit="MPa"/>
                <GH title="Prestress Losses (CSA §18.6)"/>
                <Row label="Elastic shortening" formula="(Ep/Eci)·fcir" result={f(r.ES,2)} unit="MPa" code="§18.6"/>
                <Row label="Creep" formula="2(Ep/Ec)·fcir" result={f(r.CR,2)} unit="MPa"/>
                <Row label="Shrinkage" formula="8.2e-6·Ep·(1−0.06V/S)·(100−RH)" result={f(r.SHv,2)} unit="MPa"/>
                <Row label="Total loss" formula="(ES+CR+SH)/fpi×100" result={f(r.totalLoss,2)} unit="%" code="§18.6"/>
                <GH title="Flexural Strength (CSA §18.6.2)"/>
                <Row label="Depth of stress block" formula="c = Aps·fpu/(α₁f'c·b + kp·Aps·fpu/dp)" result={f(r.cpp,2)} unit="mm" code="§10.1.7"/>
                <Row label="Strand stress at Mn" formula="fpr = fpu(1 − kp·c/dp)" result={f(r.fpr,1)} unit="MPa" code="§18.6.2"/>
                <Row label="Mr" formula="Φ·Aps·fpr·(dp − a/2)/1e6" result={f(r.Mr,2)} unit="kNm" ok={r.Mr>r.Mf} code="§18.6.2"/>
                <Row label="Mf" formula="1.25Md + 1.5(SDL+LL+SL)·b/1000·L²/8" result={f(r.Mf,2)} unit="kNm"/>
                <Row label="Flexure utilization" formula="Mf / Mr" result={f((flexUtil)*100,0)} unit="%" ok={r.Mr>r.Mf}/>
                <GH title="Shear Strength (CSA §11.3.4)"/>
                <Row label="Effective shear depth" formula="dv = 0.9·dp" result={f(r.dv,1)} unit="mm" code="§11.3.2"/>
                <Row label="Vc" formula="Φc·β·√f'c·bw·dv / 1000" result={f(r.Vc,2)} unit="kN" ok={r.Vc>r.Vf} code="§11.3.4"/>
                <Row label="Vf" formula="1.25·wSW·L/2 + 1.5·wLL·L/2" result={f(r.Vf,2)} unit="kN"/>
                <Row label="Shear utilization" formula="Vf / Vc" result={f(shearUtil*100,0)} unit="%" ok={r.Vc>r.Vf}/>
              </>);
            })()}
          </div>}
        </div>
        ...
        {/* RIGHT: results panel */}
        {showOutputsC&&<div style={{width:240,flexShrink:0,background:"#fff",borderLeft:"1px solid #ccc",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{background:"#333",padding:"10px 14px",flexShrink:0}}>
            <button onClick={()=>setSaveOpen(true)} style={{width:"100%",padding:"8px",borderRadius:3,border:"none",background:"#555",color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer"}}>Create report →</button>
          </div>
          <div style={{display:"flex",borderBottom:"1px solid #ddd",flexShrink:0}}>
          ...
          <div style={{flex:1,overflowY:"auto"}}>
            <div style={{padding:"8px 10px",background:overallOk?"#f0fdf4":"#fef2f2",borderBottom:"1px solid #ddd"}}>
              <div style={{fontSize:12,fontWeight:700,color:overallOk?"#15803d":"#b91c1c"}}>{overallOk?"✓ DESIGN OK":"✗ DESIGN FAILS"}</div>
              <div style={{fontSize:10,color:overallOk?"#166534":"#991b1b"}}>Flexural util: {(flexUtil*100).toFixed(0)}%</div>
            </div>
            {[
              {title:"FLEXURE",items:[{icon:"M",label:"Moment (Mr ≥ Mf)",val:flexUtil,ok:r.Mr>r.Mf}]},
              {title:"TRANSFER",items:[
                {icon:"↑",label:"Tension @ ends",val:tensUtil,ok:r.topT>r.aTe},
                {icon:"↓",label:"Compression @ ends",val:compUtil,ok:r.botT<r.aCe},
              ]},
              {title:"SHEAR",items:[{icon:"V",label:"Shear (Vc ≥ Vf)",val:shearUtil,ok:r.Vc>r.Vf}]},
            ].map((grp,gi)=>(
              <div key={gi} style={{borderBottom:"1px solid #e8e8e8"}}>
                <div style={{padding:"5px 10px",background:"#f5f5f5",fontSize:10,fontWeight:700,color:"#555",letterSpacing:0.5}}>{grp.title}</div>
                {grp.items.map((item,ii)=>{
                  const pct=Math.min(100,Math.round(Math.max(0,item.val||0)*100));
                  const bc=item.ok?(pct>80?"#f59e0b":"#22c55e"):"#ef4444";
                  return(<div key={ii} style={{padding:"7px 10px",borderBottom:"1px solid #f0f0f0"}}>
                    <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:4}}>
                      <span style={{width:18,height:18,border:"1px solid #ddd",borderRadius:2,background:"#f5f5f5",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,flexShrink:0,color:"#555"}}>{item.icon}</span>
                      <span style={{flex:1,fontSize:11,color:"#333"}}>{item.label}</span>
                    ...
                    <div style={{height:5,background:"#e5e7eb",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:pct+"%",background:bc,borderRadius:2,transition:"width 0.4s"}}/></div>
                  </div>);
                })}
              </div>
            ))}
            <div>
              ...
              <div style={{padding:"8px 10px",display:"flex",flexDirection:"column",gap:6}}>
                <div style={{padding:"7px 8px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:3,fontSize:10,color:"#1e40af",display:"flex",gap:6}}>
                  <span>ℹ</span><span>Design per CSA A23.3-19 and CPCI Design Manual, 5th Edition</span>
                </div>
                {!overallOk&&<div style={{padding:"7px 8px",background:"#fef2f2",border:"1px solid #fecaca",borderRadius:3,fontSize:10,color:"#991b1b",display:"flex",gap:6}}>
                  <span>✕</span><span>One or more checks fail. Review flexure, shear, or transfer stresses.</span>
                </div>}
              </div>
            </div>
          </div>
...
// HC CRUSH TAB
// ═════════════════════════════════════════
function CrushTab({loadedCalc, onConsumedLoad, workspace, uiMode}){
  ...
  if(uiMode==="profis"){
    ...
    const [showStepsCr,   setShowStepsCr]   = useState(true);
    const [showGraphicsCr,setShowGraphicsCr]= useState(true);
    const [showInputsCr,  setShowInputsCr]  = useState(true);
    const [showOutputsCr, setShowOutputsCr] = useState(true);
    return(
      <div style={{fontFamily:SANS,display:"flex",height:"100vh",overflow:"hidden",background:"#e8e8e8"}}>
        <SaveCalcModal open={saveOpen} onClose={()=>setSaveOpen(false)} onSave={doSave} existingProjects={allCalcs} defaultProject={activeCalc?.project_name||workspace?.project} defaultPart={activeCalc?.part_name||workspace?.part}/>
        ...
        <div style={{width:36,flexShrink:0,background:"#2d2d2d",display:"flex",flexDirection:"column",alignItems:"center",paddingTop:8,gap:4,zIndex:10}}>
          {sections.map(s2=>(
            <button key={s2.id} onClick={()=>setActiveSection(s2.id)} title={s2.label}
              style={{width:32,height:32,border:"none",borderRadius:3,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",
                background:activeSection===s2.id?"#555":"transparent",color:activeSection===s2.id?"#fff":"#aaa",transition:"all 0.15s"}}>
              {s2.icon}
            </button>
          ))}
        </div>
        ...
        {/* LEFT: input panel */}
        {showInputsCr&&<div style={{width:230,flexShrink:0,background:"#fff",borderRight:"1px solid #ccc",overflowY:"auto",fontSize:12}}>
          <div style={{padding:"8px 12px",background:"#f5f5f5",borderBottom:"1px solid #ddd",fontSize:11,color:"#555"}}>
            <b style={{color:"#333"}}>{activeSection.toUpperCase()}</b>
          </div>
          ...
          {activeSection==="design"&&<>
            ...
            {!collapsed.section&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:10,color:"#555",marginBottom:3}}>Slab Type</div>
              <select value={sec} onChange={e=>setSec(e.target.value)} style={{width:"100%",padding:"5px 8px",border:"1px solid #ccc",borderRadius:2,fontSize:12,background:"#f9f9f9",marginBottom:8}}>
                {Object.keys(HC_CRUSH).map(k=><option key={k}>{k}</option>)}
              </select>
              <div style={{padding:"6px 8px",background:"#f0f0f0",borderRadius:2,fontSize:10,color:"#555",display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
                <span>h = {r.s.h} mm</span><span>b = {r.s.b} mm</span>
                <span>bw = {r.s.bw} mm</span>
              </div>
            </div>}
          </>}

          {activeSection==="loads"&&<>
            <SectionHeader id="bearing" title="Bearing Conditions"/>
            {!collapsed.bearing&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="Bearing width (mm)" value={w} onChange={setW} unit="mm" step={5}/>
              <Spinner label="Eccentricity (mm)" value={ecc} onChange={setEcc} unit="mm" step={5}/>
              <Spinner label="Bearing height (mm)" value={hB} onChange={setHB} unit="mm" step={5}/>
            </div>}
          </>}
          ...
          {activeSection==="concrete"&&<>
            <SectionHeader id="basemat" title="Base Material"/>
            {!collapsed.basemat&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <Spinner label="f'c (MPa)" value={fc} onChange={setFc} unit="MPa" step={2}/>
            </div>}
          </>}
          ...
          {activeSection==="geometry"&&<>
            ...
            {!collapsed.props&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                {[["h",r.s.h,"mm"],["b",r.s.b,"mm"],["bw",r.s.bw,"mm"],["Re",fmt(r.eF,4),""],["Φ",r.phi,""]].map(([l,v,u])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderBottom:"1px solid #f3f4f6"}}>
                    <span style={{color:"#555"}}>{l}</span><span><b>{v}</b> <span style={{color:"#999",fontSize:9}}>{u}</span></span>
                  </div>
                ))}
          ...
          {activeSection==="results"&&<>
            <SectionHeader id="area" title="Effective Bearing Area"/>
            {!collapsed.area&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                <div>Ae (horiz) = {fmt(r.AeH,1)} mm²/m</div>
                <div>Ae (single) = {fmt(r.AeS,1)} mm²</div>
              </div>
            </div>}
            <SectionHeader id="capacity" title="Crushing Capacity"/>
            {!collapsed.capacity&&<div style={{padding:"10px 12px",borderBottom:"1px solid #eee"}}>
              <div style={{fontSize:11,fontFamily:MONO,display:"grid",gap:3}}>
                <div>ΦPn (horiz) = {fmt(r.PnH,2)} kN/m</div>
                <div>ΦPn (single) = {fmt(r.PnS,2)} kN</div>
              </div>
            </div>}
          </>}
        </div>}

        {/* CENTER: bearing diagram + calc steps */}
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:4,padding:"4px 8px",background:"#fff",borderBottom:"1px solid #ccc",flexShrink:0}}>
            {["🔍","⊙","👁"].map((bt,i)=><button key={i} style={{width:28,height:28,border:"1px solid #ccc",borderRadius:2,background:"#f5f5f5",cursor:"pointer",fontSize:12}}>{bt}</button>)}
            <div style={{width:1,height:20,background:"#ddd",margin:"0 4px"}}/>
            {[["Steps",showStepsCr,setShowStepsCr],["Diagram",showGraphicsCr,setShowGraphicsCr],["Inputs",showInputsCr,setShowInputsCr],["Outputs",showOutputsCr,setShowOutputsCr]].map(([lbl,val,set])=>(
              <button key={lbl} onClick={()=>set(v=>!v)} style={{padding:"3px 10px",border:"1px solid #ccc",borderRadius:2,background:val?"#212529":"#f5f5f5",color:val?"#fff":"#555",fontWeight:700,fontSize:11,cursor:"pointer",fontFamily:MONO}}>{lbl}</button>
            ))}
            <div style={{flex:1}}/>
            <span style={{fontSize:10,color:"#555",fontFamily:MONO}}>{sec} · h={r.s.h}mm · w={w}mm · e={ecc}mm</span>
          </div>
          {showGraphicsCr&&<div style={{flexShrink:0,height:200,overflow:"auto",background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",padding:16,borderBottom:"1px solid #eee"}}>
            <BearingDiagram h={r.s.h} w={w} bw={r.s.bw} b={r.s.b}/>
          </div>}
          <div style={{flexShrink:0,padding:"4px 12px",background:"#fff",borderTop:"1px solid #ccc",borderBottom:showStepsCr?"1px solid #ccc":"none",display:"flex",gap:16,fontSize:11,alignItems:"center"}}>
            <span><b>ΦPn (horiz):</b> {fmt(r.PnH,2)} kN/m</span>
            <span><b>ΦPn (single):</b> {fmt(r.PnS,2)} kN</span>
            <span><b>Re:</b> {fmt(r.eF,4)}</span>
            <div style={{flex:1}}/>
            <span style={{color:"#16a34a",fontWeight:700,fontFamily:MONO}}>✓ CAPACITY COMPUTED</span>
          </div>
          {showStepsCr&&<div style={{flex:1,overflowY:"auto",background:"#111",borderTop:"1px solid #222",minHeight:0}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"5px 12px",background:"#1a1a1a",borderBottom:"1px solid #333",position:"sticky",top:0,zIndex:5}}>
              <span style={{fontSize:10,fontWeight:700,color:"#e8a838",fontFamily:MONO,letterSpacing:1}}>CALCULATION STEPS — CPCI Handbook §4.5 · CSA A23.3-19 §10.8.1</span>
              <span style={{fontSize:10,color:"#555",fontFamily:MONO}}><span style={{color:"#e2e8f0"}}>▪ computed values</span></span>
            </div>
            {(()=>{
              const f=(v,d=2)=>(v==null||isNaN(v))?"—":Number(v).toFixed(d);
              const Row=({label,formula,result,unit,code})=>(
                <div style={{display:"grid",gridTemplateColumns:"180px 1fr 100px 60px",gap:0,borderBottom:"1px solid #2a2a2a",alignItems:"center",minHeight:26}}>
                  <div style={{padding:"3px 10px",fontSize:10,color:"#ccc",borderRight:"1px solid #2a2a2a",fontWeight:600}}>{label}</div>
                  <div style={{padding:"3px 10px",fontSize:10,color:"#888",fontFamily:MONO,borderRight:"1px solid #2a2a2a"}}>{formula}</div>
                  <div style={{padding:"3px 10px",fontSize:11,fontFamily:MONO,fontWeight:700,textAlign:"right",color:"#e2e8f0",borderRight:"1px solid #2a2a2a"}}>{result} <span style={{fontSize:9,color:"#666"}}>{unit}</span></div>
                  <div style={{padding:"3px 6px",fontSize:9,color:"#555",textAlign:"center"}}>{code}</div>
                </div>
              );
              const GH=({title,color="#e8a838"})=>(
              ...
              return (<>
                <GH title="Geometry"/>
                <Row label="Slab depth" formula="h (from section table)" result={r.s.h} unit="mm" code="CPCI §4.5"/>
                <Row label="Slab width" formula="b (from section table)" result={r.s.b} unit="mm"/>
                <Row label="Web width" formula="bw (from section table)" result={r.s.bw} unit="mm"/>
                <Row label="Bearing width" formula="user input w" result={w} unit="mm"/>
                <Row label="Eccentricity" formula="user input e" result={ecc} unit="mm"/>
                <Row label="Bearing height" formula="user input hB" result={hB} unit="mm"/>
                <GH title="Effective Bearing Area (CSA §10.8.1)"/>
                <Row label="Re (horiz)" formula="1 − 2e/h = 1 − 2·ecc/h" result={f(r.eF,4)} unit="" code="§10.8.1"/>
                <Row label="Ae (per metre)" formula="Re · w · b/1000" result={f(r.AeH,1)} unit="mm²/m" code="§10.8.1"/>
                <Row label="Ae (single)" formula="Re · w · bw" result={f(r.AeS,1)} unit="mm²" code="§10.8.1"/>
                <GH title="Crushing Capacity"/>
                <Row label="f'c" formula="user input" result={fc} unit="MPa"/>
                <Row label="Φ" formula="0.65 (bearing)" result={r.phi||0.65} unit="" code="§10.8.1"/>
                <Row label="ΦPn (horiz)" formula="Φ · 0.85 · f'c · Ae(horiz) / 1000" result={f(r.PnH,2)} unit="kN/m" code="§10.8.1"/>
                <Row label="ΦPn (single)" formula="Φ · 0.85 · f'c · Ae(single) / 1000" result={f(r.PnS,2)} unit="kN" code="§10.8.1"/>
              </>);
            })()}
          </div>}
        </div>
        ...
        {/* RIGHT: results panel */}
        {showOutputsCr&&<div style={{width:240,flexShrink:0,background:"#fff",borderLeft:"1px solid #ccc",display:"flex",flexDirection:"column",overflow:"hidden"}}>
          <div style={{background:"#333",padding:"10px 14px",flexShrink:0}}>
            <button onClick={()=>setSaveOpen(true)} style={{width:"100%",padding:"8px",borderRadius:3,border:"none",background:"#555",color:"#fff",fontWeight:700,fontSize:12,cursor:"pointer"}}>Create report →</button>
          </div>
          <div style={{display:"flex",borderBottom:"1px solid #ddd",flexShrink:0}}>
          ...
          <div style={{flex:1,overflowY:"auto"}}>
            <div style={{padding:"8px 10px",background:"#f0fdf4",borderBottom:"1px solid #ddd"}}>
              <div style={{fontSize:12,fontWeight:700,color:"#15803d"}}>✓ CAPACITY COMPUTED</div>
              <div style={{fontSize:10,color:"#166534"}}>No factored reaction supplied — capacities only</div>
            </div>
            <div style={{borderBottom:"1px solid #e8e8e8"}}>
              <div style={{padding:"5px 10px",background:"#f5f5f5",fontSize:10,fontWeight:700,color:"#555",letterSpacing:0.5}}>KEY VALUES</div>
              {[
                {label:"Ae (horiz)",value:fmt(r.AeH,1),unit:"mm²/m"},
                {label:"Ae (single)",value:fmt(r.AeS,1),unit:"mm²"},
                {label:"Re",value:fmt(r.eF,4),unit:""},
                {label:"ΦPn (horiz)",value:fmt(r.PnH,2),unit:"kN/m"},
                {label:"ΦPn (single)",value:fmt(r.PnS,2),unit:"kN"},
              ].map((kv,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"7px 10px",borderBottom:"1px solid #f3f4f6"}}>
                  <span style={{fontSize:11,color:"#374151",fontFamily:MONO}}>{kv.label}</span>
                  <span style={{fontSize:11,fontWeight:700,fontFamily:MONO,color:"#111827"}}>{kv.value} <span style={{fontSize:9,color:"#9ca3af"}}>{kv.unit}</span></span>
                </div>
              ))}
            </div>
            <div>
              ...
              <div style={{padding:"8px 10px",display:"flex",flexDirection:"column",gap:6}}>
                <div style={{padding:"7px 8px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:3,fontSize:10,color:"#1e40af",display:"flex",gap:6}}>
                  <span>ℹ</span><span>Core crushing capacity per CPCI Handbook §4.5 and CSA A23.3-19 §10.8.1. Provide a factored reaction to compare against a demand.</span>
                </div>
              </div>
            </div>
          </div>
...
function ProfisUtilBar({ label, value, ok, color }) {
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 800, fontFamily: MONO, color: ok ? "#16a34a" : "#dc2626" }}>
          {pct}%
        </span>
      </div>
      <div style={{ height: 8, background: "#e5e7eb", borderRadius: 4, overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${pct}%`, background: ok ? color || "#16a34a" : "#dc2626", borderRadius: 4, transition: "width 0.4s ease" }}/>
      </div>
      <div style={{ fontSize: 9, color: ok ? "#16a34a" : "#dc2626", marginTop: 2, fontWeight: 700 }}>
        {ok ? "✓ OK" : "✗ FAILS"} — {pct}% utilized
      </div>
    </div>
  );
}

function ProfisResultsPanel({ r }) {
  if (!r) return <div style={{ padding: 20, color: "#9ca3af", fontSize: 12 }}>Run a calculation to see results</div>;
  const MONO = "'JetBrains Mono','Fira Code','Consolas',monospace";
  ...
  const sections = [
    {
      title: "FLEXURE",
      items: [
        { label: "Moment (ΦMn ≥ Mu)", value: r.flexUtil || 0, ok: r.flexOk, color: "#2563eb" },
      ],
    },
    {
      title: "STRESS AT TRANSFER",
      items: [
        { label: "Tension @ ends", value: r.tensEndOk ? Math.abs((r.nTtop*1000||0)/Math.abs(r.aTE||1)) : 1.1, ok: r.tensEndOk, color: "#7c3aed" },
        { label: "Compression @ ends", value: Math.abs((r.nTbot*1000||0)/(r.aCE||1)), ok: r.compEndOk, color: "#7c3aed" },
      ],
    },
    {
      title: "STRESS AT SERVICE",
      items: [
        { label: "Tension @ midspan", value: r.aTens > 0 ? Math.abs(Math.min(r.fbot||0,0))/r.aTens : 0, ok: r.classU, color: "#0891b2" },
        { label: "Compression @ midspan", value: Math.abs((r.nMbot||0)/(r.aCM||1)), ok: r.compMidOk, color: "#0891b2" },
      ],
    },
    {
      title: "SHEAR",
      items: [
        { label: "Web shear (Vcw)", value: r.Vcw && r.Vu2 ? r.Vu2 / (0.75 * r.Vcw) : 0, ok: r.Vcw && r.Vu2 ? (0.75*r.Vcw) >= r.Vu2 : true, color: "#ea580c" },
      ],
    },
  ];

  return (
    <div style={{ padding: 16, fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
      {/* Overall status */}
      <div style={{ padding: "10px 14px", borderRadius: 6, marginBottom: 16,
        background: r.allOk ? "#f0fdf4" : "#fef2f2",
        border: `2px solid ${r.allOk ? "#16a34a" : "#dc2626"}` }}>
        <div style={{ fontSize: 14, fontWeight: 800, color: r.allOk ? "#15803d" : "#b91c1c", fontFamily: MONO }}>
          {r.allOk ? "✓ DESIGN OK" : "✗ DESIGN FAILS"}
        </div>
        <div style={{ fontSize: 10, color: r.allOk ? "#166534" : "#991b1b", marginTop: 2 }}>
          Flexural util: {((r.flexUtil||0)*100).toFixed(0)}%
        </div>
      </div>

      {sections.map((sec, si) => (
        <div key={si} style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1, color: "#6b7280",
            fontFamily: MONO, marginBottom: 10, paddingBottom: 4,
            borderBottom: "1px solid #e5e7eb" }}>
            {sec.title}
          </div>
          {sec.items.map((item, ii) => (
            <ProfisUtilBar key={ii} label={item.label} value={item.value} ok={item.ok} color={item.color}/>
          ))}
        </div>
      ))}
      ...
      <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: 12, marginTop: 4 }}>
        <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1, color: "#6b7280", fontFamily: MONO, marginBottom: 8 }}>KEY VALUES</div>
        {[
          { label: "ΦMn", value: r.phiMn3, unit: "k-ft" },
          { label: "Mu", value: r.Mu, unit: "k-ft" },
          { label: "ΦVcw", value: r.Vcw ? 0.75*r.Vcw : null, unit: "kips" },
          { label: "Camber (final)", value: r.cF, unit: "in" },
          { label: "Net camber", value: r.netC, unit: "in" },
        ].map((kv, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", borderBottom: "1px solid #f3f4f6" }}>
            <span style={{ fontSize: 11, color: "#374151", fontFamily: MONO }}>{kv.label}</span>
            <span style={{ fontSize: 11, fontWeight: 700, fontFamily: MONO, color: "#111827" }}>
              {kv.value != null ? kv.value.toFixed(2) : "—"} <span style={{ fontSize: 9, color: "#9ca3af" }}>{kv.unit}</span>
            </span>
          </div>
        ))}
      </div>
...
// ═══════════════════════════════════════════════════
// MODE PICKER — Choose interface style after login
// ═══════════════════════════════════════════════════
function ModePicker({ onSelect, userName, onSignOut }) {
  const MONO = "'JetBrains Mono','Fira Code','Consolas',monospace";
  const modes = [
    {
      id: "concise",
      name: "ConCise Mode",
      icon: "📋",
      desc: "Traditional scrollable layout matching ConCise Beam. Full calculation output, step-by-step equations, all phases visible.",
      tags: ["Equations shown", "Scrollable", "Print-ready", "All phases"],
      color: "#2563eb",
    },
    {
      id: "profis",
      name: "Profis Mode",
      icon: "⚡",
      desc: "Three-panel interface inspired by Hilti PROFIS. 3D model center stage, inputs on the left, live utilization results on the right.",
      tags: ["3D always visible", "Live results", "Utilization bars", "Clean UI"],
      color: "#dc2626",
    },
  ];

  return (
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",minHeight:"100vh",background:"#0f172a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{marginBottom:32,textAlign:"center"}}>
        <div style={{fontSize:32,marginBottom:8}}>🏗️</div>
        <div style={{fontSize:20,fontWeight:800,color:"#f8fafc",fontFamily:MONO,textTransform:"uppercase",letterSpacing:1}}>BT Structural Calculator</div>
        <div style={{fontSize:13,color:"#64748b",marginTop:6}}>Welcome, {userName} — choose your interface</div>
      </div>

      <div style={{display:"flex",gap:20,maxWidth:720,width:"100%",flexWrap:"wrap",justifyContent:"center"}}>
        {modes.map(m=>(
          <div key={m.id} onClick={()=>onSelect(m.id)} style={{
            flex:1, minWidth:280, maxWidth:340,
            background:"#1e293b", border:`2px solid ${m.color}40`,
            borderRadius:12, padding:28, cursor:"pointer",
            transition:"all 0.2s",
          }}
            onMouseEnter={e=>{e.currentTarget.style.border=`2px solid ${m.color}`;e.currentTarget.style.background="#273344";e.currentTarget.style.transform="translateY(-2px)";}}
            onMouseLeave={e=>{e.currentTarget.style.border=`2px solid ${m.color}40`;e.currentTarget.style.background="#1e293b";e.currentTarget.style.transform="translateY(0)";}}>
            <div style={{fontSize:36,marginBottom:12}}>{m.icon}</div>
            <div style={{fontSize:16,fontWeight:800,color:"#f1f5f9",marginBottom:8,fontFamily:MONO}}>{m.name}</div>
            <div style={{fontSize:12,color:"#94a3b8",lineHeight:1.6,marginBottom:16}}>{m.desc}</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {m.tags.map(t=>(
                <span key={t} style={{fontSize:10,padding:"3px 8px",borderRadius:10,background:`${m.color}20`,color:m.color,fontWeight:700,fontFamily:MONO}}>{t}</span>
              ))}
            </div>
            <button style={{marginTop:20,width:"100%",padding:"10px",borderRadius:6,border:"none",background:m.color,color:"#fff",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:MONO,letterSpacing:0.5}}>
              USE {m.name.toUpperCase().split(" ")[0]} MODE →
            </button>
          </div>
        ))}
      </div>
      ...
      <div style={{marginTop:24,display:"flex",gap:12,alignItems:"center"}}>
        <span style={{fontSize:11,color:"#475569"}}>You can switch modes anytime from the top bar</span>
        <button onClick={onSignOut} style={{fontSize:11,color:"#475569",background:"none",border:"none",cursor:"pointer",textDecoration:"underline"}}>Sign out</button>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const[uiMode,setUiMode]=useState(null); // null=not chosen, 'concise', 'profis'
  ...
  // Show mode picker after login but before workspace
  if(!uiMode) return(
    <CurrentUserContext.Provider value={user}>
      <ModePicker onSelect={setUiMode} userName={user.name} onSignOut={()=>setUser(null)}/>
  ...
  return(
    ...
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",color:"#212529",background:"#fff"}}>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"12px 14px"}}>
          ...
          <div className="no-print" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,paddingBottom:8,borderBottom:"1px solid #dee2e6"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <button onClick={()=>setUiMode(m=>m==="profis"?"concise":"profis")} className="no-print" style={{fontSize:10,padding:"4px 10px",borderRadius:4,border:"1px solid #dee2e6",background:uiMode==="profis"?"#212529":"#fff",color:uiMode==="profis"?"#fff":"#495057",cursor:"pointer",fontFamily:MONO}}>{uiMode==="profis"?"◧ ConCise":"▣ Profis"}</button>
          ...
          <ModuleThemeContext.Provider value={MODULE_THEMES[ALL_TABS.find(t=>t.id===tab)?.mod||"pci"]}>
            ...
            {tab==="pci"&&<PCITab loadedCalc={loadedCalc?.module==="pci"?loadedCalc:null} onConsumedLoad={()=>setLoadedCalc(null)} workspace={workspace} uiMode={uiMode}/>}
            {tab==="cpci"&&<CPCITab uiMode={uiMode} loadedCalc={loadedCalc?.module==="cpci"?loadedCalc:null} onConsumedLoad={()=>setLoadedCalc(null)} workspace={workspace}/>}
            {tab==="col"&&<ColTab uiMode={uiMode} loadedCalc={loadedCalc?.module==="col"?loadedCalc:null} onConsumedLoad={()=>setLoadedCalc(null)} workspace={workspace}/>}
            {tab==="crush"&&<CrushTab uiMode={uiMode} loadedCalc={loadedCalc?.module==="crush"?loadedCalc:null} onConsumedLoad={()=>setLoadedCalc(null)} workspace={workspace}/>}
            {tab==="beam"&&<BeamTab/>}
        ...
        {/* RIGHT: sticky 3D viewer — only in ConCise mode */}
        {tab!=="dashboard"&&tab!=="projects"&&uiMode!=="profis"&&(
