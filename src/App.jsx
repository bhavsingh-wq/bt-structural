...
// â”€â”€ Shared project storage: data access helpers â”€â”€
...
// â”€â”€ View settings context: controls whether formulas, graphics, inputs, outputs show â”€â”€
...
// â”€â”€ Per-module color identity: PCI=blue, CPCI=purple, Column=green, Crush=orange â”€â”€
const MODULE_THEMES = {
...
// â”€â”€ Employee credentials + roles â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// role: "admin"  â†’ sees all projects, can manage access
//       "user"   â†’ sees only their own projects
const EMPLOYEES = {
...
// â”€â”€ Runtime role overrides (persisted in Supabase user_roles table) â”€â”€
...
// â”€â”€ Role-aware data fetching â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const fmt=(v,d=3)=>v==null||isNaN(v)?"â€”":Number(v).toFixed(d);

// â”€â”€ SVG GRAPHICS â”€â”€
  ...
  return(
    <svg className="live-svg" viewBox={`0 0 ${svgW} ${svgH}`} style={{width:"100%",maxWidth:420,display:"block",margin:"12px auto"}}>
      ...
      <g transform={`translate(${ox}, ${oy+H+64})`}>
        ...
        <text x={20} y={5} fontSize={13} fill="#495057" fontWeight={600} fontFamily="'JetBrains Mono','Fira Code','Consolas',monospace">Bottom: {nBot} Ã— {botLabel}</text>
      </g>
      <g transform={`translate(${ox}, ${oy+H+86})`}>
        <circle cx={6} cy={0} r={7} fill="#2980b9" stroke="#1a4971" strokeWidth={1.5}/>
        <text x={20} y={5} fontSize={13} fill="#495057" fontWeight={600} fontFamily="'JetBrains Mono','Fira Code','Consolas',monospace">Top: {nTop} Ã— {topLabel}</text>
      </g>
    </svg>
  );
}
...
// SCIENTIFIC CHARTS â€” reusable SVG primitives + specific charts
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
// Chart type picker â€” small pill-button row, used by every tab's chart section
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// STATION RESULTS TABLE â€” Phase 3
// Mirrors ConCise Beam's station-by-station output table.
// Shows M(x), V(x), top/bottom fiber stress, Î¦Mn(x), Vn(x),
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// CONSTRUCTION STAGES VIEW â€” Phase 5
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// VIBRATION PANEL â€” Phase 6 + Phase 8
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:12, padding:"10px 12px",
        ...
        <div>
          ...
          <select value={activity} onChange={e=>setActivity(e.target.value)} style={{ ...iS, width:180 }}>
            <option value="">â€” Walking only â€”</option>
      ...
      {phase6 && (<>
        ...
        <div style={{ marginBottom:16 }}>
          <div style={{ fontSize:12, fontWeight:800, fontFamily:MONO, color:"#2563eb", marginBottom:8,
            ...
            WALKING VIBRATION ANALYSIS â€” {occupancy}
          </div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:10 }}>
            ...
            <Stat label="Damping Î²" value={fmt((phase6.walking?.beta||0)*100,0)} unit="%"/>
          </div>
            ...
            <table style={{ borderCollapse:"collapse", width:"100%", fontSize:11 }}>
              <thead>
                <tr style={{ background:"#eff6ff" }}>
                  {["Harmonic","Freq Range (Hz)","DLF Î±","Resonant?","ap/g (%)"].map(h=>(
              ...
              <tbody>
                {(phase6.walking?.harmonicResults||[]).map((h,i)=>(
                  <tr key={i} style={{ background:h.resonant?"#fef3c7":"#fff" }}>
                    ...
                    <td style={{ padding:"4px 8px", border:"1px solid #dee2e6", fontFamily:MONO }}>{h.fi_range[0]}â€“{h.fi_range[1]}</td>
                    ...
                    <td style={{ padding:"4px 8px", border:"1px solid #dee2e6", textAlign:"center", fontWeight:700,
                      color:h.resonant?"#b45309":"#6c757d" }}>{h.resonant?"âš  YES":"No"}</td>
        ...
        {fillCores && phase6.shearAtD && (
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:10 }}>
              ...
              <Stat label="Î¦Vc total" value={fmt(phase6.shearAtD.phiVc,2)} unit="kips"/>
            </div>
                ...
                <table style={{ borderCollapse:"collapse", fontSize:10, fontFamily:MONO }}>
                  <thead>
                    <tr style={{ background:"#fff7ed" }}>
                      {["x (ft)","Zone","bw_eff (in)","Vcw (kip)","VcFill (kip)","Î¦Vc (kip)"].map(h=>(
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// LATERAL STABILITY PANEL â€” Phase 7
// PCI Â§8.3 Mast Method: FS against cracking + rollover during lift
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// MOVING LOAD PANEL â€” Phase 9
// Vehicle selection, axle editor, M/V envelope diagram, results
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// TRANSFORMED SECTION PANEL â€” Phase 10a
// Shows gross vs net vs transformed section properties side by side
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
function TransformedSectionPanel({ data, g1Flag }) {
  const MONO = "'JetBrains Mono','Fira Code','Consolas',monospace";
  const f = (v,d=3) => (v==null||isNaN(v)) ? "â€”" : Number(v).toFixed(d);
  ...
  const Row = ({ label, g, n, t, unit="" }) => (
    <tr>
      ...
      <td style={{ padding:"5px 10px", border:"1px solid #dee2e6", textAlign:"right", fontFamily:MONO, fontSize:10,
        ...
        {g!==0 ? `${((t-g)/Math.abs(g)*100).toFixed(2)}%` : "â€”"}
      </td>
    </tr>
  );
  ...
  return (
    <div>
      <div style={{ marginBottom:10, padding:"8px 12px", background: g1Flag?"#f0fdf4":"#fff7ed",
        ...
        <b>G1 Flag:</b> {g1Flag
          ? "âœ… ON â€” Transformed section used in stress calculations (matches ConCise default)"
          : "âš ï¸ OFF â€” Gross section used. Enable G1 in Options â†’ Calculation Options â†’ General"}
      </div>

      <div style={{ marginBottom:8, fontSize:12, color:"#495057", lineHeight:1.6 }}>
        <b>np = Eps/Ec = </b>{f(gross.np,2)} (prestress modular ratio) &nbsp;Â·&nbsp;
        ...
        <table style={{ borderCollapse:"collapse", width:"100%", fontSize:11 }}>
          <thead>
            <tr style={{ background:"#f8f9fa" }}>
              ...
              <th style={{ padding:"6px 10px", border:"1px solid #dee2e6", textAlign:"right", fontFamily:MONO, fontSize:10 }}>Î” (T vs G)</th>
            </tr>
          </thead>
          <tbody>
            <Row label="A (inÂ²)"   g={gross.A}  n={net.A}  t={transformed.A}/>
            <Row label="Ix (inâ´)"  g={gross.Ix} n={net.Ix} t={transformed.Ix}/>
            <Row label="yb (in)"   g={gross.yb} n={net.yb} t={transformed.yb}/>
            <Row label="yt (in)"   g={gross.yt} n={net.yt} t={transformed.yt}/>
            <Row label="Sb (inÂ³)"  g={gross.Sb} n={net.Sb} t={transformed.Sb}/>
            <Row label="St (inÂ³)"  g={gross.St} n={net.St} t={transformed.St}/>
      ...
      <div style={{ marginTop:10, padding:"8px 12px", background:"#f8f9fa", border:"1px solid #dee2e6", borderRadius:4, fontSize:11 }}>
        ...
        (npâˆ’1)Ã—Aps = <b>{f(transformed.deltaNp,2)} Ã— {f(transformed.A_steel_ps/transformed.deltaNp,4)} = {f(transformed.A_steel_ps,4)} inÂ²</b>
        {transformed.A_steel_s > 0 && <>
          &nbsp;Â·&nbsp; (nsâˆ’1)Ã—As = <b>{f(transformed.A_steel_s,4)} inÂ²</b>
        </>}
      </div>

      <div style={{ marginTop:8, fontSize:10, color:"#6c757d", fontFamily:MONO }}>
        Ref: PCI Design Handbook 8th Ed. Â§4.2.1 Â· ACI 318-19 R24.5.2 Â· ConCise Calculation Option G1
      </div>
    </div>
  );
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// TORSION PANEL â€” Phase 10b
// Full ACI 318-19 Â§22.7 torsion design: Tcr, threshold, stirrups, Al
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
function TorsionPanel({ phase10b, Tu_kft, setTu_kft, torsionAt, setTorsionAt,
  ...
  const MONO = "'JetBrains Mono','Fira Code','Consolas',monospace";
  const f = (v,d=3) => (v==null||isNaN(v)||!isFinite(v)) ? "â€”" : Number(v).toFixed(d);
  ...
  const Check = ({ label, ok, val, limit, unit, note }) => (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
      ...
      <span style={{ color:ok?"#166534":"#991b1b" }}>
        {f(val,3)} {unit} {ok?"â‰¤":">"} {f(limit,3)} {unit}
        <span style={{ marginLeft:8, fontWeight:800, fontSize:10,
          background:ok?"#22c55e":"#ef4444", color:"#fff", padding:"1px 6px", borderRadius:3 }}>
          {ok?"PASS":"FAIL"}
        </span>
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:12, padding:"10px 12px",
        background:"#f8f9fa", border:"1px solid #dee2e6", borderRadius:6 }}>
        <div>
          <div style={{ fontSize:9, color:"#6c757d", marginBottom:2, fontFamily:MONO }}>Factored Torsion Táµ¤</div>
          <div style={{ display:"flex", alignItems:"center", gap:4 }}>
        ...
        <div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:4 }}>
            ...
            <span style={{ fontSize:10, color:"#868e96" }}>inÂ²</span>
          </div>
        </div>
        <div>
      ...
      {phase10b && (<>
        ...
        <div style={{ marginBottom:12, padding:"10px 12px",
          ...
          <b>{phase10b.neglect ? "âœ… Torsion Neglected" : "âš ï¸ Torsion Design Required"}</b>
          <div style={{ fontSize:11, marginTop:4, color:"#495057" }}>{phase10b.neglectReason}</div>
        </div>

        {/* Key values */}
        <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:12 }}>
          <Stat label="Táµ¤ (factored)" value={f(phase10b.Tu_kft,2)} unit="k-ft"/>
          <Stat label="Tth (threshold Î¦Tcr/4)" value={f(phase10b.Tth_kft,3)} unit="k-ft"/>
          <Stat label="Tcr (cracking torque)" value={f(phase10b.Tcr_kft,3)} unit="k-ft"/>
          <Stat label="Aoh" value={f(phase10b.Aoh,1)} unit="inÂ²"/>
          <Stat label="poh" value={f(phase10b.poh,1)} unit="in"/>
          <Stat label="Ao (eff.)" value={f(phase10b.Ao,1)} unit="inÂ²"/>
        </div>
        ...
        {!phase10b.neglect && (<>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:8 }}>
            <Stat label="At/s required" value={f(phase10b.AtS_req,5)} unit="inÂ²/in"/>
            <Stat label="At/s minimum" value={f(phase10b.AtS_min,5)} unit="inÂ²/in"/>
            <Stat label="At/s design" value={f(phase10b.AtS_design,5)} unit="inÂ²/in" highlight/>
            <Stat label="Al required" value={f(phase10b.Al_req,4)} unit="inÂ²" highlight/>
            <Stat label="Al minimum" value={f(phase10b.Al_min,4)} unit="inÂ²"/>
          </div>
          <div style={{ padding:"8px 12px", background:"#f5f3ff", border:"1px solid #ddd6fe",
            borderRadius:6, fontSize:11, fontFamily:MONO }}>
            <b>Design stirrups:</b> At/leg = {f(phase10b.AtS_design * torsionSpacing / torsionLegs,4)} inÂ² per leg
            at s = {torsionSpacing}" with {torsionLegs} legs
            (At/s provided = {f(torsionAt * torsionLegs / torsionSpacing,5)} inÂ²/in)
            <br/>
            <b>Longitudinal steel:</b> Distribute Al = {f(phase10b.Al_design,4)} inÂ² around perimeter of stirrups
          </div>
        </>)}

        <div style={{ marginTop:10, fontSize:10, color:"#6c757d", fontFamily:MONO }}>
          Ref: ACI 318-19 Â§22.7 Â· Space truss analogy Â· Î¸ = 45Â° Â· Ï† = 0.75
          Â· ConCise Define â†’ Torsion Parameters
        </div>
      </>)}
    </div>
  );
...
function MovingLoadPanel({ phase9, vehicle, setVehicle, IM, setIM, gammaLL, setGammaLL,
  ...
  const MONO = "'JetBrains Mono','Fira Code','Consolas',monospace";
  const f = (v,d=2) => (v==null||isNaN(v)) ? "â€”" : Number(v).toFixed(d);
  const iS = { padding:"4px 8px", border:"1.5px solid #e8a838", borderRadius:3,
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:12, padding:"10px 12px",
        ...
        <div>
          <div style={{ fontSize:9, color:"#6c757d", marginBottom:2, fontFamily:MONO }}>Live Load Factor Î³_LL</div>
          <div style={{ display:"flex", alignItems:"center", gap:4 }}>
      ...
      <div style={{ fontSize:11, color:"#495057", marginBottom:10, padding:"6px 10px",
        ...
        <b>{vehicle}</b> â€” {veh.description} &nbsp;Â·&nbsp; Code: <b>{veh.code}</b>
        &nbsp;Â·&nbsp; IM = <b>{f(imVal,2)}</b>
        {veh.lanekplf&&<>&nbsp;Â·&nbsp; Lane load: <b>{veh.lanekplf} kip/ft</b></>}
      </div>

      {/* Axle table â€” editable for Custom */}
                  ...
                  <td style={{ padding:"4px 8px", border:"1px solid #dee2e6", textAlign:"center" }}>
                    <button onClick={()=>setCustomAxles(customAxles.filter((_,j)=>j!==i))}
                      style={{ padding:"2px 8px", border:"1px solid #fecaca", background:"#fef2f2",
                        color:"#991b1b", borderRadius:3, cursor:"pointer", fontSize:10 }}>âœ•</button>
                  </td>
                )}
              </tr>
            ))}
      ...
      <div style={{ border:"1px solid #dee2e6", borderRadius:6, padding:"8px 4px", background:"#fafafa", marginBottom:10 }}>
        ...
        <svg viewBox={`0 0 ${svgW} ${svgH}`} style={{ width:"100%", maxWidth:svgW }}>
          ...
          {/* â”€â”€ Axle loads at critical position â”€â”€ */}
          {axles.map((ax,i)=>{
      ...
      <div style={{ fontSize:10, color:"#6c757d", padding:"6px 10px", background:"#f8f9fa", borderRadius:4, fontFamily:MONO }}>
        Method: MÃ¼ller-Breslau influence lines â€” simply-supported beam Â· {veh.code}
        &nbsp;Â·&nbsp; Combined: {f(gammaLL,2)}Ã—M_LL + M_DL
      </div>
    </div>
  );
}
...
function LateralStabilityPanel({ phase7, liftPt, setLiftPt, yLift, setYLift, sweepIn, setSweepIn, span, sectionLabel }) {
  ...
  const Check = ({ label, val, limit, ok, unit, higherBetter=false }) => (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
      ...
      <span style={{ color:ok?"#14532d":"#7f1d1d", fontWeight:600 }}>{label}</span>
      <span style={{ color:ok?"#166534":"#991b1b" }}>
        {f(val,3)} {unit} {higherBetter?(ok?"â‰¥":"<"):(ok?"â‰¤":">")} {f(limit,3)} {unit}
        <span style={{ marginLeft:8, fontWeight:800, fontSize:10,
          background:ok?"#22c55e":"#ef4444", color:"#fff", padding:"1px 6px", borderRadius:3 }}>
          {ok?"PASS":"FAIL"}
        </span>
    ...
    <div>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:12, padding:"10px 12px",
        ...
        <div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:4 }}>
            ...
            <span style={{ fontSize:10, color:"#868e96" }}>Ã— L = {f(liftPt*span,1)} ft</span>
          </div>
        </div>
        <div>
      ...
      {result && (<>
        ...
        <div style={{ fontSize:11, color:"#495057", marginBottom:10, lineHeight:1.6 }}>
          <b>Section:</b> {sectionLabel} &nbsp;Â·&nbsp;
          <b>Span:</b> {span} ft ({f(span*12,0)} in) &nbsp;Â·&nbsp;
          <b>Lift points at:</b> {f(result.a/12,1)} ft from each end &nbsp;Â·&nbsp;
          <b>Initial sweep z<sub>i</sub>:</b> {f(result.z_i,3)}"
        </div>

        <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:12 }}>
          <Stat label="EIy (lateral)" value={f(result.EIy/1e6,1)} unit="Ã—10â¶ kÂ·inÂ²"/>
          ...
          <Stat label="Roll stiffness KÎ¸" value={f(result.Ktheta,2)} unit="kÂ·in/rad"/>
          <Stat label="Equilibrium tilt Î¸_eq" value={f(result.theta_eq*180/Math.PI,3)} unit="Â°" highlight={result.theta_eq>0.1}/>
        ...
        {/* Stability diagram â€” SVG roll diagram */}
          ...
          <svg viewBox="0 0 480 160" style={{ width:"100%", maxWidth:480 }}>
            ...
            {(() => {
              ...
              return (<>
                ...
                {/* Tilt angle arc */}
                <text x={lx+10} y={20} fontSize={10} fill="#2563eb" fontFamily={MONO}>Î¸={f(theta*180/Math.PI,2)}Â°</text>
                {/* Weight vector */}
        ...
        {/* Code reference */}
        <div style={{ fontSize:10, color:"#6c757d", padding:"6px 10px", background:"#f8f9fa", borderRadius:4, fontFamily:MONO }}>
          Method: Mast (1989) / PCI Design Handbook 8th Ed. Â§8.3 &nbsp;Â·&nbsp;
...
function ConstructionStagesView({ stages }) {
  ...
  const f4 = v => isNaN(v) || v == null ? "â€”" : v.toFixed(4);
  const f2 = v => isNaN(v) || v == null ? "â€”" : v.toFixed(2);
  const f3 = v => isNaN(v) || v == null ? "â€”" : v.toFixed(3);
  const MONO = "'JetBrains Mono','Fira Code','Consolas',monospace";
    ...
    <div>
      ...
      <div style={{ background:"#f8f9fa", border:`1px solid ${STAGE_COLORS[st.id]}30`, borderLeft:`4px solid ${STAGE_COLORS[st.id]}`, borderRadius:4, padding:12, marginBottom:10 }}>
        <div style={{ fontSize:12, fontWeight:700, color:STAGE_COLORS[st.id], marginBottom:4, fontFamily:MONO }}>
          STAGE {st.id}: {st.name.toUpperCase()}  Â·  Day {st.day}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:6, marginBottom:10 }}>
          {[
            ...
            { label:"Moment @ midspan", value:st.M_mid!=null?`${f2(st.M_mid/12)} kip-ft`:"â€”" },
            st.M_overhang!=null&&{ label:"M @ overhang/ends", value:`${f2(st.M_overhang/12)} kip-ft` },
            st.dynFactor&&{ label:"Dynamic factor", value:`Ã—${st.dynFactor}` },
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:10 }}>
          {[
            ...
            ["Net Deflection", st.netDefl!=null?st.netDefl:st.netDefl_sus, "in (â†‘ = camber)"],
            st.liveRatio&&["L/Î´_LL", `L/${Math.round(st.liveRatio)}`, ""],
        ...
        {st.id===5 && st.phiMn && (
          <div style={{ marginTop:8, padding:"6px 10px", background:"#fff", border:"1px solid #dee2e6", borderRadius:4, fontSize:11, fontFamily:MONO }}>
            Î¦Mâ‚™ = <b>{f2(st.phiMn)}</b> kip-ft &nbsp;Â·&nbsp;
            Máµ¤ = <b>{f2(st.Mu_ft)}</b> kip-ft &nbsp;Â·&nbsp;
  ...
  return (
    <div>
        ...
        <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 520 }}>
          <thead>
            <tr>
              ...
              {activeView === "moment" && (<>
                <th style={th}>Máµ¤ (kip-ft)</th>
                <th style={th}>Váµ¤ (kips)</th>
                <th style={th}>Mâ‚›áµ¥c (kip-ft)</th>
              </>)}
              {activeView === "stress" && (<>
              ...
              {activeView === "flex" && (<>
                <th style={th}>Máµ¤ (kip-ft)</th>
                <th style={th}>Î¦Mâ‚™ (kip-ft)</th>
                <th style={th}>Má¶œÊ³ (kip-ft)</th>
                <th style={th}>Máµ¤/Î¦Mâ‚™</th>
                <th style={th}>Status</th>
              </>)}
              {activeView === "shear" && (<>
                <th style={th}>|Váµ¤| (kips)</th>
                <th style={th}>Î¦Vâ‚™ (kips)</th>
                <th style={th}>Váµ¤/Î¦Vâ‚™</th>
                <th style={th}>Status</th>
              </>)}
              {activeView === "deflect" && (<>
                <th style={th}>Î´ @ Release (in)</th>
                <th style={th}>Î´ @ Erection (in)</th>
                <th style={th}>Î´ Final Sus. (in)</th>
                <th style={th}>Î´ Final Total (in)</th>
                <th style={th}>L/Î´</th>
              </>)}
            </tr>
          </thead>
            ...
            {stations.map((st, i) => (
              <tr key={i}>
                ...
                {activeView === "stress" && (<>
                  ...
                  <td style={td(null, true)}>âˆ’0.424</td>
                ...
                {activeView === "deflect" && (<>
                  ...
                  <td style={td(null, true)}>{st.Lratio > 9000 ? "â€”" : `L/${st.Lratio}`}</td>
                </>)}
              </tr>
            ))}
          </tbody>
      ...
      <div style={{ marginTop: 8, padding: "6px 10px", background: "#f8f9fa", border: "1px solid #dee2e6", borderRadius: 4, fontSize: 10, fontFamily: STATION_MONO, color: "#495057" }}>
        <b>Span {span} ft</b> Â· {stations.length} analysis stations Â·
        Máµ¤,max = <b>{fmt(Math.max(...stations.map(s=>s.Mu)), 2)} kip-ft</b> Â·
        |Váµ¤|,max = <b>{fmt(Math.max(...stations.map(s=>Math.abs(s.Vu))), 2)} kips</b> Â·
        Î´_net,max = <b>{fmt(Math.max(...stations.map(s=>Math.abs(s.netF))), 3)} in</b>
      </div>
    </div>
  );
}
...
const Sym=({base,sub,prime})=>(
  <span style={{fontStyle:"italic"}}>{base}{prime&&"â€²"}{sub&&<sub style={{fontStyle:"italic",fontSize:"0.78em"}}>{sub}</sub>}</span>
);
...
// Formula with code ref on the right â€” hidden when "step-by-step" view is off
const Eq=({tex,code})=>{
  ...
  return(
    <div style={{
      ...
      <div style={{fontSize:13,fontWeight:800,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",color:ok?"#1e7e34":"#a52a1f",whiteSpace:"nowrap"}}>
        {ok?"âœ“ DESIGN OK":"âœ— CHECK FAILS"}
      </div>
      <div style={{flex:1}}>
...
// â”€â”€ Current logged-in user, available to every tab for save attribution â”€â”€
const CurrentUserContext = createContext(null);
const useCurrentUser = () => useContext(CurrentUserContext);

// â”€â”€ Modal for saving a calc into Project > Part > Calc Name â”€â”€
function SaveCalcModal({open, onClose, onSave, existingProjects, defaultProject, defaultPart}){
  ...
  const submit = async()=>{
    ...
    setSaving(false);
    if(ok) onClose();
    else setErr("Save failed â€” check your connection and try again.");
  };
  ...
  return(
    <div className="no-print" onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:8,padding:22,width:"100%",maxWidth:400,boxShadow:"0 12px 36px rgba(0,0,0,0.25)"}}>
        <div style={{fontSize:14,fontWeight:800,marginBottom:14,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",textTransform:"uppercase"}}>ðŸ’¾ Save Calculation</div>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input value={calcName} onChange={e=>setCalcName(e.target.value)} placeholder="e.g. HC10 Slab â€” Rev 2"
            style={{width:"100%",padding:"8px 10px",borderRadius:4,border:"2px solid #e8a838",background:"#fff8ef",fontSize:13,boxSizing:"border-box"}}/>
        </div>
...
function PCITab({loadedCalc, onConsumedLoad, workspace}){
  ...
  // â”€â”€ Phase 7: Lateral stability state â”€â”€
  ...
  // â”€â”€ Phase 9: Moving loads state â”€â”€
  ...
  // â”€â”€ Phase 10b: Torsion state â”€â”€
  ...
  const r=useMemo(()=>{
    ...
    // â”€â”€ Phase 3: station-by-station analysis (21 stations along span) â”€â”€
    ...
    // â”€â”€ Phase 4: composite section, interface shear, crack width â”€â”€
    ...
    // â”€â”€ Phase 5: construction stages lifecycle â”€â”€
  ...
  // â”€â”€ Phase 10a: Transformed section properties â”€â”€
  ...
  // â”€â”€ Phase 10b: Torsion design â”€â”€
  ...
  // â”€â”€ Phase 6+8: vibration + filled-core shear (separate memo, lighter) â”€â”€
  ...
  // â”€â”€ Phase 7: Lateral stability (separate memo) â”€â”€
  ...
  // â”€â”€ Phase 9: Moving loads (separate memo) â”€â”€
  ...
  if(reportStyle==="tedds"){
    return(
      <TeddsDoc>
        <TeddsTitle calcId={`PCI Non-Composite Hollowcore Slab Design â€” ${sec}`} codeBasis="In accordance with ACI 318-19 and the PCI Design Handbook, 8th Edition" version="Calc version 1.0"/>
        ...
        <TeddsLine label="Cross sectional area" sym="A" val={fmt(r.s.A,2)} unit="inÂ²" label2="Web width" sym2="bw" val2={r.s.bw} unit2="in"/>
        <TeddsLine label="Second moment of area" sym="Ix" val={fmt(r.s.Ix,1)} unit="inâ´" label2="Centroid from bottom" sym2="yb" val2={fmt(r.s.yb,3)} unit2="in"/>
        <TeddsLine label="Section modulus (bottom)" sym="Sb" val={fmt(r.Sb,2)} unit="inÂ³" label2="Section modulus (top)" sym2="St" val2={fmt(r.St,2)} unit2="inÂ³"/>
        ...
        <TeddsLine label="Concrete strength at 28 days" sym="fâ€²c" val={fc} unit="ksi" label2="Concrete strength at release" sym2="fâ€²ci" val2={fci} unit2="ksi"/>
        <TeddsCalc sym="Ec" formula={`57âˆš(${fc}Ã—1000)`} result={fmt(r.Ec,0)} unit="ksi"/>
        <TeddsCalc sym="Eci" formula={`57âˆš(${fci}Ã—1000)`} result={fmt(r.Eci,0)} unit="ksi"/>
        <TeddsCalc sym="Î²1" formula={`0.85 âˆ’ 0.05(${fc}âˆ’4)`} result={fmt(r.b1,3)}/>
        ...
        <TeddsCalc sym="Aps" formula={`${nH}Ã—0.153 + ${nS}Ã—0.217`} result={fmt(r.Aps,4)} unit="inÂ²"/>
        ...
        <TeddsCalc sym="Po" formula={`${fmt(r.fpi,1)} Ã— ${fmt(r.Aps,3)} Ã— 0.95`} result={fmt(r.Po,2)} unit="kip"/>
        ...
        <TeddsCalc sym="Total loss" formula="(ES+CR+SH+RE)/fpi Ã— 100" result={fmt(r.totalLoss,2)} unit="%"/>
        ...
        <TeddsCalc sym="Î¦Mn" formula={`0.9 Ã— ${fmt(r.Aps,3)} Ã— ${fmt(r.fps1,1)} Ã— (${fmt(r.dp,2)} âˆ’ ${fmt(r.a1,3)}/2) / 12`} result={fmt(r.phiMn1,2)} unit="kip-ft"/>
        <TeddsCalc sym="Mu" formula="wu Ã— (b/12) Ã— lÂ² / 8" result={fmt(r.Mu,2)} unit="kip-ft"/>
        ...
        <TeddsLine label="Final camber" sym="Î”final" val={fmt(r.cF,4)} unit="in" label2="Net camber (less SDL)" sym2="Î”net" val2={fmt(r.netC,4)} unit2="in"/>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:"Calibri,Arial,sans-serif",marginBottom:12}}>
          <thead>
            <tr style={{background:"#f0f0f0"}}>
              {["x (ft)","Máµ¤ (k-ft)","Váµ¤ (kips)","f_top (ksi)","f_bot (ksi)","Î¦Mâ‚™ (k-ft)","Î¦Vâ‚™ (kips)","Î´ Final (in)","L/Î´","Status"].map((h,i)=>(
          ...
          <tbody>
            {r.stations.map((st,i)=>(
              <tr key={i} style={{background:st.allOk?"#fff":"#fff0f0"}}>
                ...
                <td style={{padding:"2px 5px",borderBottom:"1px solid #e0e0e0",textAlign:"right"}}>{st.Lratio>9000?"â€”":`L/${st.Lratio}`}</td>
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:"Calibri,Arial,sans-serif",marginBottom:12}}>
          <thead>
            <tr style={{background:"#f0f0f0"}}>
              {["Stg","Name","Day","Pe (k)","f_top e","f_bot e","f_top m","f_bot m","Î´ cam","Î´ net","âœ“"].map((h,i)=>(
  ...
  return(<div>
    <div className="no-print" style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,marginBottom:12,padding:"8px 12px",background:"#f8f9fa",border:"1px solid #dee2e6",borderRadius:6}}>
      <div style={{fontSize:12,color:"#495057"}}>
        {activeCalc ? <>ðŸ“„ <b>{activeCalc.calc_name}</b> <span style={{color:"#868e96"}}>({activeCalc.project_name} / {activeCalc.part_name})</span></> : <span style={{color:"#868e96"}}>Not yet saved to a project</span>}
      ...
      <div style={{display:"flex",gap:6}}>
        {activeCalc && activeCalc.created_by_id===user?.id && (
          <button onClick={doUpdate} style={{padding:"6px 12px",borderRadius:4,border:`1px solid ${theme.accent}`,background:theme.soft,color:theme.text,fontWeight:700,fontSize:11,cursor:"pointer"}}>â†» Update Saved File</button>
        )}
        <button onClick={()=>setSaveOpen(true)} style={{padding:"6px 12px",borderRadius:4,border:`2px solid ${theme.accentDark}`,background:theme.accentDark,color:"#fff",fontWeight:700,fontSize:11,cursor:"pointer"}}>ðŸ’¾ Save Asâ€¦</button>
      </div>
    </div>
    ...
    <LiveStatusBanner ok={r.allOk} util={r.flexUtil} label="Flexural Utilization (Mu / Î¦Mn)"/>

    {/* â”€â”€ Two-column sticky layout â”€â”€ */}
    ...
    <Card title="Slab Properties">
      ...
      {(slabHOverride !== null || slabBOverride !== null) && (
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"4px 8px",background:"#fff8ef",border:"1px solid #e8a838",borderRadius:4,marginTop:4,fontSize:11}}>
          <span style={{color:"#b07020"}}>âš  Custom dimensions active: h={slabHOverride??r.s.h}" b={slabBOverride??r.s.b}"</span>
      ...
      {/* â”€â”€ Dimension inputs â€” always visible â”€â”€ */}
      ...
      <Graphic><HollowcoreXSection h={r.s.h} b={r.s.b} cores={r.s.cores} coreD={r.s.coreD} nStrands={nH+nS} dp={r.dp} yb={r.s.yb} e={r.e} scale={7}/></Graphic>
      <Eq tex="Sb = Ix / yb     St = Ix / (h âˆ’ yb)" code="PCI 8th Â§4.2"/>
      <R><CI label="h" value={r.s.h} unit="in"/><CI label="A" value={r.s.A} unit="inÂ²"/><CI label={<Sym base="I" sub="x"/>} value={r.s.Ix} unit="inâ´"/><CI label="yb" value={fmt(r.s.yb,4)} unit="in"/><CI label="b" value={r.s.b} unit="in"/></R>
      <R><CI label={<Sym base="S" sub="b"/>} value={fmt(r.Sb,2)} unit="inÂ³"/><CI label={<Sym base="S" sub="t"/>} value={fmt(r.St,2)} unit="inÂ³"/><CI label="SW" value={r.s.SW} unit="psf"/><CI label="bw" value={r.s.bw} unit="in"/></R>
    </Card>
    <Card title="Concrete Properties">
      <Eq tex="Ec = 57âˆš(fâ€²c Ã— 1000)" code="ACI 318-19 Â§19.2.2.1"/>
      <Eq tex="Î²â‚ = 0.85 âˆ’ 0.05(fâ€²c âˆ’ 4) â‰¥ 0.65" code="ACI 318-19 Â§22.2.2.4.3"/>
      <Eq tex="V/S = A / [2(b + h)]" code="PCI 8th Â§4.7.3"/>
      ...
      <R><CI label={<Sym base="E" sub="c"/>} value={fmt(r.Ec,1)} unit="ksi"/><CI label={<Sym base="E" sub="ci"/>} value={fmt(r.Eci,1)} unit="ksi"/><CI label="Î²â‚" value={fmt(r.b1,4)}/><CI label="V/S" value={fmt(r.VS,3)}/></R>
    </Card>
    <Card title="Prestressing Steel">
      <Eq tex="Aps = nâ‚€.â‚… Ã— 0.153 + nâ‚€.â‚† Ã— 0.217" code="PCI 8th Table 11.2.3"/>
      <Eq tex="fpi = (fpi/fpu) Ã— fpu" code="ACI 318-19 Â§25.5.10.4"/>
      <Eq tex="dp = h âˆ’ cover     e = yb âˆ’ cover" code="PCI 8th Â§4.2.1"/>
      <R><OI label="# Â½â€³ Strands" value={nH} onChange={setNH} step={1}/><OI label="# â…â€³ Strands" value={nS} onChange={setNS} step={1}/><OI label="fpi/fpu" value={fpiR} onChange={setFpiR} step={0.01}/></R>
      <R><CI label={<Sym base="A" sub="ps"/>} value={fmt(r.Aps,4)} unit="inÂ²"/><CI label="fpi" value={fmt(r.fpi,1)} unit="ksi"/><CI label="dp" value={fmt(r.dp,2)} unit="in"/><CI label="e" value={fmt(r.e,3)} unit="in"/></R>
    </Card>
    <Card title="Reinforcing Steel">
      <Eq tex="As = n Ã— A_bar" code="ACI 318-19 Â§20.2.1"/>
      <R><OI label="# Rebar" value={nRe} onChange={setNRe} step={1}/><OI label="Bar Size" value={reSz} onChange={setReSz} options={Object.keys(REBAR)}/><CI label={<Sym base="A" sub="s"/>} value={fmt(r.As,3)} unit="inÂ²"/></R>
    </Card>
    ...
    <Card title="Composite Topping (Phase 4)">
      ...
      {useComposite && (<>
        <Eq tex="n = Ec_top / Ec_pre    yb_comp = Î£AÂ·y / Î£A    I_comp = Î£I + Î£AÂ·dÂ²" code="ACI 318-19 Â§26.5.6"/>
        <R>
          <OI label="Topping tc" value={tc} onChange={setTc} unit="in" step={0.5}/>
          <OI label="fâ€²c_top" value={fcTop} onChange={setFcTop} unit="ksi" step={0.5}/>
        ...
        {r.phase4?.comp && (<>
          <R>
            ...
            <CI label="A_comp" value={fmt(r.phase4.comp.A_comp,1)} unit="inÂ²"/>
            <CI label="I_comp" value={fmt(r.phase4.comp.I_comp,0)} unit="inâ´"/>
          </R>
          <R>
    ...
    <Card title="Prestress Force & Stresses at Release">
      <Eq tex="Po = fpi Ã— Aps Ã— (1 âˆ’ assumed_loss)" code="ACI 318-19 Â§24.5.3.1"/>
      <Eq tex="f_top = Po/A âˆ’ PoÂ·e/St     f_bot = Po/A + PoÂ·e/Sb" code="PCI 8th Â§4.2.2"/>
      <Res label="Pâ‚€" value={fmt(r.Po,2)} unit="kip"/>
    ...
    <Card title="Self-Weight at Transfer Point">
      <Eq tex="lt = 50 Ã— d_strand" code="ACI 318-19 Â§25.4.8.1"/>
      <Eq tex="Md = (lpc/2 Ã— lt_ft âˆ’ lt_ftÂ²/2) Ã— w_sw" code="PCI 8th Â§4.5"/>
    ...
    <Card title="Net Stresses & Permissible Checks">
      ...
      <Eq tex="Tension allow (end)    = âˆ’6âˆš(fâ€²ci Ã— 1000)" code="ACI 318-19 Â§24.5.3.2(a)"/>
      <Eq tex="Tension allow (mid)    = âˆ’3âˆš(fâ€²ci Ã— 1000)" code="ACI 318-19 Â§24.5.3.2(a)"/>
      <Eq tex="Compression allow (end)   = 0.70 fâ€²ci" code="ACI 318-19 Â§24.5.3.1"/>
      <Eq tex="Compression allow (mid)   = 0.60 fâ€²ci" code="ACI 318-19 Â§24.5.3.2(b)"/>
    ...
    <Card title="Elastic Shortening (ES)">
      <Eq tex="fcir = Kcir(Pi/A + PiÂ·eÂ²/Ix) âˆ’ MgÂ·e/Ix" code="PCI 8th Â§4.7.2.1"/>
      <Eq tex="ES = Kes Ã— (Eps/Eci) Ã— fcir" code="PCI 8th Eq. 4-19"/>
      <Res label="fcir" value={fmt(r.fcir,4)} unit="ksi"/><Res label="ES" value={fmt(r.ES,3)} unit="ksi"/>
    </Card>
    <Card title="Creep (CR)">
      <Eq tex="fcds = Msd Ã— e / Ix" code="PCI 8th Â§4.7.2.2"/>
      <Eq tex="CR = Kcr Ã— (Eps/Ec) Ã— (fcir âˆ’ fcds)" code="PCI 8th Eq. 4-22"/>
      <Res label="fcds" value={fmt(r.fcds,4)} unit="ksi"/><Res label="CR" value={fmt(r.CR,3)} unit="ksi"/>
    </Card>
    <Card title="Shrinkage & Relaxation">
      <Eq tex="SH = 8.2Ã—10â»â¶ Ã— Ksh Ã— Eps Ã— (1âˆ’0.06Â·V/S) Ã— (100âˆ’RH)" code="PCI 8th Eq. 4-24"/>
      <Eq tex="RE = [Kre/1000 âˆ’ J(ES+CR+SH)] Ã— C" code="PCI 8th Eq. 4-27"/>
      <Res label="SH" value={fmt(r.SHv,3)} unit="ksi"/><Res label="RE" value={fmt(r.RE,3)} unit="ksi"/>
    </Card>
    <Card title="Total Losses">
      <Eq tex="Total (%) = (ES + CR + SH + RE) / fpi Ã— 100" code="PCI 8th Â§4.7"/>
    ...
    <Card title="Service Stresses">
      <Eq tex="Pe = Aps Ã— fpi Ã— (1 âˆ’ loss/100)" code="ACI 318-19 Â§24.5.2"/>
      <Eq tex="f_bot = Pe/A + PeÂ·e/Sb âˆ’ Mserv/Sb" code="PCI 8th Â§4.3.1"/>
      <Eq tex="f_top = Pe/A âˆ’ PeÂ·e/St + M/St" code="PCI 8th Â§4.3.1"/>
      <Eq tex="Tension allow = 7.5âˆš(fâ€²cÃ—1000) / 1000" code="ACI 318-19 Â§24.5.2.1"/>
      <Res label="Pâ‚‘" value={fmt(r.Pe,2)} unit="kip"/><Res label={<Sym base="M" sub="sus"/>} value={fmt(r.Msus,2)} unit="kip-in"/><Res label={<Sym base="M" sub="service"/>} value={fmt(r.Mserv,2)} unit="kip-in"/>
      <Check label="Tension (Class U)" actual={fmt(r.fbot,4)} limit={fmt(r.aTens,4)} unit="ksi" ok={r.classU} tag={r.classU?"CLASS U":"FAIL"}/>
      <Check label="Comp (sustained, 0.45fâ€²c)" actual={fmt(r.ftSus,4)} limit={fmt(0.45*fc,3)} unit="ksi" ok={0.45*fc>r.ftSus}/>
      <Check label="Comp (total, 0.6fâ€²c)" actual={fmt(r.ftTot,4)} limit={fmt(0.6*fc,3)} unit="ksi" ok={0.6*fc>r.ftTot}/>
    </Card>

    <SH>Design Flexural Strength</SH>
    <Card title="Method #1 â€” ACI Eq. 18-1">
      <Eq tex="Ïp = Aps / (b Ã— dp)" code="ACI 318-19 Â§20.3.2.3"/>
      <Eq tex="fps = fpu[1 âˆ’ (Î³p/Î²â‚)(ÏpÂ·fpu/fâ€²c)]" code="ACI 318-19 Eq. 20.3.2.3.1"/>
      <Eq tex="a = ApsÂ·fps / (0.85Â·fâ€²cÂ·b)" code="ACI 318-19 Â§22.2.2.4.1"/>
      <Eq tex="c = a / Î²â‚     Îµt = [(dpâˆ’c)/c]Ã—0.003" code="ACI 318-19 Â§21.2.2"/>
      <Eq tex="Î¦Mn = Î¦Â·ApsÂ·fpsÂ·(dp âˆ’ a/2) / 12" code="ACI 318-19 Â§22.3.2"/>
      <Eq tex="Mu = wuÂ·(b/12)Â·lÂ²/8     wu = (1.2DL+1.6LL)/1000" code="ACI 318-19 Â§5.3.1"/>
      <Res label="Ïp" value={fmt(r.rhoP,6)}/><Res label="fps" value={fmt(r.fps1,2)} unit="ksi"/>
      <Res label="a" value={fmt(r.a1,4)} unit="in"/><Res label="c" value={fmt(r.c1,4)} unit="in"/>
      <Res label="Îµt" value={fmt(r.eT1,6)}/>
      <Res label="" value={<Badge ok={r.eT1>0.005} label={r.eT1>0.005?"Tension Controlled (Î¦=0.9)":"Comp/Trans"}/>}/>
      <Check label="Î¦Mn > Mu" actual={fmt(r.phiMn1,2)} limit={fmt(r.Mu,2)} unit="kip-ft" ok={r.phiMn1>r.Mu}/>
      <Eq tex="Mcr = (Ix/yb)Â·[f_bot,rel + 7.5âˆš(fâ€²cÃ—1000)/1000] / 12" code="ACI 318-19 Â§24.2.3.5"/>
      <Check label="Î¦Mn > 1.2 Mcr" actual={fmt(r.phiMn1,2)} limit={fmt(r.Mcr*1.2,2)} unit="kip-ft" ok={r.phiMn1>r.Mcr*1.2}/>
    </Card>
    <Card title="Method #3 â€” Strain Compatibility">
      <Eq tex="fse = fpi(1âˆ’loss/100)     Îµse = fse/Eps" code="PCI 8th Â§4.2.3"/>
      <Eq tex="Îµps = Îµse + [(dpâˆ’c)/c]Ã—0.003" code="PCI 8th Â§4.2.3.3"/>
      <Eq tex="fps = 270 âˆ’ 0.04/(Îµpsâˆ’0.007)  [if Îµps>0.0085]" code="PCI Strand Eq."/>
      <Eq tex="T = ApsÂ·fps + AsÂ·fy     C = 0.85Â·fâ€²cÂ·aÂ·b" code="ACI 318-19 Â§22.2"/>
      <Eq tex="Iterate c until T â‰ˆ C" code="PCI 8th Â§4.2.3.3"/>
      <R><OI label='Trial c' value={cT} onChange={setCT} unit="in" width={140}/></R>
      <Res label="Îµps" value={fmt(r.ePS,6)}/><Res label="fps" value={fmt(r.fps3,2)} unit="ksi"/>
      <div style={{display:"flex",gap:16,justifyContent:"center",padding:"6px 0",background:"#f8f9fa",borderRadius:4,margin:"6px 0"}}>
        ...
        <Badge ok={Math.abs(r.T3-r.C3)<5} label={`Î”=${fmt(Math.abs(r.T3-r.C3),2)}`}/>
      </div>
      <Res label="Î¦Mn" value={fmt(r.phiMn3,2)} unit="kip-ft"/>
      <Check label="Î¦Mn > Mu" actual={fmt(r.phiMn3,2)} limit={fmt(r.Mu,2)} unit="kip-ft" ok={r.phiMn3>r.Mu}/>
    </Card>

    <SH>Shear Strength</SH>
    <Card title="Factored Shear">
      <Eq tex="wu = 1.2Â·DL_line + 1.6Â·LL_line     Ay = wuÂ·l/2" code="ACI 318-19 Â§5.3.1"/>
      <Eq tex="Vc = 0.75[0.6Î»âˆšfâ€²c + 700(VuÂ·dp/Mu)]Â·bwÂ·dp" code="ACI 318-19 Eq. 22.5.8.3.1"/>
      <Eq tex="Vcw = 0.75[3.5Î»âˆšfâ€²c + 0.3fpc]Â·bwÂ·dp" code="ACI 318-19 Â§22.5.8.3.2"/>
      ...
      <Res label="Î¦Vcw (x=0.75ft)" value={fmt(r.Vcw,3)} unit="kips"/>
    </Card>

    {/* â”€â”€ Phase 4: Composite, Interface Shear, Crack Width â”€â”€ */}
    {useComposite && r.phase4?.composite && (<>
      <SH>Composite Section Stresses</SH>
      <Card title="Service Stresses â€” Composite Section">
        <Eq tex="f_bot = Pe/A + PeÂ·e/Sb_pre âˆ’ M_DL/Sb_pre âˆ’ (M_SDL+M_LL)/Sb_comp" code="PCI 8th Â§4.4"/>
        <Eq tex="f_top = Pe/A âˆ’ PeÂ·e/St_pre + M_DL/St_pre + (M_SDL+M_LL)/St_comp" code="PCI 8th Â§4.4"/>
        {r.phase4.stresses && (<>
          <R>
      ...
      <Card title="Horizontal Shear at Precast / Topping Interface">
        <Eq tex="Vh = Vu / d_v" code="ACI 318-19 Â§26.5.6.1"/>
        <Eq tex="Vnh = Î¦(cÂ·b_v + Î¼Â·AvfÂ·fy)" code="ACI 318-19 Eq. 22.9.4.2"/>
        {r.phase4.interfaceShear && (<>
          <R>
            ...
            <CI label="Î¼" value={fmt(r.phase4.interfaceShear.mu,2)}/>
    ...
    {r.phase4?.crackWidth && nRe > 0 && (<>
      ...
      <Card title="Maximum Flexural Crack Width (ACI 318-19 Â§24.3)">
        <Eq tex="w = 0.076Â·Î²Â·fsÂ·(dcÂ·A)^(1/3)" code="ACI 224 / ACI 318-19 Â§24.3.2"/>
        <R>
    ...
    <Card title="Camber & Deflection">
      <Eq tex="Î”_ps = PoÂ·eÂ·LÂ² / (8Â·EciÂ·Ix)" code="PCI 8th Table 4.8.4"/>
      <Eq tex="Î”_sw = 5Â·wÂ·Lâ´ / (384Â·EciÂ·Ix)" code="PCI 8th Table 4.8.4"/>
      <Eq tex="Î”_erec = 1.80Â·Î”_ps âˆ’ 1.85Â·Î”_sw" code="PCI 8th Table 4.8.4"/>
      <Eq tex="Î”_final = 2.45Â·Î”_ps âˆ’ 2.70Â·Î”_sw" code="PCI 8th Table 4.8.4"/>
...
function ColTab({loadedCalc, onConsumedLoad, workspace}){
  ...
  const r=useMemo(()=>{
    ...
    // Ï† factor: tied compression member, transitions per ACI but simplified to 0.65 here
    const phi=0.65;
  ...
  if(reportStyle==="tedds"){
    return(
      <TeddsDoc>
        ...
        <TeddsLine label="Concrete strength" sym="fâ€²c" val={fc} unit="ksi" label2="Clear cover" sym2="cc" val2={cov} unit2="in"/>
        <TeddsLine label="Reinforcement yield strength" sym="fy" val={fy} unit="ksi" label2="Steel modulus" sym2="Es" val2={Es} unit2="ksi"/>
        <TeddsLine label="Bottom bars" sym="As,bot" val={`${bQ} Ã— ${bBar}`} label2="Top bars" sym2="As,top" val2={`${tQ} Ã— ${tBar}`}/>
        ...
        <TeddsCalc sym="d" formula="h âˆ’ cover âˆ’ dtie âˆ’ dbar/2" result={fmt(r.d,3)} unit="in"/>
        <TeddsCalc sym="Î²1" formula={`0.85 âˆ’ 0.05(${fc}âˆ’4)`} result={fmt(r.b1,3)}/>
        <TeddsCalc sym="Pn,A" formula="(bÂ·h âˆ’ Î£As)Â·0.85fâ€²c + Î£AsÂ·fy" result={fmt(r.PnA,1)} unit="kips"/>
        <TeddsCalc sym="cb" formula="d Ã— 0.003 / (0.003 + Îµy)" result={fmt(r.cb,3)} unit="in"/>
        <TeddsCalc sym="Pn,B" formula="0.85fâ€²cÂ·abÂ·b + As'Â·f's âˆ’ AsÂ·fy" result={fmt(r.PnB,1)} unit="kips"/>
        <TeddsCalc sym="Mn,B" formula="Î£FÂ·(arm about centroid)" result={fmt(r.MnB,1)} unit="kip-ft"/>
        <TeddsCalc sym="Î¦Mn @ Pu" formula="interpolated along Pâ€“M envelope" result={fmt(r.phiMnAtPu,1)} unit="kip-ft"/>
  ...
  return(<div>
    ...
    <SaveCalcModal open={saveOpen} onClose={()=>setSaveOpen(false)} onSave={doSave} existingProjects={allCalcs}
      defaultProject={activeCalc?.project_name || workspace?.project} defaultPart={activeCalc?.part_name || workspace?.part}/>
    <LiveStatusBanner ok={r.demandOk} util={r.util} label="Demand vs. Capacity (Mu / Î¦Mn at Pu)"/>
    <InputsBlock><>
    <Card title="Factored Design Loads (Demand)">
      <Eq tex="Check: Pu â‰¤ Î¦Pn  and  Mu â‰¤ Î¦Mn @ Pu" code="ACI 318-19 Â§22.4"/>
      <R><OI label="Páµ¤" value={Pu} onChange={setPu} unit="kips"/><OI label="Máµ¤" value={Mu} onChange={setMu} unit="kip-ft"/></R>
    </Card>
    ...
    <Card title="Effective Depths">
      <Eq tex="d  = h âˆ’ cover âˆ’ d_tie âˆ’ d_bar/2" code="ACI 318-19 Â§22.2.2.1"/>
      <Eq tex="d' = cover + d_tie + d_bar/2" code="ACI 318-19 Â§22.2.2.1"/>
      <Eq tex="Î²â‚ = 0.85 âˆ’ 0.05(fâ€²c âˆ’ 4) â‰¥ 0.65" code="ACI 318-19 Â§22.2.2.4.3"/>
      <Res label="d" value={fmt(r.d,3)} unit="in"/><Res label="d'" value={fmt(r.dp,3)} unit="in"/><Res label="Î²â‚" value={fmt(r.b1,4)}/>
    </Card>
    <Card title="Point A â€” Pure Compression">
      <Eq tex="Pn = (bÂ·h âˆ’ Î£As)Â·0.85fâ€²c + Î£AsÂ·fy" code="ACI 318-19 Â§22.4.2.2"/>
      <Res label="Pâ‚™" value={fmt(r.PnA,1)} unit="kips"/>
    </Card>
    <Card title="Point B â€” Balanced Condition">
      <Eq tex="cb = d Ã— 0.003 / (0.003 + Îµy)" code="ACI 318-19 Â§22.2.2.1"/>
      <Eq tex="Îµ's = 0.003(cb âˆ’ d')/cb     f's = min(fy, EsÂ·|Îµ's|)" code="ACI 318-19 Â§22.2.1.1"/>
      <Eq tex="Pn = 0.85fâ€²cÂ·abÂ·b + As'Â·f's âˆ’ AsÂ·fy" code="ACI 318-19 Â§22.4.2"/>
      <Eq tex="Mn = Î£FÂ·arm about centroid" code="ACI 318-19 Â§22.4.2"/>
      <Res label="cb" value={fmt(r.cb,3)} unit="in"/><Res label="ab" value={fmt(r.ab,3)} unit="in"/>
      <Res label="Îµ's" value={fmt(r.esT,6)}/><Res label="f's" value={fmt(r.fsT,2)} unit="ksi"/>
      <Res label="Pâ‚™" value={fmt(r.PnB,1)} unit="kips"/><Res label="Mâ‚™" value={fmt(r.MnB,1)} unit="kip-ft"/>
    </Card>
    <Card title="Scientific Charts & 3D Model">
      <ChartPicker value={chartType} onChange={setChartType} options={[
        {id:"pm",label:"Pâ€“M Interaction"},
        {id:"capacity",label:"Capacity Breakdown"},
        {id:"3d",label:"ðŸ§Š 3D Model"},
        {id:"stress3d",label:"ðŸŒ¡ Stress Contour"},
      ]}/>
      {chartType==="3d" && (
        <Graphic>
        <Viewer3D
      ...
      {chartType==="stress3d" && (
        ...
        <div style={{marginBottom:8,padding:"8px 12px",background:"#f8f9fa",borderRadius:6,fontSize:11,color:"#495057"}}>
          Color contour from <i>Ïƒ = P<sub>u</sub>/A Â± M<sub>u</sub>Â·c/I</i> across the gross section â€” a simplified linear-elastic estimate
        ...
        <Viewer3D
          ...
          caption="Drag to rotate Â· Scroll to zoom Â· Red = compression, Blue = tension, White = neutral axis"
        />
        </Graphic>
      )}
      {chartType==="pm" && (<>
      <Eq tex="Sweep c from h â†’ 0, compute Îµs â†’ fs for each layer, sum P & M" code="ACI 318-19 Â§22.4"/>
      <Graphic>
      <svg className="live-svg" viewBox={`0 0 ${W} ${H}`} style={{width:"100%",maxWidth:540,display:"block",margin:"0 auto"}}>
        ...
        <circle cx={sx(r.MnB)} cy={sy(r.PnB)} r={5} fill="#27ae60"/><text x={sx(r.MnB)+10} y={sy(r.PnB)+4} fill="#27ae60" fontSize={10} fontWeight={700} fontFamily="'JetBrains Mono','Fira Code','Consolas',monospace">B ({fmt(r.PnB,0)}k, {fmt(r.MnB,0)}kÂ·ft)</text>
        ...
        <text x={sx(Mu)+12} y={sy(Pu)-8} fill={r.demandOk?"#1d4ed8":"#a52a1f"} fontSize={11} fontWeight={800} fontFamily="'JetBrains Mono','Fira Code','Consolas',monospace">
          Demand ({fmt(Pu,0)}k, {fmt(Mu,0)}kÂ·ft)
        </text>
      ...
      {chartType==="capacity" && (
        ...
        <BarChart title="Capacity Breakdown" unit="kips" bars={[
          ...
          {label:"Î¦Pn @ Point A", value:r.phi*r.PnA, color:"#e8a838"},
          {label:"Pn @ Point B (balanced)", value:r.PnB, color:"#27ae60"},
          {label:"Î¦Mn @ Pu (kip-ft)", value:r.phiMnAtPu, color:"#2563eb"},
...
function CPCITab({loadedCalc, onConsumedLoad, workspace}){
  ...
  if(reportStyle==="tedds"){
    return(
      <TeddsDoc>
        <TeddsTitle calcId={`CPCI Non-Composite Hollowcore Slab Design â€” ${sec}`} codeBasis="In accordance with CSA A23.3-19 and the CPCI Design Manual, 5th Edition" version="Calc version 1.0"/>
        ...
        <TeddsLine label="Cross sectional area" sym="A" val={r.s.A} unit="mmÂ²" label2="Web width" sym2="bw" val2={r.s.bw} unit2="mm"/>
        <TeddsLine label="Section modulus (bottom)" sym="Sb" val={fmt(r.Sb,0)} unit="mmÂ³" label2="Section modulus (top)" sym2="St" val2={fmt(r.St,0)} unit2="mmÂ³"/>

        <TeddsHeading>Material properties</TeddsHeading>
        <TeddsLine label="Concrete strength at 28 days" sym="fâ€²c" val={fc} unit="MPa" label2="Concrete strength at release" sym2="fâ€²ci" val2={fci} unit2="MPa"/>
        <TeddsCalc sym="Ec" formula={`4500âˆš${fc}`} result={r.Ec} unit="MPa"/>
        <TeddsCalc sym="Eci" formula={`4500âˆš${fci}`} result={r.Eci} unit="MPa"/>

        <TeddsHeading>Prestressing steel</TeddsHeading>
        <TeddsLine label="Number of 13mm strands" sym="n13" val={nH} label2="Number of 15mm strands" sym2="n15" val2={nS}/>
        <TeddsCalc sym="Aps" formula={`${nH}Ã—99 + ${nS}Ã—143`} result={r.Aps} unit="mmÂ²"/>
        ...
        <TeddsHeading>Transfer stress analysis</TeddsHeading>
        <TeddsCalc sym="Po" formula={`${fmt(r.fpi,0)} Ã— ${r.Aps} / 1000`} result={fmt(r.Po,2)} unit="kN"/>
        ...
        <TeddsHeading>Loss of prestress</TeddsHeading>
        <TeddsCalc sym="Total loss" formula="(ES+CR+SH)/fpi Ã— 100" result={fmt(r.totalLoss,2)} unit="%"/>

        <TeddsHeading>Design flexural strength</TeddsHeading>
        <TeddsCalc sym="fpr" formula="fpu(1 âˆ’ kpÂ·c/dp)" result={fmt(r.fpr,1)} unit="MPa"/>
        <TeddsCalc sym="Mr" formula="Î¦Â·ApsÂ·fprÂ·(dp âˆ’ a/2)" result={fmt(r.Mr,2)} unit="kNm"/>
        <TeddsCalc sym="Mf" formula="1.25Md + 1.5(SDL+LL+SL)Â·(b/1000)Â·LÂ²/8" result={fmt(r.Mf,2)} unit="kNm"/>

        <TeddsHeading>Shear strength</TeddsHeading>
        <TeddsCalc sym="Vc" formula="Î¦cÂ·Î²Â·âˆšfâ€²cÂ·bwÂ·dv" result={fmt(r.Vc,2)} unit="kN"/>
  ...
  return(<div>
    ...
    <Card title="Scientific Charts & 3D Model">
      ...
      {chartType==="util" && (
        <BarChart title="Utilization â€” Capacity vs Maximum" unit="" bars={[
    ...
    <Card title="Slab Properties (Metric)">
      ...
      <R><CI label="h" value={r.s.h} unit="mm"/><CI label="A" value={r.s.A} unit="mmÂ²"/><CI label="b" value={r.s.b} unit="mm"/><CI label="bw" value={r.s.bw} unit="mm"/></R>
    </Card>
    <Card title="Concrete">
      <Eq tex="Ec = 4500âˆšfâ€²c" code="CSA A23.3-19 Â§8.6.2.2"/>
      <Eq tex="Î±â‚ = 0.85 âˆ’ 0.0015fâ€²c â‰¥ 0.67" code="CSA A23.3-19 Â§10.1.7"/>
      <Eq tex="Î²â‚ = 0.97 âˆ’ 0.0025fâ€²c â‰¥ 0.67" code="CSA A23.3-19 Â§10.1.7"/>
      ...
      <R><CI label={<Sym base="E" sub="c"/>} value={r.Ec} unit="MPa"/><CI label={<Sym base="E" sub="ci"/>} value={r.Eci} unit="MPa"/><CI label="Î±â‚" value={fmt(r.a1,3)}/><CI label="Î²â‚" value={fmt(r.b1,3)}/></R>
    </Card>
    <Card title="Strands">
      <Eq tex="Aps = nâ‚â‚ƒ Ã— 99 + nâ‚â‚… Ã— 143 (mmÂ²)" code="CPCI Handbook Â§3.2"/>
      <R><OI label="# 13mm" value={nH} onChange={setNH} step={1}/><OI label="# 15mm" value={nS} onChange={setNS} step={1}/></R>
      <R><CI label={<Sym base="A" sub="ps"/>} value={r.Aps} unit="mmÂ²"/><CI label="fpi" value={fmt(r.fpi,0)} unit="MPa"/><CI label="e" value={fmt(r.e,1)} unit="mm"/><CI label="dp" value={fmt(r.dp,1)} unit="mm"/></R>
    </Card>
    <Card title="Loading">
    ...
    <Card title="Transfer Stresses">
      <Eq tex="f = Â±Po/A Â± PoÂ·e/S" code="CSA A23.3-19 Â§18.3.1"/>
      <Eq tex="Tension allow = âˆ’0.5âˆšfâ€²ci     Comp allow = 0.67fâ€²ci" code="CSA A23.3-19 Â§18.3.1"/>
      <Res label="Pâ‚€" value={fmt(r.Po,2)} unit="kN"/>
    ...
    <Card title="Losses"><Eq tex="Total = (ES+CR+SH)/fpi Ã— 100" code="CSA A23.3-19 Â§18.6"/><Res label="Total Loss" value={<span style={{fontSize:18,fontWeight:800}}>{fmt(r.totalLoss,2)}%</span>}/></Card>
    <Card title="Flexural Strength">
      <Eq tex="c = ApsÂ·fpu / (Î±â‚fâ€²cÂ·b + kpÂ·ApsÂ·fpu/dp)" code="CSA A23.3-19 Â§18.6.2"/>
      <Eq tex="fpr = fpu(1 âˆ’ kpÂ·c/dp)" code="CSA A23.3-19 Â§18.6.2"/>
      <Eq tex="Mr = Î¦Â·ApsÂ·fprÂ·(dp âˆ’ a/2)" code="CSA A23.3-19 Â§18.6.2"/>
      <Eq tex="Mf = 1.25Md + 1.5(SDL+LL+SL)Â·(b/1000)Â·LÂ²/8" code="CSA A23.3-19 Â§8.3.2"/>
    ...
    <Card title="Shear">
      <Eq tex="Vc = Î¦cÂ·Î²Â·âˆšfâ€²cÂ·bwÂ·dv" code="CSA A23.3-19 Â§11.3.4"/>
    ...
    <Card title="Torsion">
      <Eq tex="Tcr = 0.38âˆšfâ€²cÂ·(AÂ²/Pc)Â·âˆš(1 + fcp/0.38âˆšfâ€²c)" code="CSA A23.3-19 Â§11.2.9.1"/>
...
function CrushTab({loadedCalc, onConsumedLoad, workspace}){
  ...
  if(reportStyle==="tedds"){
    return(
      <TeddsDoc>
        <TeddsTitle calcId={`Hollowcore End Crushing Capacity â€” ${sec}`} codeBasis="In accordance with CSA A23.3-19 and the CPCI Design Manual, 5th Edition" version="Calc version 1.0"/>
        ...
        <TeddsLine label="Bearing width" sym="w" val={w} unit="mm" label2="Concrete strength" sym2="fâ€²c" val2={fc} unit2="MPa"/>
        ...
        <TeddsCalc sym="Ae,horiz" formula={`2 Ã— ${w} Ã— ${r.s.bw} / (${r.s.b}/1000)`} result={fmt(r.AeH,1)} unit="mmÂ²/m"/>
        <TeddsCalc sym="Ae,single" formula={`1 Ã— ${w} Ã— ${r.s.bw} / (${r.s.b}/1000)`} result={fmt(r.AeS,1)} unit="mmÂ²"/>

        <TeddsHeading>Core crushing capacity</TeddsHeading>
        <TeddsCalc sym="Re" formula={`1 âˆ’ 2(${ecc})/${hB}`} result={fmt(r.eF,4)}/>
        <TeddsCalc sym="Î¦Pn,horiz" formula={`0.65 Ã— 0.85 Ã— Ae Ã— ${fc} Ã— Re / 1000`} result={fmt(r.PnH,2)} unit="kN/m"/>
        <TeddsCalc sym="Î¦Pn,single" formula={`0.65 Ã— 0.85 Ã— Ae Ã— ${fc} Ã— Re / 1000`} result={fmt(r.PnS,2)} unit="kN"/>

        <TeddsHeading>Results summary</TeddsHeading>
        <TeddsTable
          headers={["Check","Unit","Capacity","Maximum","Utilization","Result"]}
          rows={[
            [ "Horizontal abutting joints","kN/m", fmt(r.PnH,2), "â€”", "â€”", "PASS" ],
            [ "Single end joint","kN", fmt(r.PnS,2), "â€”", "â€”", "PASS" ],
          ]}
        />
      </TeddsDoc>
    );
  ...
  return(<div>
    ...
    <Card title="Effective Bearing Area">
      <Eq tex="Ae(horiz) = 2Â·wÂ·bw / (b/1000)" code="CPCI Handbook Â§4.5.1"/>
      <Eq tex="Ae(single) = 1Â·wÂ·bw / (b/1000)" code="CPCI Handbook Â§4.5.1"/>
      <Res label="Ae (horiz)" value={fmt(r.AeH,1)} unit="mmÂ²/m"/><Res label="Ae (single)" value={fmt(r.AeS,1)} unit="mmÂ²"/>
    </Card>
    <Card title="Core Crushing Capacity">
      <Eq tex="Re = 1 âˆ’ 2e/h" code="CPCI Handbook Â§4.5.2"/>
      <Eq tex="Î¦Pn = Î¦ Ã— 0.85 Ã— Ae Ã— fâ€²c Ã— Re / 1000" code="CSA A23.3-19 Â§10.8.1"/>
      <Res label="Râ‚‘" value={fmt(r.eF,4)}/><Res label="Î¦" value={r.phi}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:10}}>
        <div style={{padding:14,background:"#f8f9fa",borderRadius:4,textAlign:"center",border:"1px solid #dee2e6"}}>
          ...
          <div style={{fontSize:11,color:"#999"}}>Î¦Pn (kN/m)</div>
        </div>
        <div style={{padding:14,background:"#f8f9fa",borderRadius:4,textAlign:"center",border:"1px solid #dee2e6"}}>
    ...
    <Card title="Scientific Charts">
      ...
      {chartType==="ecc" && (
        <Graphic>
        <LineChart title="Capacity vs. Bearing Eccentricity" xLabel="Eccentricity, e (mm)" yLabel="Î¦Pn,horiz (kN/m)"
          series={[{
      ...
      <div className="no-print" style={{width:340,flexShrink:0,position:"sticky",top:80,alignSelf:"flex-start",maxHeight:"calc(100vh - 100px)",overflowY:"auto"}}>
        <div style={{fontWeight:700,fontSize:11,fontFamily:"'JetBrains Mono',monospace",textTransform:"uppercase",letterSpacing:0.8,color:"#2563eb",marginBottom:8,padding:"6px 10px",background:"#eff6ff",borderRadius:6,border:"1px solid #bfdbfe"}}>ðŸ“ 3D Model &amp; Charts</div>
        <Card title="Scientific Charts & 3D Model">
          <ChartPicker value={chartType} onChange={setChartType} options={[
            {id:"stages",   label:"ðŸ— Construction Stages"},
            {id:"vibration",label:"ðŸ”Š Vibration"},
            {id:"lateral",  label:"â†” Lateral Stability"},
            {id:"moving",   label:"ðŸš› Moving Loads"},
            {id:"transformed",label:"âŠ• Transformed Section"},
            {id:"torsion",  label:"ðŸŒ€ Torsion"},
            {id:"stations", label:"ðŸ“‹ Station Results"},
            {id:"stress",label:"Stress vs. Depth"},
            {id:"beam",label:"Moment & Shear"},
            {id:"losses",label:"Prestress Losses"},
          ...
          {chartType==="beam" && r.stations && (
            <div>
              <LineChart title="Factored Bending Moment Diagram â€” Máµ¤ (kip-ft)"
                xLabel="Position along span (ft)" yLabel="Máµ¤ (kip-ft)" yZeroLine
                series={[{points:r.stations.map(st=>({x:st.x,y:st.Mu})),color:"#2563eb",label:"Máµ¤"},
                         {points:r.stations.map(st=>({x:st.x,y:st.phiMn})),color:"#22c55e",label:"Î¦Mn"}]}/>
              <LineChart title="Factored Shear Force Diagram â€” Váµ¤ (kips)"
                xLabel="Position along span (ft)" yLabel="Váµ¤ (kips)" yZeroLine
                series={[{points:r.stations.map(st=>({x:st.x,y:st.Vu})),color:"#c0392b",label:"Váµ¤"},
                         {points:r.stations.map(st=>({x:st.x,y:st.Vn})),color:"#22c55e",label:"Î¦Vn"}]}/>
            </div>
          )}
          {chartType==="losses" && (
...
const ALL_TABS=[{id:"dashboard",label:"ðŸ  Dashboard",mod:null},...TABS];
...
function LiveStatusBanner({ok, util, label}) {
  ...
  return (
    <div className="no-print" style={{marginBottom:12,padding:"8px 14px",borderRadius:6,background:ok?"#f0fdf4":"#fef2f2",border:`1px solid ${ok?"#bbf7d0":"#fecaca"}`,display:"flex",alignItems:"center",gap:12}}>
      <span style={{fontSize:11,fontWeight:700,color:ok?"#16a34a":"#dc2626"}}>{ok?"âœ“ PASS":"âœ— FAIL"}</span>
...
function Dashboard({ onOpenCalc, onNavigate }) {
  ...
  if (loading) return <div style={{padding:40,textAlign:"center",color:"#868e96",fontFamily:MONO}}>Loading dashboardâ€¦</div>;
  ...
  const timeAgo=(d)=>{if(!d)return"â€”";const diff=Date.now()-new Date(d);const m=Math.floor(diff/60000),h=Math.floor(diff/3600000),dy=Math.floor(diff/86400000);if(m<2)return"just now";if(m<60)return`${m}m ago`;if(h<24)return`${h}h ago`;if(dy<7)return`${dy}d ago`;return new Date(d).toLocaleDateString();};
  const MI={pci:{label:"PCI Hollowcore",color:"#2563eb",icon:"ðŸ”·"},cpci:{label:"CPCI Hollowcore",color:"#7c3aed",icon:"ðŸ”¶"},col:{label:"Column Design",color:"#16a34a",icon:"ðŸŸ©"},crush:{label:"HC End Crushing",color:"#ea580c",icon:"ðŸŸ "},beam:{label:"Beam Design",color:"#0ea5e9",icon:"ðŸ”¹"}};
  ...
  return (
    <div>
      <div style={{marginBottom:20,padding:"18px 22px",background:"linear-gradient(135deg,#1e3a5f 0%,#2563eb 100%)",borderRadius:10,color:"#fff"}}>
        <div style={{fontSize:18,fontWeight:800,fontFamily:MONO,marginBottom:4}}>{isAdmin(user)?"ðŸ‘‘":"ðŸ‘‹"} Welcome back, {user?.name?.split(" ")[0]}</div>
        <div style={{fontSize:11,opacity:0.8}}>{new Date().toLocaleDateString("en-US",{weekday:"long",year:"numeric",month:"long",day:"numeric"})} Â· BT Structural Calculator</div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10,marginBottom:20}}>
        {[{icon:"ðŸ“",value:totalProjects,label:"Projects",color:"#2563eb"},{icon:"ðŸ“„",value:totalFiles,label:"Files",color:"#16a34a"},{icon:"ðŸ“…",value:timeAgo(recentFiles[0]?.updated_at||recentFiles[0]?.created_at),label:"Last Activity",color:"#0891b2"}].map((s,i)=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
        <div style={{background:"#fff",border:"1px solid #dee2e6",borderRadius:8,overflow:"hidden"}}>
          <div style={{padding:"10px 14px",borderBottom:"1px solid #dee2e6",background:"#f8f9fa",fontWeight:700,fontSize:12,fontFamily:MONO}}>ðŸ• Recent Files</div>
          {recentFiles.length===0?<div style={{padding:20,textAlign:"center",color:"#868e96",fontSize:12}}>No files yet</div>:recentFiles.map((f,i)=>{
            const mi=MI[f.module]||{label:f.module,color:"#868e96",icon:"ðŸ“„"};
            return(<div key={i} onClick={()=>onOpenCalc(f)} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderBottom:"1px solid #f1f3f5",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#f8f9fa"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:10,color:"#868e96"}}><span style={{color:mi.color,fontWeight:600}}>{mi.label}</span> Â· {f.project_name}{isAdmin(user)?` Â· ${f.created_by_name}`:""}</div>
              </div>
        ...
        <div style={{background:"#fff",border:"1px solid #dee2e6",borderRadius:8,overflow:"hidden"}}>
          <div style={{padding:"10px 14px",borderBottom:"1px solid #dee2e6",background:"#f8f9fa",fontWeight:700,fontSize:12,fontFamily:MONO}}>ðŸ“Š Module Usage</div>
          ...
          <div style={{padding:"10px 14px",borderTop:"1px solid #dee2e6",background:"#f8f9fa"}}>
            <div style={{fontSize:11,fontWeight:700,fontFamily:MONO,marginBottom:6,color:"#374151"}}>âš¡ Quick Start</div>
...
function ProjectsBrowser({onOpenCalc}){
  ...
  const handleRoleToggle=async(targetId,currentRole)=>{const newRole=currentRole==="admin"?"user":"admin";const ok=await setUserRole(targetId,newRole,user?.id);if(ok){setRoleMsg(`âœ“ ${targetId} is now ${newRole}`);fetchAllUserRoles().then(setUserRoles);setTimeout(()=>setRoleMsg(""),3000);}};
  ...
  return(
    <div>
      {isAdmin(user)&&(
        <div style={{marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:showAdmin?10:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:11,fontWeight:700,background:"#1d4ed8",color:"#fff",padding:"2px 10px",borderRadius:10,fontFamily:MONO}}>ðŸ‘‘ ADMIN</span>
              <span style={{fontSize:11,color:"#6c757d"}}>Viewing all employees' projects</span>
            </div>
            <button onClick={()=>setShowAdmin(v=>!v)} style={{padding:"4px 12px",border:"1px solid #ced4da",borderRadius:4,background:"#fff",fontSize:11,cursor:"pointer",fontFamily:MONO}}>{showAdmin?"â–² Hide":"âš™ Admin Panel"}</button>
          </div>
          {showAdmin&&(
            <div style={{background:"#f8f9fa",border:"1px solid #dee2e6",borderRadius:8,padding:14}}>
              <div style={{fontSize:13,fontWeight:700,marginBottom:10}}>ðŸ” Access Management</div>
      ...
      {!isAdmin(user)&&<div style={{marginBottom:10,padding:"6px 12px",background:"#f8f9fa",border:"1px solid #dee2e6",borderRadius:4,fontSize:11,color:"#6c757d",fontFamily:MONO}}>ðŸ‘¤ Viewing your own projects.</div>}
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:14,fontSize:13,flexWrap:"wrap"}}>
        {isAdmin(user)&&<span style={stage!=="employees"?crumbS:{fontWeight:700}} onClick={()=>{if(stage!=="employees"){setStage("employees");setSelEmployee(null);setSelProject(null);setSelPart(null);}}}>ðŸ‘¥ All Employees</span>}
        {!isAdmin(user)&&<span style={stage==="projects"?{fontWeight:700}:crumbS} onClick={()=>{setStage("projects");setSelProject(null);setSelPart(null);}}>ðŸ“ All Projects</span>}
        ...
        {selProject&&<><span style={{color:"#adb5bd"}}>/</span><span style={stage==="parts"?{fontWeight:700}:crumbS} onClick={()=>{setStage("parts");setSelPart(null);}}>ðŸ“ {selProject}</span></>}
        {selPart&&<><span style={{color:"#adb5bd"}}>/</span><span style={{fontWeight:700}}>ðŸ—‚ï¸ {selPart}</span></>}
      </div>
      {loading&&<div style={{color:"#868e96",fontSize:13,padding:20,textAlign:"center"}}>Loadingâ€¦</div>}
      {!loading&&stage==="employees"&&isAdmin(user)&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
          {Object.entries(EMPLOYEES).map(([id,emp])=>{const cnt=calcs.filter(c=>c.created_by_id===id).length;const pCnt=[...new Set(calcs.filter(c=>c.created_by_id===id).map(c=>c.project_name))].length;return(<div key={id} onClick={()=>{setSelEmployee(id);setSelProject(null);setSelPart(null);setStage("projects");}} style={{...cardS,borderLeft:`4px solid ${emp.role==="admin"?"#1d4ed8":"#16a34a"}`}} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 3px 12px rgba(0,0,0,0.1)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}><div style={{fontSize:26,marginBottom:4}}>{emp.role==="admin"?"ðŸ‘‘":"ðŸ‘¤"}</div><div style={{fontWeight:800,fontSize:13}}>{emp.name}</div><div style={{fontSize:10,fontFamily:MONO,color:"#868e96"}}>{id}{user?.id===id?" (you)":""}</div><div style={{marginTop:6,fontSize:11,color:"#374151"}}><b>{pCnt}</b> projects Â· <b>{cnt}</b> files</div></div>);})}</div>
      )}
      {!loading&&stage==="projects"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:10}}>
          {projects.map(p=>{const cnt=filteredCalcs.filter(c=>c.project_name===p).length;const pCnt=[...new Set(filteredCalcs.filter(c=>c.project_name===p).map(c=>c.part_name))].length;return(<div key={p} onClick={()=>{setSelProject(p);setSelPart(null);setStage("parts");}} style={cardS} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}><div style={{fontSize:24,marginBottom:4}}>ðŸ“</div><div style={{fontWeight:700,fontSize:13}}>{p}</div><div style={{fontSize:11,color:"#868e96",marginTop:3}}>{pCnt} part{pCnt!==1?"s":""} Â· {cnt} file{cnt!==1?"s":""}</div></div>);})}
          {(!isAdmin(user)||selEmployee===user?.id)&&<div onClick={()=>setNewProjectOpen(true)} style={{...cardS,border:"2px dashed #ced4da",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#868e96"}}><div style={{fontSize:24,marginBottom:4}}>ï¼‹</div><div style={{fontWeight:700,fontSize:12}}>New Project</div></div>}
        </div>
      )}
      {!loading&&stage==="parts"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:10}}>
          {parts.map(pt=>{const cnt=filteredCalcs.filter(c=>c.project_name===selProject&&c.part_name===pt).length;return(<div key={pt} onClick={()=>{setSelPart(pt);setStage("files");}} style={cardS} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}><div style={{fontSize:22,marginBottom:4}}>ðŸ—‚ï¸</div><div style={{fontWeight:700,fontSize:13}}>{pt}</div><div style={{fontSize:11,color:"#868e96",marginTop:3}}>{cnt} file{cnt!==1?"s":""}</div></div>);})}
          {(!isAdmin(user)||selEmployee===user?.id)&&<div onClick={()=>setNewPartOpen(true)} style={{...cardS,border:"2px dashed #ced4da",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#868e96"}}><div style={{fontSize:22,marginBottom:4}}>ï¼‹</div><div style={{fontWeight:700,fontSize:12}}>New Part</div></div>}
        </div>
      )}
      {!loading&&stage==="files"&&(
        <div>
          {(!isAdmin(user)||selEmployee===user?.id)&&<button onClick={()=>onOpenCalc&&onOpenCalc({action:"new",project:selProject,part:selPart})} style={{width:"100%",padding:12,borderRadius:8,border:"none",background:"#212529",color:"#fff",fontWeight:800,fontSize:12,cursor:"pointer",marginBottom:12,fontFamily:MONO,textTransform:"uppercase",letterSpacing:0.5}}>ï¼‹ Start New Calculation</button>}
          {files.map(f=>{const isOwner=user&&(f.created_by_id===user.id||isAdmin(user));return(<div key={f.id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",border:"1px solid #dee2e6",borderRadius:6,marginBottom:6,background:"#fff"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:10,color:"#868e96"}}>{MODULE_LABELS[f.module]||f.module} Â· {f.created_by_name} Â· {new Date(f.created_at).toLocaleDateString()}</div>
            </div>
    ...
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",maxWidth:860,margin:"0 auto",padding:"16px 12px",color:"#212529",background:"#fff",minHeight:"100vh"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"2px solid #212529",paddingBottom:10,marginBottom:16}}>
        <div>
          ...
          <p style={{fontSize:11,color:"#868e96",margin:"2px 0 0"}}>{isAdmin(user)?"Admin view â€” all employees' projects":"Select a project and part to begin"}</p>
        </div>
        <div style={{textAlign:"right"}}>
          {isAdmin(user)&&<span style={{fontSize:10,fontWeight:800,background:"#1d4ed8",color:"#fff",padding:"2px 8px",borderRadius:10,fontFamily:MONO,display:"block",marginBottom:4}}>ðŸ‘‘ ADMIN</span>}
      ...
      {loading&&<div style={{textAlign:"center",padding:40,color:"#868e96"}}>Loadingâ€¦</div>}
      {!loading&&stage==="employees"&&isAdmin(user)&&(
      ...
      {!loading&&stage==="projects"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:10}}>
          {projects.map(p=>{const cnt=filteredCalcs.filter(c=>c.project_name===p).length;return(<div key={p} onClick={()=>{setSelProject(p);setStage("parts");}} style={cardS} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}><div style={{fontSize:24,marginBottom:4}}>ðŸ“</div><div style={{fontWeight:700,fontSize:13}}>{p}</div><div style={{fontSize:11,color:"#868e96",marginTop:3}}>{cnt} file{cnt!==1?"s":""}</div></div>);})}
          <div onClick={()=>setNewProjectOpen(true)} style={{...cardS,border:"2px dashed #ced4da",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#868e96"}}><div style={{fontSize:24,marginBottom:4}}>ï¼‹</div><div style={{fontWeight:700,fontSize:12}}>New Project</div></div>
        </div>
      )}
      {!loading&&stage==="parts"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:10}}>
          {parts.map(pt=>{const cnt=filteredCalcs.filter(c=>c.project_name===selProject&&c.part_name===pt).length;return(<div key={pt} onClick={()=>{setSelPart(pt);setStage("files");}} style={cardS} onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}><div style={{fontSize:22,marginBottom:4}}>ðŸ—‚ï¸</div><div style={{fontWeight:700,fontSize:13}}>{pt}</div><div style={{fontSize:11,color:"#868e96",marginTop:3}}>{cnt} file{cnt!==1?"s":""}</div></div>);})}
          <div onClick={()=>setNewPartOpen(true)} style={{...cardS,border:"2px dashed #ced4da",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#868e96"}}><div style={{fontSize:22,marginBottom:4}}>ï¼‹</div><div style={{fontWeight:700,fontSize:12}}>New Part</div></div>
        </div>
      )}
      {!loading&&stage==="files"&&(
        <div>
          <button onClick={()=>onEnter(selProject,selPart)} style={{width:"100%",padding:12,borderRadius:8,border:"none",background:"#212529",color:"#fff",fontWeight:800,fontSize:12,cursor:"pointer",marginBottom:12,fontFamily:MONO,textTransform:"uppercase",letterSpacing:0.5}}>ï¼‹ Start New Calculation</button>
          {files.map(f=>(<div key={f.id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",border:"1px solid #dee2e6",borderRadius:6,marginBottom:6,background:"#fff"}}>
            <span style={{fontSize:18}}>ðŸ“„</span>
            <div style={{flex:1,minWidth:0}}><div style={{fontWeight:700,fontSize:12}}>{f.calc_name}</div><div style={{fontSize:10,color:"#868e96"}}>{MODULE_LABELS[f.module]||f.module} Â· {new Date(f.created_at).toLocaleDateString()}</div></div>
            <button onClick={()=>onOpenCalc(f)} style={{padding:"5px 10px",borderRadius:4,border:"2px solid #2563eb",background:"#eff6ff",color:"#1d4ed8",fontWeight:700,fontSize:11,cursor:"pointer"}}>Open</button>
          </div>))}
        </div>
      )}
  ...
  return(
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",minHeight:"100vh",background:"#212529",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#fff",borderRadius:8,padding:"32px 28px",width:"100%",maxWidth:340,boxShadow:"0 10px 40px rgba(0,0,0,0.3)"}}>
        ...
        <div style={{textAlign:"center",marginTop:12,fontSize:10,color:"#adb5bd"}}>Building Theory Â· Miami, FL</div>
      </div>
    </div>
  );
}
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// MAIN APP â€” with sticky 3D viewer on the right
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        ...
        <div style={{flex:1,overflowY:"auto",padding:"12px 14px"}}>
          ...
          <div className="no-print" style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,paddingBottom:8,borderBottom:"1px solid #dee2e6"}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>
              ...
              <button onClick={()=>setWorkspace(null)} style={{fontSize:10,color:"#868e96",background:"none",border:"1px solid #dee2e6",cursor:"pointer",padding:"3px 8px",borderRadius:4}}>â† Projects</button>
              ...
              <button onClick={()=>handlePrint(false)} className="no-print" style={{fontSize:10,padding:"4px 10px",borderRadius:4,border:"1px solid #dee2e6",background:"#fff",cursor:"pointer",fontFamily:MONO}}>ðŸ–¨ Print</button>
            </div>
          </div>
          ...
          <div style={{textAlign:"center",marginTop:24,fontSize:10,color:"#adb5bd",padding:"10px 0",borderTop:"1px solid #dee2e6",fontFamily:MONO}}>
            PCI 8th Ed. Â· ACI 318-19 Â· CSA A23.3-19 Â· CPCI 5th Ed.
          </div>
        </div>

        {/* RIGHT: sticky 3D viewer â€” always visible */}
...
// Stub viewers for right panel â€” these render the appropriate 3D
// model using the shared state from each tab via a small context bridge
function PCIViewer3D(){ return <div style={{padding:12,textAlign:"center",color:"#868e96",fontSize:11,fontFamily:"'JetBrains Mono',monospace"}}>Switch to ðŸ§Š 3D Model in the chart picker to see the slab</div>; }
function CPCIViewer3D(){ return <div style={{padding:12,textAlign:"center",color:"#868e96",fontSize:11,fontFamily:"'JetBrains Mono',monospace"}}>Switch to ðŸ§Š 3D Model in the chart picker to see the slab</div>; }
function ColViewer3D(){ return <div style={{padding:12,textAlign:"center",color:"#868e96",fontSize:11,fontFamily:"'JetBrains Mono',monospace"}}>Switch to ðŸ§Š 3D Model in the chart picker to see the column</div>; }
function CrushViewer3D(){ return <div style={{padding:12,textAlign:"center",color:"#868e96",fontSize:11,fontFamily:"'JetBrains Mono',monospace"}}>Switch to ðŸ§Š 3D Model in the chart picker to see the section</div>; }
