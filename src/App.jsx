...
// ═════════════════════════════════════════
// PROJECTS BROWSER — shared, multi-user saved file system
// Project folder > Part subfolder > saved calc files
// ═════════════════════════════════════════
const MODULE_LABELS = { pci:"PCI Hollowcore", cpci:"CPCI Hollowcore", col:"Column Design", crush:"HC End Crushing", beam:"Beam Design" };

// ═══════════════════════════════════════════════════════════════
// DASHBOARD — Home screen with project stats, recent activity,
// team overview, and quick-access to recent files.
// ═══════════════════════════════════════════════════════════════
function Dashboard({ onOpenCalc, onNavigate }) {
  ...
  if (loading) return (
    <div style={{ padding:40, textAlign:"center", color:"#868e96", fontFamily:MONO }}>
      Loading dashboard…
    </div>
  );
  ...
  const totalParts    = [...new Set(calcs.map(c => `${c.project_name}::${c.part_name}`))].length;
  const totalUsers    = [...new Set(calcs.map(c => c.created_by_id))].length;

  // Module breakdown
  const moduleCounts = calcs.reduce((acc, c) => {
    acc[c.module] = (acc[c.module] || 0) + 1;
    return acc;
  }, {});

  // Recent files (last 8, across all users for admins)
  const recentFiles = [...calcs]
    .sort((a, b) => new Date(b.updated_at || b.created_at) - new Date(a.updated_at || a.created_at))
    .slice(0, 8);

  // Recent activity by user (admins see everyone)
  const activityByUser = Object.entries(EMPLOYEES).map(([id, emp]) => {
    const userCalcs = calcs.filter(c => c.created_by_id === id);
    const lastFile = userCalcs.sort((a, b) => new Date(b.updated_at || b.created_at) - new Date(a.updated_at || a.created_at))[0];
    return { id, name: emp.name, role: emp.role, count: userCalcs.length, lastFile };
  }).filter(u => u.count > 0 || u.id === user?.id)
    .sort((a, b) => {
      if (!a.lastFile) return 1;
      if (!b.lastFile) return -1;
      return new Date(b.lastFile.updated_at || b.lastFile.created_at) - new Date(a.lastFile.updated_at || a.lastFile.created_at);
    });

  // Module colors + labels
  const MODULE_INFO = {
    pci:   { label:"PCI Hollowcore",  color:"#2563eb", icon:"🔷" },
    cpci:  { label:"CPCI Hollowcore", color:"#7c3aed", icon:"🔶" },
    col:   { label:"Column Design",   color:"#16a34a", icon:"🟩" },
    crush: { label:"HC End Crushing", color:"#ea580c", icon:"🟠" },
    beam:  { label:"Beam Design",     color:"#0ea5e9", icon:"🔹" },
  };
  ...
  const StatCard = ({ icon, value, label, color="#212529", onClick }) => (
    <div onClick={onClick} style={{
      background:"#fff", border:"1px solid #dee2e6", borderRadius:8, padding:"16px 20px",
      ...
      <div style={{ fontSize:11, color:"#868e96", marginTop:2 }}>{label}</div>
    </div>
  );
  ...
  return (
    <div>
      {/* ── Welcome header ── */}
      <div style={{ marginBottom:24, padding:"20px 24px", background:"linear-gradient(135deg,#1e3a5f 0%,#2563eb 100%)",
        borderRadius:10, color:"#fff" }}>
        <div style={{ fontSize:20, fontWeight:800, fontFamily:MONO, marginBottom:4 }}>
          {isAdmin(user) ? "👑" : "👋"} Welcome back, {user?.name?.split(" ")[0]}
        </div>
        <div style={{ fontSize:12, opacity:0.8 }}>
          {new Date().toLocaleDateString("en-US", { weekday:"long", year:"numeric", month:"long", day:"numeric" })}
          &nbsp;·&nbsp; BT Structural Calculator &nbsp;·&nbsp;
          {isAdmin(user) ? "Admin — full access" : "Employee access"}
        </div>
        {calcs.length > 0 && (
          <div style={{ marginTop:12, fontSize:12, opacity:0.9 }}>
            You have <b>{calcs.filter(c=>c.created_by_id===user?.id).length}</b> saved files
            across <b>{[...new Set(calcs.filter(c=>c.created_by_id===user?.id).map(c=>c.project_name))].length}</b> projects.
          </div>
        )}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))", gap:12, marginBottom:24 }}>
        <StatCard icon="📁" value={totalProjects} label="Projects" color="#2563eb"
          onClick={()=>onNavigate("projects")}/>
        <StatCard icon="🗂️" value={totalParts} label="Parts" color="#7c3aed"/>
        <StatCard icon="📄" value={totalFiles} label="Saved Files" color="#16a34a"/>
        {isAdmin(user) && <StatCard icon="👥" value={totalUsers} label="Active Users" color="#ea580c"/>}
        <StatCard icon="📅" value={timeAgo(recentFiles[0]?.updated_at||recentFiles[0]?.created_at)}
          label="Last Activity" color="#0891b2"/>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:24 }}>
        ...
        <div style={{ background:"#fff", border:"1px solid #dee2e6", borderRadius:8, overflow:"hidden" }}>
          <div style={{ padding:"12px 16px", borderBottom:"1px solid #dee2e6", background:"#f8f9fa",
            display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontWeight:700, fontSize:13, fontFamily:MONO }}>🕐 Recent Files</span>
            <button onClick={()=>onNavigate("projects")} style={{ fontSize:10, color:"#2563eb",
              background:"none", border:"none", cursor:"pointer", fontFamily:MONO }}>View all →</button>
          </div>
          {recentFiles.length === 0
            ? <div style={{ padding:20, textAlign:"center", color:"#868e96", fontSize:12 }}>No files saved yet</div>
            : recentFiles.map((f, i) => {
                const mi = MODULE_INFO[f.module] || { label:f.module, color:"#868e96", icon:"📄" };
                return (
                  <div key={i} onClick={()=>onOpenCalc(f)} style={{
                    display:"flex", alignItems:"center", gap:10, padding:"10px 16px",
                    borderBottom:"1px solid #f1f3f5", cursor:"pointer", transition:"background 0.1s",
                  }}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      ...
                      <div style={{ fontSize:10, color:"#868e96", marginTop:1 }}>
                        <span style={{ color:mi.color, fontWeight:600 }}>{mi.label}</span>
                        {" · "}{f.project_name}/{f.part_name}
                        {isAdmin(user) && ` · ${f.created_by_name}`}
                      </div>
                    </div>
                    <div style={{ fontSize:10, color:"#adb5bd", whiteSpace:"nowrap", fontFamily:MONO }}>
        ...
        <div style={{ background:"#fff", border:"1px solid #dee2e6", borderRadius:8, overflow:"hidden" }}>
          <div style={{ padding:"12px 16px", borderBottom:"1px solid #dee2e6", background:"#f8f9fa" }}>
            <span style={{ fontWeight:700, fontSize:13, fontFamily:MONO }}>📊 Module Usage</span>
          </div>
          <div style={{ padding:"12px 16px" }}>
            {totalFiles === 0
              ? <div style={{ textAlign:"center", color:"#868e96", fontSize:12, padding:20 }}>No data yet</div>
              : Object.entries(MODULE_INFO).map(([key, mi]) => {
                  ...
                  return (
                    <div key={key} style={{ marginBottom:10 }}>
                      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:700, fontFamily:MONO, color:mi.color }}>
                          {count} file{count!==1?"s":""}
                        </span>
                      </div>
                      <div style={{ height:6, background:"#f1f3f5", borderRadius:3, overflow:"hidden" }}>
                        <div style={{ height:"100%", width:`${pct}%`, background:mi.color,
          ...
          {/* Quick actions */}
          <div style={{ padding:"10px 16px", borderTop:"1px solid #dee2e6", background:"#f8f9fa" }}>
            <div style={{ fontSize:11, fontWeight:700, fontFamily:MONO, marginBottom:8, color:"#374151" }}>
              ⚡ Quick Start
            </div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {Object.entries(MODULE_INFO).map(([key, mi]) => (
                <button key={key} onClick={()=>onNavigate(key)} style={{
                  padding:"4px 10px", borderRadius:12, border:`1px solid ${mi.color}30`,
                  background:`${mi.color}10`, color:mi.color, fontSize:10,
                  fontFamily:MONO, cursor:"pointer", fontWeight:600,
                }}>
                  {mi.icon} {key.toUpperCase()}
                </button>
              ))}
            </div>
          </div>
      ...
      {isAdmin(user) && activityByUser.length > 0 && (
        <div style={{ background:"#fff", border:"1px solid #dee2e6", borderRadius:8, overflow:"hidden", marginBottom:16 }}>
          <div style={{ padding:"12px 16px", borderBottom:"1px solid #dee2e6", background:"#f8f9fa" }}>
            <span style={{ fontWeight:700, fontSize:13, fontFamily:MONO }}>👥 Team Activity</span>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))" }}>
            {activityByUser.map((u, i) => {
              const empData = EMPLOYEES[u.id] || {};
              return (
                <div key={i} style={{ padding:"14px 16px", borderRight:"1px solid #f1f3f5",
                  borderBottom:"1px solid #f1f3f5" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                    <span style={{ fontSize:20 }}>{empData.role==="admin"?"👑":"👤"}</span>
                    <div>
                      <div style={{ fontWeight:700, fontSize:12 }}>{u.name}</div>
                      <div style={{ fontSize:10, color:"#868e96", fontFamily:MONO }}>{u.id}</div>
                    </div>
                  </div>
                  <div style={{ display:"flex", gap:12, marginBottom:6 }}>
                    <span style={{ fontSize:11, color:"#374151" }}>
                      <b>{u.count}</b> file{u.count!==1?"s":""}
                    </span>
                    <span style={{ fontSize:11, color:"#374151" }}>
                      <b>{[...new Set(calcs.filter(c=>c.created_by_id===u.id).map(c=>c.project_name))].length}</b> project{[...new Set(calcs.filter(c=>c.created_by_id===u.id).map(c=>c.project_name))].length!==1?"s":""}
                    </span>
                  </div>
                  {u.lastFile && (
                    <div style={{ fontSize:10, color:"#868e96", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                      Last: <span style={{ color:"#495057" }}>{u.lastFile.calc_name}</span>
                      {" · "}{timeAgo(u.lastFile.updated_at||u.lastFile.created_at)}
                    </div>
                  )}
                  {!u.lastFile && (
                    <div style={{ fontSize:10, color:"#adb5bd", fontStyle:"italic" }}>No activity yet</div>
                  )}
                </div>
              );
            })}
      ...
      {totalProjects > 0 && (
        <div style={{ background:"#fff", border:"1px solid #dee2e6", borderRadius:8, overflow:"hidden" }}>
          <div style={{ padding:"12px 16px", borderBottom:"1px solid #dee2e6", background:"#f8f9fa",
            display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontWeight:700, fontSize:13, fontFamily:MONO }}>📁 Projects Overview</span>
            <button onClick={()=>onNavigate("projects")} style={{ fontSize:10, color:"#2563eb",
              background:"none", border:"none", cursor:"pointer", fontFamily:MONO }}>Open Projects →</button>
          </div>
          <div style={{ padding:"12px 16px", display:"flex", flexWrap:"wrap", gap:8 }}>
            {[...new Set(calcs.map(c=>c.project_name))].sort().map(p => {
              const projFiles = calcs.filter(c=>c.project_name===p);
              const projParts = [...new Set(projFiles.map(c=>c.part_name))].length;
              const lastUpdate = projFiles.sort((a,b)=>new Date(b.updated_at||b.created_at)-new Date(a.updated_at||a.created_at))[0];
              const owners = [...new Set(projFiles.map(c=>c.created_by_name))];
              return (
                <div key={p} onClick={()=>onNavigate("projects")} style={{
                  padding:"10px 14px", border:"1px solid #dee2e6", borderRadius:6,
                  cursor:"pointer", minWidth:160, transition:"box-shadow 0.15s", background:"#fff",
                }}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.08)"}
                  onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                  <div style={{ fontSize:18, marginBottom:4 }}>📁</div>
                  <div style={{ fontWeight:700, fontSize:12, marginBottom:4 }}>{p}</div>
                  <div style={{ fontSize:10, color:"#868e96" }}>
                    {projParts} part{projParts!==1?"s":""} · {projFiles.length} file{projFiles.length!==1?"s":""}
                  </div>
                  {isAdmin(user) && owners.length > 0 && (
                    <div style={{ fontSize:10, color:"#9ca3af", marginTop:2, overflow:"hidden",
                      textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{owners.join(", ")}</div>
                  )}
                  <div style={{ fontSize:10, color:"#adb5bd", marginTop:4, fontFamily:MONO }}>
                    {timeAgo(lastUpdate?.updated_at||lastUpdate?.created_at)}
                  </div>
                </div>
              );
            })}
      ...
      {totalFiles === 0 && (
        <div style={{ textAlign:"center", padding:"40px 20px", background:"#f8f9fa",
          border:"2px dashed #dee2e6", borderRadius:10 }}>
          <div style={{ fontSize:48, marginBottom:12 }}>🏗️</div>
          <div style={{ fontSize:16, fontWeight:700, marginBottom:8 }}>No calculations yet</div>
          <div style={{ fontSize:13, color:"#868e96", marginBottom:16 }}>
            Start a new calculation from any of the design tabs above.
          </div>
          <div style={{ display:"flex", gap:8, justifyContent:"center", flexWrap:"wrap" }}>
            {Object.entries(MODULE_INFO).map(([key, mi]) => (
              <button key={key} onClick={()=>onNavigate(key)} style={{
                padding:"8px 16px", borderRadius:6, border:`1px solid ${mi.color}`,
                background:`${mi.color}15`, color:mi.color, fontSize:11,
...
function ProjectsBrowser({onOpenCalc}){
  ...
  const [calcs,setCalcs]=useState([]);
  const [loading,setLoading]=useState(true);
  const [view,setView]=useState({level:"projects", project:null, part:null});
  ...
  const reload = async()=>{
    setLoading(true);
    const data = await fetchCalcsForUser(user);
    setCalcs(data);
    setLoading(false);
  };
  useEffect(()=>{ reload(); },[user?.id,user?.role]);

  // Load user roles when admin panel is opened
  useEffect(()=>{
    if(showAdmin && isAdmin(user)){
      fetchAllUserRoles().then(setUserRoles);
    }
  },[showAdmin]);

  const handleRoleToggle = async(targetId, currentRole) => {
    const newRole = currentRole === "admin" ? "user" : "admin";
    const ok = await setUserRole(targetId, newRole, user?.id);
    if(ok){
      setRoleMsg(`✓ ${targetId} is now ${newRole}`);
      fetchAllUserRoles().then(setUserRoles);
  ...
  const parts = view.project ? [...new Set(calcs.filter(c=>c.project_name===view.project).map(c=>c.part_name))].sort() : [];
  const files = (view.project&&view.part) ? calcs.filter(c=>c.project_name===view.project && c.part_name===view.part).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)) : [];
  ...
  const crumbStyle = {cursor:"pointer",color:"#2563eb",fontWeight:600};
  const MONO = "'JetBrains Mono','Fira Code','Consolas',monospace";
      ...
      {isAdmin(user) && (
        <div style={{marginBottom:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:showAdmin?10:0}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:11,fontWeight:700,background:"#1d4ed8",color:"#fff",padding:"2px 10px",borderRadius:10,fontFamily:MONO}}>
                👑 ADMIN
              </span>
              <span style={{fontSize:11,color:"#6c757d"}}>
                {isAdmin(user)?"Viewing all employees' projects":"Viewing your projects only"}
              </span>
            </div>
            <button onClick={()=>setShowAdmin(v=>!v)}
              style={{padding:"4px 12px",border:"1px solid #ced4da",borderRadius:4,background:"#fff",
                fontSize:11,cursor:"pointer",fontFamily:MONO,color:"#374151"}}>
          ...
          {showAdmin && (
            <div style={{background:"#f8f9fa",border:"1px solid #dee2e6",borderRadius:8,padding:16}}>
              <div style={{fontSize:13,fontWeight:700,marginBottom:12,color:"#212529"}}>
                🔐 Access Management
              </div>
              {roleMsg && (
                <div style={{padding:"6px 12px",background:"#dcfce7",border:"1px solid #bbf7d0",
                  borderRadius:4,fontSize:11,fontFamily:MONO,color:"#15803d",marginBottom:10}}>
                  {roleMsg}
                </div>
              )}

              {/* Employee list with role toggles */}
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  <thead>
                    <tr style={{background:"#e9ecef"}}>
                      {["Employee ID","Name","Current Role","Saved Files","Action"].map(h=>(
                        <th key={h} style={{padding:"6px 10px",border:"1px solid #dee2e6",
                          textAlign:"left",fontWeight:700,fontFamily:MONO,fontSize:11}}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(EMPLOYEES).map(([id,emp])=>{
                      // Get effective role: runtime override takes precedence over hardcoded
                      const runtimeEntry = userRoles.find(r=>r.user_id===id);
                      const effectiveRole = runtimeEntry?.role || emp.role || "user";
                      const isHardcodedAdmin = emp.role === "admin";
                      const fileCount = calcs.filter(c=>c.created_by_id===id).length;
                      const isSelf = user?.id === id;

                      return(
                        <tr key={id} style={{background:isSelf?"#eff6ff":"#fff"}}>
                          <td style={{padding:"6px 10px",border:"1px solid #dee2e6",fontFamily:MONO,fontWeight:700}}>
                            {id} {isSelf&&<span style={{fontSize:9,color:"#2563eb"}}>(you)</span>}
                          </td>
                          <td style={{padding:"6px 10px",border:"1px solid #dee2e6"}}>{emp.name}</td>
                          <td style={{padding:"6px 10px",border:"1px solid #dee2e6"}}>
                            <span style={{
                              padding:"2px 8px",borderRadius:10,fontSize:10,fontWeight:700,fontFamily:MONO,
                              background:effectiveRole==="admin"?"#eff6ff":"#f3f4f6",
                              color:effectiveRole==="admin"?"#1d4ed8":"#6b7280"
                            }}>
                              {effectiveRole==="admin"?"👑 Admin":"👤 User"}
                            </span>
                            {isHardcodedAdmin&&<span style={{fontSize:9,color:"#868e96",marginLeft:4}}>(permanent)</span>}
                            {runtimeEntry&&<span style={{fontSize:9,color:"#16a34a",marginLeft:4}}>(granted)</span>}
                          </td>
                          <td style={{padding:"6px 10px",border:"1px solid #dee2e6",textAlign:"center",fontFamily:MONO}}>
                            {fileCount}
                          </td>
                          <td style={{padding:"6px 10px",border:"1px solid #dee2e6"}}>
                            {isHardcodedAdmin || isSelf
                              ? <span style={{fontSize:10,color:"#9ca3af"}}>—</span>
                              : (
                                <button
                                  onClick={()=>handleRoleToggle(id, effectiveRole)}
                                  style={{
                                    padding:"3px 10px",borderRadius:4,fontSize:10,cursor:"pointer",fontFamily:MONO,
                                    border:`1px solid ${effectiveRole==="admin"?"#fecaca":"#bbf7d0"}`,
                                    background:effectiveRole==="admin"?"#fef2f2":"#f0fdf4",
                                    color:effectiveRole==="admin"?"#dc2626":"#16a34a",fontWeight:700,
                                  }}>
                                  {effectiveRole==="admin"?"Revoke Admin":"Grant Admin"}
                                </button>
                              )}
                          </td>
                        </tr>
              ...
              {/* Project overview by user (admins see breakdown) */}
              <div style={{marginTop:14}}>
                <div style={{fontSize:12,fontWeight:700,marginBottom:8,color:"#374151"}}>Projects by Employee</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                  {Object.entries(EMPLOYEES).map(([id,emp])=>{
                    const count = calcs.filter(c=>c.created_by_id===id).length;
                    if(!count) return null;
                    return(
                      <div key={id} style={{padding:"6px 12px",background:"#fff",border:"1px solid #dee2e6",
                        borderRadius:6,fontSize:11,fontFamily:MONO}}>
                        <span style={{fontWeight:700,color:"#374151"}}>{emp.name}</span>
                        <span style={{color:"#868e96",marginLeft:6}}>{count} file{count!==1?"s":""}</span>
                      </div>
                    );
                  })}
                  {calcs.length===0&&<span style={{color:"#9ca3af",fontSize:11}}>No files saved yet</span>}
                </div>
              </div>

              {/* SQL setup reminder */}
              <details style={{marginTop:12}}>
                <summary style={{fontSize:10,color:"#868e96",cursor:"pointer",fontFamily:MONO}}>
                  ▶ Supabase setup (run once if role-granting isn't working)
                </summary>
                <pre style={{fontSize:10,background:"#1e293b",color:"#e2e8f0",padding:10,borderRadius:4,
                  marginTop:6,overflow:"auto",fontFamily:MONO}}>
{`CREATE TABLE IF NOT EXISTS user_roles (
  user_id TEXT PRIMARY KEY,
  role TEXT NOT NULL DEFAULT 'user',
  granted_by TEXT,
  granted_at TIMESTAMPTZ DEFAULT NOW()
      ...
      {!isAdmin(user) && (
        <div style={{marginBottom:12,padding:"6px 12px",background:"#f8f9fa",border:"1px solid #dee2e6",
          borderRadius:4,fontSize:11,color:"#6c757d",fontFamily:MONO}}>
          👤 You are viewing your own projects. Contact an admin to access shared projects.
        </div>
      )}

      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:16,fontSize:13,flexWrap:"wrap"}}>
        <span style={crumbStyle} onClick={()=>setView({level:"projects",project:null,part:null})}>📁 All Projects</span>
        {view.project && <>
          <span style={{color:"#adb5bd"}}>/</span>
          <span style={view.part?crumbStyle:{fontWeight:700}} onClick={()=>setView({level:"parts",project:view.project,part:null})}>{view.project}</span>
        </>}
        {view.part && <>
          <span style={{color:"#adb5bd"}}>/</span>
          <span style={{fontWeight:700}}>{view.part}</span>
        </>}
      </div>

      {loading && <div style={{color:"#868e96",fontSize:13}}>Loading saved files…</div>}

      {!loading && view.level==="projects" && (
        <div>
          {projects.length===0 && <div style={{color:"#868e96",fontSize:13,padding:20,textAlign:"center",background:"#f8f9fa",borderRadius:6}}>No projects saved yet. Use the 💾 Save button inside any calculator tab to create one.</div>}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
            {projects.map(p=>{
              const count = calcs.filter(c=>c.project_name===p).length;
              // For admins: show distinct owners of this project
              const owners = isAdmin(user)
                ? [...new Set(calcs.filter(c=>c.project_name===p).map(c=>c.created_by_name))].join(", ")
                : null;
              return(
                <div key={p} onClick={()=>setView({level:"parts",project:p,part:null})} style={{cursor:"pointer",padding:16,border:"1px solid #dee2e6",borderRadius:8,background:"#fff",transition:"box-shadow 0.15s"}}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 10px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                  <div style={{fontSize:24,marginBottom:6}}>📁</div>
                  <div style={{fontWeight:700,fontSize:13}}>{p}</div>
                  <div style={{fontSize:11,color:"#868e96",marginTop:2}}>{count} file{count!==1?"s":""}</div>
                  {owners && <div style={{fontSize:10,color:"#9ca3af",marginTop:4,fontStyle:"italic"}}>{owners}</div>}
                </div>
              );
            })}
          </div>
      ...
      {!loading && view.level==="parts" && (
        <div>
          {parts.length===0 && <div style={{color:"#868e96",fontSize:13}}>No parts in this project yet.</div>}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
            {parts.map(pt=>{
              const count = calcs.filter(c=>c.project_name===view.project && c.part_name===pt).length;
              return(
                <div key={pt} onClick={()=>setView({level:"files",project:view.project,part:pt})} style={{cursor:"pointer",padding:16,border:"1px solid #dee2e6",borderRadius:8,background:"#fff",transition:"box-shadow 0.15s"}}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 10px rgba(0,0,0,0.08)"} onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                  <div style={{fontSize:22,marginBottom:6}}>🗂️</div>
                  <div style={{fontWeight:700,fontSize:13}}>{pt}</div>
                  <div style={{fontSize:11,color:"#868e96",marginTop:2}}>{count} file{count!==1?"s":""}</div>
                </div>
              );
            })}
      ...
      {!loading && view.level==="files" && (
        <div>
          {files.length===0 && <div style={{color:"#868e96",fontSize:13}}>No files in this part yet.</div>}
          {files.map(f=>{
            const isOwner = user && (f.created_by_id===user.id || isAdmin(user));
            return(
              <div key={f.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",border:"1px solid #dee2e6",borderRadius:6,marginBottom:8,background:"#fff"}}>
                <div style={{fontSize:20}}>📄</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:700,fontSize:13}}>{f.calc_name}</div>
                  <div style={{fontSize:11,color:"#868e96"}}>
                    {MODULE_LABELS[f.module]||f.module} · saved by {f.created_by_name} · {new Date(f.created_at).toLocaleDateString()} {new Date(f.created_at).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}
                  </div>
                </div>
                <button onClick={()=>onOpenCalc(f)} style={{padding:"6px 12px",borderRadius:4,border:"2px solid #2563eb",background:"#eff6ff",color:"#1d4ed8",fontWeight:700,fontSize:11,cursor:"pointer",whiteSpace:"nowrap"}}>Open</button>
                {isOwner && (
                  confirmDelete===f.id ? (
                    <div style={{display:"flex",gap:4}}>
                      <button onClick={()=>handleDelete(f.id)} style={{padding:"6px 10px",borderRadius:4,border:"none",background:"#c0392b",color:"#fff",fontWeight:700,fontSize:11,cursor:"pointer"}}>Confirm</button>
                      <button onClick={()=>setConfirmDelete(null)} style={{padding:"6px 10px",borderRadius:4,border:"1px solid #ced4da",background:"#fff",fontSize:11,cursor:"pointer"}}>✕</button>
                    </div>
                  ) : (
                    <button onClick={()=>setConfirmDelete(f.id)} title="Delete (only you can do this)" style={{padding:"6px 10px",borderRadius:4,border:"1px solid #f5c6cb",background:"#fdecea",color:"#a52a1f",fontSize:11,cursor:"pointer"}}>🗑</button>
                  )
                )}
              </div>
            );
...
// ═════════════════════════════════════════
// WORKSPACE PICKER — the post-login landing screen.
// User must pick (or create) a Project, then a Part inside it,
// before reaching any calculator. Mirrors a real file-folder workflow.
// ═════════════════════════════════════════
function WorkspacePicker({userName, onSignOut, onEnter, onOpenCalc}){
  const user = useCurrentUser();
  const [calcs,setCalcs]=useState([]);
  const [loading,setLoading]=useState(true);
  // For admins: 4 levels. For users: 3 levels (skip employee level)
  const [stage,setStage]=useState(isAdmin(user) ? "employees" : "projects");
  const [selEmployee,setSelEmployee]=useState(null);  // admin only
  ...
  // Derived lists
  // Employees who have saved at least one calc (admins see all)
  const employeeList = isAdmin(user)
    ? Object.entries(EMPLOYEES).map(([id,emp])=>({
        id, name:emp.name, role:emp.role,
        count: calcs.filter(c=>c.created_by_id===id).length,
      })).filter(e=>e.count>0)
    : [];

  // When admin has selected an employee, filter calcs to that employee
  const filteredCalcs = isAdmin(user) && selEmployee
    ? calcs.filter(c=>c.created_by_id===selEmployee)
    : calcs;
  ...
  const openEmployee=(id)=>{ setSelEmployee(id); setSelProject(null); setSelPart(null); setStage("projects"); };
  const openProject=(p)=>{ setSelProject(p); setSelPart(null); setStage("parts"); };
  const openPart=(pt)=>{ setSelPart(pt); setStage("files"); };

  const goBack=()=>{
    if(stage==="files"){ setStage("parts"); setSelPart(null); }
    else if(stage==="parts"){ setStage("projects"); setSelProject(null); }
    else if(stage==="projects" && isAdmin(user)){ setStage("employees"); setSelEmployee(null); }
  };

  const startNewProject=()=>{ if(!newName.trim()) return; setSelProject(newName.trim()); setSelPart(null); setNewName(""); setNewProjectOpen(false); setStage("parts"); };
  const startNewPart=()=>{ if(!newName.trim()) return; setSelPart(newName.trim()); setNewName(""); setNewPartOpen(false); setStage("files"); };
  const handleDelete=async(id)=>{ const res=await deleteCalc(id); if(res.ok){ setConfirmDelete(null); reload(); } };

  const cardS={cursor:"pointer",padding:18,border:"1px solid #dee2e6",borderRadius:8,background:"#fff",transition:"box-shadow 0.15s"};
  const crumbS={cursor:"pointer",color:"#2563eb",fontWeight:600};
  const MONO="'JetBrains Mono','Fira Code','Consolas',monospace";

  // Find selected employee info
  const selEmpInfo = selEmployee ? EMPLOYEES[selEmployee] : null;

  return(
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",maxWidth:900,margin:"0 auto",padding:"16px 12px",color:"#212529",background:"#fff",minHeight:"100vh"}}>
      {/* Header */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",borderBottom:"2px solid #212529",paddingBottom:10,marginBottom:18,flexWrap:"wrap",gap:10}}>
        <div>
          <h1 style={{fontSize:17,fontWeight:800,margin:0,fontFamily:MONO,letterSpacing:-0.5,textTransform:"uppercase"}}>BT Structural Calculator</h1>
          <p style={{fontSize:11,color:"#868e96",margin:"3px 0 0"}}>
            {isAdmin(user) ? "Admin view — all employees' projects" : "Select a project and part to begin"}
          </p>
        </div>
        <div style={{textAlign:"right",display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            {isAdmin(user)&&<span style={{fontSize:10,fontWeight:800,background:"#1d4ed8",color:"#fff",padding:"2px 8px",borderRadius:10,fontFamily:MONO}}>👑 ADMIN</span>}
            <div style={{fontSize:12,fontWeight:600,color:"#495057"}}>{userName}</div>
          </div>
          <button onClick={onSignOut} style={{fontSize:10,color:"#868e96",background:"none",border:"none",cursor:"pointer",textDecoration:"underline",padding:0,fontFamily:MONO}}>Sign out</button>
        </div>
      </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:18,fontSize:13,flexWrap:"wrap"}}>
        {isAdmin(user) && (
          <span style={stage!=="employees"?crumbS:{fontWeight:700}}
            onClick={()=>{ if(stage!=="employees"){setStage("employees");setSelEmployee(null);setSelProject(null);setSelPart(null); }}}>
            👥 All Employees
          </span>
        )}
        {!isAdmin(user) && (
          <span style={stage==="projects"?{fontWeight:700}:crumbS}
            onClick={()=>{setStage("projects");setSelProject(null);setSelPart(null);}}>
            📁 All Projects
          </span>
        )}
        {selEmployee && (<>
          ...
          <span style={stage==="projects"?{fontWeight:700}:crumbS}
            onClick={()=>{setStage("projects");setSelProject(null);setSelPart(null);}}>
            👤 {selEmpInfo?.name||selEmployee}
          </span>
        </>)}
        {selProject && (<>
          <span style={{color:"#adb5bd"}}>/</span>
          <span style={stage==="parts"?{fontWeight:700}:crumbS}
            onClick={()=>{setStage("parts");setSelPart(null);}}>
            📁 {selProject}
          </span>
        </>)}
        {selPart && (<>
          <span style={{color:"#adb5bd"}}>/</span>
          <span style={{fontWeight:700}}>🗂️ {selPart}</span>
        </>)}
      </div>

      {loading && <div style={{color:"#868e96",fontSize:13,padding:20,textAlign:"center"}}>Loading…</div>}

      {/* ── LEVEL 0 (admin): Employees ── */}
      {!loading && stage==="employees" && isAdmin(user) && (
        <div>
          <div style={{fontSize:12,color:"#6c757d",marginBottom:12,fontFamily:MONO}}>
            Select an employee to browse their projects, or create a new project under your own account.
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12,marginBottom:16}}>
            {Object.entries(EMPLOYEES).map(([id,emp])=>{
              const count=calcs.filter(c=>c.created_by_id===id).length;
              const projCount=[...new Set(calcs.filter(c=>c.created_by_id===id).map(c=>c.project_name))].length;
              const isSelf=user?.id===id;
              return(
                <div key={id} onClick={()=>openEmployee(id)} style={{...cardS,borderLeft:`4px solid ${emp.role==="admin"?"#1d4ed8":"#16a34a"}`,opacity:count===0?0.55:1}}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow="0 3px 12px rgba(0,0,0,0.1)"}
                  onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                  <div style={{fontSize:28,marginBottom:6}}>{emp.role==="admin"?"👑":"👤"}</div>
                  <div style={{fontWeight:800,fontSize:14}}>{emp.name}</div>
                  <div style={{fontSize:10,fontFamily:MONO,color:"#868e96",marginTop:2}}>{id}{isSelf?" (you)":""}</div>
                  <div style={{marginTop:8,display:"flex",gap:10}}>
                    <span style={{fontSize:11,color:"#374151"}}><b>{projCount}</b> project{projCount!==1?"s":""}</span>
                    <span style={{fontSize:11,color:"#374151"}}><b>{count}</b> file{count!==1?"s":""}</span>
                  </div>
                  {emp.role==="admin"&&<div style={{fontSize:9,fontFamily:MONO,color:"#1d4ed8",marginTop:4,fontWeight:700}}>ADMIN</div>}
                </div>
              );
            })}
          </div>
          {/* Quick stats */}
          <div style={{padding:"10px 14px",background:"#f8f9fa",border:"1px solid #dee2e6",borderRadius:6,fontSize:11,color:"#495057",fontFamily:MONO}}>
            Total across all employees: <b>{calcs.length}</b> files · <b>{[...new Set(calcs.map(c=>c.project_name))].length}</b> projects
          </div>
        </div>
      )}
      ...
      {!loading && stage==="projects" && (
        <div>
          {selEmployee && (
            <div style={{padding:"8px 12px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:6,marginBottom:12,fontSize:12,display:"flex",alignItems:"center",gap:8}}>
              <span style={{fontSize:18}}>👤</span>
              <span>Browsing <b>{selEmpInfo?.name||selEmployee}</b>'s projects</span>
              <button onClick={()=>{setStage("employees");setSelEmployee(null);}} style={{marginLeft:"auto",fontSize:10,padding:"3px 8px",border:"1px solid #bfdbfe",borderRadius:4,background:"#fff",cursor:"pointer",color:"#2563eb"}}>← All Employees</button>
            </div>
          )}
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:12}}>
            {projects.map(p=>{
              const count=filteredCalcs.filter(c=>c.project_name===p).length;
              const partCount=[...new Set(filteredCalcs.filter(c=>c.project_name===p).map(c=>c.part_name))].length;
              return(
                <div key={p} onClick={()=>openProject(p)} style={cardS}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 10px rgba(0,0,0,0.08)"}
                  onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                  <div style={{fontSize:26,marginBottom:6}}>📁</div>
                  <div style={{fontWeight:700,fontSize:14}}>{p}</div>
                  <div style={{fontSize:11,color:"#868e96",marginTop:4}}>{partCount} part{partCount!==1?"s":""} · {count} file{count!==1?"s":""}</div>
                </div>
              );
            })}
            {/* Only show New Project if browsing your own account or not an admin */}
            {(!isAdmin(user)||selEmployee===user?.id) && (
              <div onClick={()=>setNewProjectOpen(true)} style={{...cardS,border:"2px dashed #ced4da",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#868e96"}}>
                <div style={{fontSize:26,marginBottom:6}}>＋</div>
                <div style={{fontWeight:700,fontSize:13}}>New Project</div>
              </div>
            )}
          </div>
          {projects.length===0&&<div style={{color:"#868e96",fontSize:13,padding:16,textAlign:"center",background:"#f8f9fa",borderRadius:6}}>No projects yet.</div>}
        </div>
      )}
      ...
      {!loading && stage==="parts" && (
        <div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:12}}>
            {parts.map(pt=>{
              const count=filteredCalcs.filter(c=>c.project_name===selProject&&c.part_name===pt).length;
              return(
                <div key={pt} onClick={()=>openPart(pt)} style={cardS}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow="0 2px 10px rgba(0,0,0,0.08)"}
                  onMouseLeave={e=>e.currentTarget.style.boxShadow="none"}>
                  <div style={{fontSize:24,marginBottom:6}}>🗂️</div>
                  <div style={{fontWeight:700,fontSize:14}}>{pt}</div>
                  <div style={{fontSize:11,color:"#868e96",marginTop:4}}>{count} file{count!==1?"s":""}</div>
                </div>
              );
            })}
            {(!isAdmin(user)||selEmployee===user?.id) && (
              <div onClick={()=>setNewPartOpen(true)} style={{...cardS,border:"2px dashed #ced4da",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"#868e96"}}>
                <div style={{fontSize:24,marginBottom:6}}>＋</div>
                <div style={{fontWeight:700,fontSize:13}}>New Part</div>
              </div>
            )}
          </div>
          {parts.length===0&&<div style={{color:"#868e96",fontSize:13,padding:16,textAlign:"center",background:"#f8f9fa",borderRadius:6}}>No parts in this project yet.</div>}
        </div>
      )}
      ...
      {!loading && stage==="files" && (
        <div>
          {(!isAdmin(user)||selEmployee===user?.id) && (
            <button onClick={()=>onEnter(selProject,selPart)} style={{width:"100%",padding:"13px",borderRadius:8,border:"none",background:"#212529",color:"#fff",fontWeight:800,fontSize:13,cursor:"pointer",marginBottom:14,fontFamily:MONO,textTransform:"uppercase",letterSpacing:0.5}}>
              ＋ Start New Calculation in this Part
            </button>
          )}
          {files.length===0&&<div style={{color:"#868e96",fontSize:13,padding:16,textAlign:"center",background:"#f8f9fa",borderRadius:6}}>No saved files here yet.</div>}
          {files.map(f=>{
            const isOwner=user&&(f.created_by_id===user.id||isAdmin(user));
            return(
              <div key={f.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",border:"1px solid #dee2e6",borderRadius:6,marginBottom:8,background:"#fff"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:700,fontSize:13}}>{f.calc_name}</div>
                  <div style={{fontSize:11,color:"#868e96"}}>
                    {MODULE_LABELS[f.module]||f.module} · <b>{f.created_by_name}</b> · {new Date(f.created_at).toLocaleDateString()} {new Date(f.created_at).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}
                  </div>
                </div>
                <button onClick={()=>onOpenCalc(f)} style={{padding:"6px 12px",borderRadius:4,border:"2px solid #2563eb",background:"#eff6ff",color:"#1d4ed8",fontWeight:700,fontSize:11,cursor:"pointer",whiteSpace:"nowrap"}}>Open</button>
                {isOwner&&(
                  confirmDelete===f.id
                    ...
                    : <button onClick={()=>setConfirmDelete(f.id)} style={{padding:"6px 10px",borderRadius:4,border:"1px solid #f5c6cb",background:"#fdecea",color:"#a52a1f",fontSize:11,cursor:"pointer"}}>🗑</button>
                )}
              </div>
            );
          })}
      ...
      {/* New Project modal */}
      {newProjectOpen&&(
        <div onClick={()=>{setNewProjectOpen(false);setNewName("");}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:8,padding:22,width:"100%",maxWidth:360,boxShadow:"0 12px 36px rgba(0,0,0,0.25)"}}>
            <div style={{fontSize:14,fontWeight:800,marginBottom:14}}>📁 New Project</div>
            <input autoFocus value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&startNewProject()} placeholder="e.g. 123 Main St Warehouse"
              style={{width:"100%",padding:"9px 12px",borderRadius:4,border:"2px solid #e8a838",background:"#fff8ef",fontSize:13,boxSizing:"border-box",marginBottom:14}}/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{setNewProjectOpen(false);setNewName("");}} style={{flex:1,padding:"9px",borderRadius:4,border:"1px solid #ced4da",background:"#fff",cursor:"pointer",fontSize:12,fontWeight:600}}>Cancel</button>
              <button onClick={startNewProject} style={{flex:1,padding:"9px",borderRadius:4,border:"none",background:"#212529",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:700}}>Create</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* New Part modal */}
      {newPartOpen&&(
        <div onClick={()=>{setNewPartOpen(false);setNewName("");}} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:8,padding:22,width:"100%",maxWidth:360,boxShadow:"0 12px 36px rgba(0,0,0,0.25)"}}>
            <div style={{fontSize:14,fontWeight:800,marginBottom:14}}>🗂️ New Part</div>
            <input autoFocus value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&startNewPart()} placeholder="e.g. Roof Slab"
              style={{width:"100%",padding:"9px 12px",borderRadius:4,border:"2px solid #e8a838",background:"#fff8ef",fontSize:13,boxSizing:"border-box",marginBottom:14}}/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{setNewPartOpen(false);setNewName("");}} style={{flex:1,padding:"9px",borderRadius:4,border:"1px solid #ced4da",background:"#fff",cursor:"pointer",fontSize:12,fontWeight:600}}>Cancel</button>
              <button onClick={startNewPart} style={{flex:1,padding:"9px",borderRadius:4,border:"none",background:"#212529",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:700}}>Create</button>
            </div>
          </div>
        </div>
      )}
...
function LoginScreen({onLogin}){
  ...
  const submit = async () => {
    ...
    if (u && u.pass === pwd) {
      setErr("");
      // Check if there's a runtime role override in Supabase
      ...
      onLogin({
        ...
        name: u.name,
        role: effectiveRole,
        runtimeAdmin: effectiveRole === "admin",
      });
    } else {
      setErr("Invalid employee ID or password.");
    }
  };
  ...
  return(
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",minHeight:"100vh",background:"#212529",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#fff",borderRadius:6,padding:"32px 28px",width:"100%",maxWidth:340,boxShadow:"0 10px 40px rgba(0,0,0,0.3)"}}>
        <div style={{textAlign:"center",marginBottom:20}}>
          <div style={{fontSize:28,marginBottom:6}}>🏗️</div>
          <h1 style={{fontSize:15,fontWeight:800,margin:0,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",textTransform:"uppercase",letterSpacing:0.5,color:"#212529"}}>Hollowcore Calculator</h1>
          <p style={{fontSize:11,color:"#868e96",margin:"4px 0 0"}}>Employee Access Only</p>
        </div>
        <div style={{marginBottom:12}}>
          <label style={{fontSize:11,fontWeight:600,color:"#495057",textTransform:"uppercase",letterSpacing:0.5,display:"block",marginBottom:4}}>Employee ID</label>
          <input autoFocus value={uid} onChange={e=>setUid(e.target.value)} onKeyDown={handleKeyDown} placeholder="e.g. engineer"
            style={{width:"100%",padding:"9px 12px",borderRadius:4,border:"1px solid #ced4da",fontSize:14,boxSizing:"border-box",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}/>
        </div>
        <div style={{marginBottom:16}}>
          <label style={{fontSize:11,fontWeight:600,color:"#495057",textTransform:"uppercase",letterSpacing:0.5,display:"block",marginBottom:4}}>Password</label>
          <input type="password" value={pwd} onChange={e=>setPwd(e.target.value)} onKeyDown={handleKeyDown} placeholder="••••••••"
            style={{width:"100%",padding:"9px 12px",borderRadius:4,border:"1px solid #ced4da",fontSize:14,boxSizing:"border-box",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}/>
        </div>
        {err && <div style={{fontSize:12,color:"#c0392b",background:"#fdecea",border:"1px solid #f5c6cb",borderRadius:4,padding:"6px 10px",marginBottom:14}}>{err}</div>}
        <button type="button" onClick={submit} style={{width:"100%",padding:"10px",borderRadius:4,border:"none",background:"#212529",color:"#fff",fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",textTransform:"uppercase",letterSpacing:0.5}}>
          Sign In
        </button>
        <p style={{fontSize:10,color:"#adb5bd",textAlign:"center",marginTop:14,marginBottom:0}}>Contact your administrator if you need access credentials.</p>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const [loadedCalc,setLoadedCalc]=useState(null); // {module, inputs, calc_name, ...} from Projects browser

  // Workspace gate: null = show project/part picker; {project,part} = inside a workspace
  ...
  const [reportStyle,setReportStyle]=useState("interactive"); // "interactive" | "tedds"
  const [menuOpen,setMenuOpen]=useState(false);

  // ── Calculation Options (ConCise-matching global settings) ──
  ...
  const calcOptionsState = {
    ...
    deflMult, setDeflMult,
    ulsCombos, setUlsCombos,
    slsCombos:DEFAULT_SLS_COMBOS,
    loadFactors, setLoadFactors,
    designCode, setDesignCode,
    units, setUnits: (fn)=>setUnits(fn),
    // File menu actions
    onPrint: ()=>handlePrint(false),
    onPrintPreview: ()=>handlePrint(false),
    onTextReports: ()=>{ setReportStyle("tedds"); handlePrint(false); },
    onGraphs: ()=>{ setReportStyle("interactive"); },
    onSave: ()=>{ /* triggers save in active tab — future: emit event */ },
    onSaveAs: ()=>{ /* triggers save-as in active tab */ },
    onNew: ()=>{ if(window.confirm("Start a new project? Unsaved changes will be lost.")) window.location.reload(); },
    recentFiles: [],
  };

  // ── Define dialogs state (ConCise-matching) ──
  ...
  const defineState = { concrete,setConcrete,beam,setBeam,cip,setCIP,
    rebar,setRebar,prestress,setPrestress,shear,setShear,designParams,setDesignParams };
  ...
  const handleOpenCalc=(fileRow)=>{
    ...
    setWorkspace({project:fileRow.project_name, part:fileRow.part_name});
  };

  // Keep the "Project / Job No." print field in sync with the active workspace
  useEffect(()=>{
    if(workspace) setProj(`${workspace.project} — ${workspace.part}`);
  },[workspace]);
  ...
  return(
    ...
    <div style={{fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",color:"#212529",background:"#fff",minHeight:"100vh"}}>
      {/* ConCise-style menu bar — full width, at the very top */}
      <div className="no-print">
        <OptionsMenuBar state={{...calcOptionsState, defineModal, setDefineModal}}/>
        <DefineDialogsController openModal={defineModal} setOpenModal={setDefineModal} defineState={defineState}/>
      </div>
      <div style={{maxWidth:860,margin:"0 auto",padding:"12px 12px"}}>
      <style>{`
        input.oi:focus,select.oi:focus{box-shadow:0 0 0 2px rgba(232,168,56,0.4);border-color:#c77c00}

        /* Live diagram animation: SVG numeric attrs animate smoothly on change */
        .live-svg circle,.live-svg ellipse,.live-svg rect,.live-svg line,.live-svg polygon,.live-svg path{
          transition: cx 0.35s ease, cy 0.35s ease, r 0.35s ease, x 0.35s ease, y 0.35s ease,
                      width 0.35s ease, height 0.35s ease, x1 0.35s ease, y1 0.35s ease,
                      x2 0.35s ease, y2 0.35s ease, points 0.35s ease, fill 0.3s ease,
                      stroke 0.3s ease, opacity 0.3s ease;
        }
        .live-svg text{ transition: fill 0.3s ease, opacity 0.3s ease; }
        ...
        .diagram-frame{ border-radius:6px; transition: border-color 0.35s ease, background 0.35s ease; }
        .diagram-frame.pulse{ animation: valuePulse 0.6s ease-out; }

        @keyframes numberFlash{
          0%{ color:#2563eb; }
          100%{ color:inherit; }
        }
        .num-flash{ animation: numberFlash 0.7s ease-out; }

        @media print{
          @page { margin: 18mm 15mm; size: A4; }
          /* ── B&W formal report — no decorative color ── */
          html,body{
          ...
          button,.no-print,.tab-bar,
          input,select,canvas{display:none!important}
          /* Page breaks */
          .print-card{page-break-inside:avoid;break-inside:avoid;}
          .print-section-head{page-break-after:avoid;break-after:avoid;}
          .print-tab-section{page-break-before:always;break-before:page;}
          .print-tab-section:first-of-type{page-break-before:auto;break-before:auto;}
          svg.live-svg{max-width:100%!important;page-break-inside:avoid;break-inside:avoid;}
          /* Strip all card colors — replace with plain black borders */
          .print-card{
            background:#fff!important;
          ...
          /* Card header: plain bold text on white, no tinted background */
          ...
          /* Section dividers: simple thick black line, no color */
          .print-section-head{
          ...
          /* Formula boxes: plain left-border only, no fill color */
          [style*="borderLeft"][style*="ced4da"]{
            background:#fff!important;
            border-left:2pt solid #555!important;
          }
          /* Pass/fail badges: text only, no colored background */
          ...
          /* Status banners: strip color, keep text */
          ...
          [style*="color:#4c1d95"],[style*="color: #4c1d95"],
          [style*="color:#14532d"],[style*="color: #14532d"],
          [style*="color:#7c2d12"],[style*="color: #7c2d12"],
          [style*="color:#0c4a6e"],[style*="color: #0c4a6e"]{
            color:#000!important;
          }
          /* Utilization bar: hide the colored fill bar */
          [style*="background:#22c55e"][style*="height: 8px"],
          [style*="background:#f59e0b"][style*="height: 8px"],
          [style*="background:#ef4444"][style*="height: 8px"]{
            background:#000!important;
          }
          /* Orange input cells → plain */
          [style*="background:#fff8ef"]{
            background:#fff!important;
            border:1pt solid #888!important;
          }
          /* Eq formula boxes → plain */
          ...
          /* Show the coversheet */
          .print-coversheet{display:block!important;}
          /* Tedds report: table borders */
          .tedds-doc table{
            page-break-inside:avoid;break-inside:avoid;
            border-collapse:collapse!important;
          }
          .tedds-doc td,.tedds-doc th{
            border:1pt solid #000!important;
          }
          /* Summary check grids */
          [style*="background:#f0fdf4"],[style*="background:#fef2f2"]{
            background:#fff!important;
          }
        }
        .print-coversheet{display:none;}
      `}</style>

      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",borderBottom:"2px solid #212529",paddingBottom:10,marginBottom:14,flexWrap:"wrap",gap:10}}>
        <div>
          <h1 style={{fontSize:17,fontWeight:800,margin:0,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",letterSpacing:-0.5,textTransform:"uppercase"}}>Hollowcore Slab & Column Calculator</h1>
          <p style={{fontSize:11,color:"#868e96",margin:"3px 0 0"}}>PCI &amp; CPCI Non-Composite Design · RC Column Interaction · Core Crushing</p>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:6}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"3px 10px",border:"2px solid #e8a838",borderRadius:3,background:"#fff8ef"}}>
              <span style={{fontSize:11,fontWeight:600,color:"#b07020",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>■ Orange = Editable Input</span>
            </div>
            <div style={{display:"inline-flex",alignItems:"center",gap:6,padding:"3px 10px",border:"1px solid #ced4da",borderRadius:3,background:"#f8f9fa"}}>
              <span style={{fontSize:11,fontWeight:600,color:"#495057",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>■ Code Ref = right of each formula</span>
            </div>
          </div>
        </div>
        <div className="no-print" style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8,position:"relative"}}>
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>setMenuOpen(v=>!v)} title="Choose what to show" style={{padding:"8px 14px",border:menuOpen?"2px solid #2563eb":"2px solid #212529",borderRadius:4,background:menuOpen?"#eff6ff":"#fff",color:menuOpen?"#1d4ed8":"#212529",cursor:"pointer",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",fontWeight:700,fontSize:11,display:"flex",alignItems:"center",gap:5}}>
              ⚙ DISPLAY OPTIONS {menuOpen?"▲":"▼"}
            </button>
            <button onClick={()=>handlePrint(false)} title="Print only the currently open tab" style={{padding:"8px 14px",border:"2px solid #212529",borderRadius:4,background:"#fff",cursor:"pointer",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",fontWeight:700,fontSize:11,display:"flex",alignItems:"center",gap:5}}>
              🖨 PRINT THIS TAB
            </button>
            <button onClick={()=>handlePrint(true)} title="Print all four sections as one report" style={{padding:"8px 14px",border:"2px solid #212529",borderRadius:4,background:"#212529",color:"#fff",cursor:"pointer",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",fontWeight:700,fontSize:11,display:"flex",alignItems:"center",gap:5}}>
              🖨 PRINT FULL REPORT
            </button>
          </div>

          {menuOpen && (
            <div style={{position:"absolute",top:42,right:0,zIndex:50,background:"#fff",border:"2px solid #212529",borderRadius:6,padding:14,width:270,boxShadow:"0 8px 24px rgba(0,0,0,0.15)"}}>
              <div style={{fontSize:11,fontWeight:800,textTransform:"uppercase",letterSpacing:0.5,color:"#212529",marginBottom:8,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>
                Report Style
              </div>
              <div style={{display:"flex",gap:6,marginBottom:12}}>
                <button onClick={()=>setReportStyle("interactive")} style={{flex:1,padding:"6px 4px",borderRadius:4,border:reportStyle==="interactive"?"2px solid #2563eb":"1px solid #ced4da",background:reportStyle==="interactive"?"#eff6ff":"#fff",color:reportStyle==="interactive"?"#1d4ed8":"#495057",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>Interactive</button>
                <button onClick={()=>setReportStyle("tedds")} style={{flex:1,padding:"6px 4px",borderRadius:4,border:reportStyle==="tedds"?"2px solid #2563eb":"1px solid #ced4da",background:reportStyle==="tedds"?"#eff6ff":"#fff",color:reportStyle==="tedds"?"#1d4ed8":"#495057",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>Tedds-Style</button>
              </div>
              <div style={{fontSize:10,color:"#868e96",marginBottom:12,marginTop:-6}}>
                {reportStyle==="tedds" ? "Formal narrative calc sheet, like Tekla Tedds output." : "Cards with live editable orange inputs."}
              </div>

              <div style={{fontSize:11,fontWeight:800,textTransform:"uppercase",letterSpacing:0.5,color:"#212529",marginBottom:10,paddingBottom:8,borderBottom:"1px solid #dee2e6",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>
                What to Show
              </div>

              <label style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,cursor:"pointer"}}>
                <input type="checkbox" checked={showSteps} onChange={e=>setShowSteps(e.target.checked)} style={{width:16,height:16,cursor:"pointer"}}/>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:"#212529"}}>Step-by-step formulas</div>
                  <div style={{fontSize:10,color:"#868e96"}}>Show every equation & code reference</div>
                </div>
              </label>

              <div style={{height:1,background:"#f1f3f5",margin:"10px 0"}}/>

              <label style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,cursor:"pointer"}}>
                <input type="checkbox" checked={showGraphics} onChange={e=>setShowGraphics(e.target.checked)} style={{width:16,height:16,cursor:"pointer"}}/>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:"#212529"}}>Graphics & diagrams</div>
                  <div style={{fontSize:10,color:"#868e96"}}>Cross-sections, stress diagrams, P–M curve</div>
                </div>
              </label>

              <label style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,cursor:"pointer"}}>
                <input type="checkbox" checked={showInputs} onChange={e=>setShowInputs(e.target.checked)} style={{width:16,height:16,cursor:"pointer"}}/>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:"#212529"}}>Inputs</div>
                  <div style={{fontSize:10,color:"#868e96"}}>Editable fields & section properties</div>
                </div>
              </label>

              <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}>
                <input type="checkbox" checked={showOutputs} onChange={e=>setShowOutputs(e.target.checked)} style={{width:16,height:16,cursor:"pointer"}}/>
                <div>
                  <div style={{fontSize:12,fontWeight:700,color:"#212529"}}>Outputs</div>
                  <div style={{fontSize:10,color:"#868e96"}}>Results, checks, and capacity status</div>
                </div>
              </label>
            </div>
          )}
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:11,fontWeight:600,color:"#495057"}}>{user.name}</div>
            <button onClick={()=>setUser(null)} style={{fontSize:10,color:"#868e96",background:"none",border:"none",cursor:"pointer",textDecoration:"underline",padding:0,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>Sign out</button>
          </div>
        </div>
      </div>

      <div className="no-print" style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:14,padding:"10px 12px",background:"#f8f9fa",border:"1px solid #dee2e6",borderRadius:4}}>
        <div style={{flex:1,minWidth:160}}>
          <span style={{fontSize:10,color:"#868e96",fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,display:"block",marginBottom:2}}>Project / Job No.</span>
          <input value={proj} onChange={e=>setProj(e.target.value)} placeholder="e.g. Project Name — Job #1234"
            style={{width:"100%",padding:"6px 9px",borderRadius:3,border:"1px solid #ced4da",fontSize:12,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",boxSizing:"border-box"}}/>
        </div>
        <div style={{flex:1,minWidth:160}}>
          <span style={{fontSize:10,color:"#868e96",fontWeight:600,textTransform:"uppercase",letterSpacing:0.5,display:"block",marginBottom:2}}>Prepared By</span>
          <input value={byName} onChange={e=>setByName(e.target.value)} placeholder={user.name}
            style={{width:"100%",padding:"6px 9px",borderRadius:3,border:"1px solid #ced4da",fontSize:12,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",boxSizing:"border-box"}}/>
        </div>
      </div>

      <div className="print-coversheet" style={{marginBottom:18,paddingBottom:14,borderBottom:"1px solid #dee2e6"}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:11,fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",color:"#495057"}}>
          <span><b>Project:</b> {proj||"—"}</span>
          <span><b>Prepared by:</b> {byName||user.name}</span>
          <span><b>Date:</b> {new Date().toLocaleDateString()}</span>
        </div>
      </div>

      <div className="no-print" style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,marginBottom:12,padding:"8px 12px",background:"#fff8ef",border:"2px solid #e8a838",borderRadius:6}}>
        <div style={{fontSize:13,fontWeight:700,color:"#b07020"}}>
          📁 {workspace.project} <span style={{color:"#868e96",fontWeight:400}}>/</span> 🗂️ {workspace.part}
        </div>
        <button onClick={()=>{setWorkspace(null);setLoadedCalc(null);}} style={{padding:"5px 10px",borderRadius:4,border:"1px solid #e8a838",background:"#fff",color:"#b07020",fontWeight:700,fontSize:11,cursor:"pointer"}}>↺ Switch Project / Part</button>
      </div>

      <div className="tab-bar no-print" style={{display:"flex",gap:0,marginBottom:14,borderBottom:"1px solid #dee2e6",flexWrap:"wrap"}}>
        {ALL_TABS.map(t=>{
          const th = t.mod ? MODULE_THEMES[t.mod] : { accent:"#374151", soft:"#f8f9fa", text:"#212529" };
          const active = tab===t.id;
          return (
            <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"7px 14px",border:"none",borderBottom:active?`3px solid ${th.accent}`:"3px solid transparent",cursor:"pointer",fontSize:11,fontWeight:active?700:500,background:active?th.soft:"transparent",color:active?th.text:"#868e96",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",textTransform:"uppercase",letterSpacing:0.5,marginBottom:-1,borderRadius:"4px 4px 0 0",transition:"all 0.15s"}}>{t.label}</button>
          );
        })}
        <button onClick={()=>setTab("projects")} style={{padding:"7px 14px",border:"none",borderBottom:tab==="projects"?"3px solid #495057":"3px solid transparent",cursor:"pointer",fontSize:11,fontWeight:tab==="projects"?700:500,background:tab==="projects"?"#f1f3f5":"transparent",color:tab==="projects"?"#212529":"#868e96",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace",textTransform:"uppercase",letterSpacing:0.5,marginBottom:-1,borderRadius:"4px 4px 0 0",transition:"all 0.15s"}}>📁 Projects</button>
      </div>
      ...
      <div onClick={()=>menuOpen&&setMenuOpen(false)}>
        {printAll ? (
          <div>
            <ModuleThemeContext.Provider value={MODULE_THEMES.pci}>
            <div className="print-tab-section">
              <SH>PCI Hollowcore Design</SH>
              <PCITab/>
            </div>
            </ModuleThemeContext.Provider>
            <ModuleThemeContext.Provider value={MODULE_THEMES.cpci}>
            <div className="print-tab-section">
              <SH>CPCI Hollowcore Design</SH>
              <CPCITab/>
            </div>
            </ModuleThemeContext.Provider>
            <ModuleThemeContext.Provider value={MODULE_THEMES.col}>
            <div className="print-tab-section">
              <SH>Column Design</SH>
              <ColTab/>
            </div>
            </ModuleThemeContext.Provider>
            <ModuleThemeContext.Provider value={MODULE_THEMES.crush}>
            <div className="print-tab-section">
              <SH>HC End Crushing</SH>
              <CrushTab/>
            </div>
            </ModuleThemeContext.Provider>
            <ModuleThemeContext.Provider value={MODULE_THEMES.beam}>
            <div className="print-tab-section">
              <SH>Beam Design (IT / Ledger / Double Tee / Box)</SH>
              <BeamTab/>
            </div>
            </ModuleThemeContext.Provider>
          </div>
      ...
      <div style={{textAlign:"center",marginTop:24,fontSize:10,color:"#adb5bd",padding:"10px 0",borderTop:"1px solid #dee2e6",fontFamily:"'JetBrains Mono','Fira Code','Consolas',monospace"}}>
        Ref: PCI Design Handbook 8th Ed. · CSA A23.3-19 · ACI 318-19 · CPCI Design Manual 5th Ed.
      </div>
